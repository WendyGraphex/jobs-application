...
class payments_employer_ajaxChangeIsActiveMethodAction extends mfAction
{
    ...
    function execute(mfWebRequest $request) {   
      ...
      {        
        $form=new ChangeForm();
        $form->bind($request->getPostParameter('PaymentEmployerMethod'));
        if (!$form->isValid())
             throw new mfException(__('Form has some errors'));    
        $item= new PaymentEmployerMethod($form->getValue('id'));    
        if ($item->isNotLoaded())
