...
<script type="text/javascript">                    
        ...
        $("#Send").click(function () { 
            var params = { EmployerMessage : { quotation : '{$item->get('id')}', id : $("#Message").attr('data-id'), token :'{mfForm::getToken('SendMessageForQuotationForm')}' } };
            $(".CustomerEmployerMessage.Input").each(function () { params.EmployerMessage[$(this).attr('name')]=$(this).val(); });
            return $.ajax2({      data :params,
                                  url :"{url_to('employers_messager_ajax',['action'=>'SendMessageForQuotation'])}",                                                                
                                  success: function (resp) 
                                        { 
                                            ...
                                            $(".NameUserQuotation").html("{$item->getEmployeeUser()->getFormatter()->getUsername()->ucfirst()}");
                                        }
                     });   
        });
</script> 