              ...
              {foreach $messages as $message}
                ...
                <div class="item-content flex-grow">
                  <h6 class="ellipsis font-weight-normal">
                     {if !$message->isSender()}{if $message->hasEmployeeUser()}{$message->getEmployeeUser()->getFormatter()->getFirstname()->ucfirst()} {$message->getEmployeeUser()->getFormatter()->getlastname()->ucfirst()}
...
<script type="text/javascript">
            ...
            $(function()
            {
                 $(".UnreadMessages").click(function () { 
                       return $.ajax2({                                
                                             url: "{url_to('employers_messager_ajax',['action'=>'ListMessage'])}", 
                                             target: "#page-wrapper",
                                             success : function () { 
                                                    $(".nav-item").removeClass('active');
                                                    $(".nav-item[id=600_employer_mymessages]").addClass('active'); 
                                                }
                       });
                 });
