...
{if $item->isLoaded()}
    ...
    <span>{__('Message from [%s] on [%s]',[(string)$item->getEmployerUser(),$item->getFormatter()->getCreatedAt()->getText()])}</span><br/>
    {else}
      <span>{__('Message from Administrator on [%s]',$item->getFormatter()->getCreatedAt()->getText())}</span><br/>
      {/if}
    ...
    <div>
     ...
     {/foreach} 
      ...
      <script type="text/javascript">                                    
         {if !$item->isSender()}
         $(".CustomerEmployerMessage.Read[id={$item->get('id')}]").html("");
         {/if}
</script> 
{else}
