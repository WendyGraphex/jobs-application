...
<div class="row admin-header-container btn-r">    
    <div class="col-sm p-0">
        <div class="pull-right">
              ...
              {if $formFilter.equal.sender->getValue() !='EMPLOYER'}     
                         ...
                         {foreach $pager as $item} 
                            <tr class="list" id="{$item->get('id')}" role="row">
                                ...
                                <td>{$item->getCreatedAt()->getText()}</td>  
                                 <td class="CustomerEmployerMessage Read" id="{$item->get('id')}">
                                        {if !$item->isRead()}
 ...
 <script type="text/javascript">
        ...
        {          
           return $.ajax2({ data: getFilterParameters(), 
                            url:"{url_to('employers_messager_ajax',['action'=>'ListPartialMessage'])}" ,                          
                            target: "#actions"
                             });
        }
        ...
        $(".pager").click(function () {                     
                return $.ajax2({ data: getFilterParameters(), 
                                 url:"{url_to('employers_messager_ajax',['action'=>'ListPartialMessage'])}?page="+$(this).attr('data-page'),                                
                                 target: "#actions"
                });
        });
        ...
        $(".Delete").click( function () { 
                ...
                return $.ajax2({ data :{ EmployerMessage: $(this).attr('id') },
                                 url :"{url_to('employers_messager_ajax',['action'=>'DeleteMessage'])}",                               
        ...
        $(".Reply").click( function () {                 
                return $.ajax2({  data :{ EmployerMessage: $(this).attr('id') },
                                  url :"{url_to('employers_messager_ajax',['action'=>'ReplyMessage'])}",                               
         ...
         $(".View").click( function () {                 
                return $.ajax2({ data :{ EmployerMessage: $(this).attr('id') },
                                 url :"{url_to('employers_messager_ajax',['action'=>'ViewMessage'])}",                               
                                 target :"#Modal-Message-ctn",
                                 success: function () { $("#Modal-Message").show(); },
                     });                                        
            });  
