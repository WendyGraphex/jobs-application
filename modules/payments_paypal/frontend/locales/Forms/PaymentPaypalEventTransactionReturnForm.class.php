...
class PaymentPaypalEventTransactionReturnForm extends mfForm {
    ...
    function checkData($validator,$values)
    {
     ...
     //   $settings=new PaypalSettings();                    
      /*  if ($settings->getPayerID()!=$values['PayerID'])
        ...
        $this->payment_event_user=new PaymentDebitEventUser(array('reference'=>$values['token']),$this->getEventUser());         
        if ($this->payment_event__user->isNotLoaded())
