...
require_once __DIR__."/../locales/Forms/AutomaticResponseReturnForm.class.php";
...
class payments_paypal_AutomaticResponseAction extends mfAction {
    ...
    function execute(mfWebRequest $request)
    {
       ...
       // $key = "EC-3D2202698D783170X"; // PAYID-L6WXKCI9MR376230G3360315
        ...
        $form = new AutomaticResponseReturnForm();
      ...
      //  $form->bind(array('data'=>$data,'key'=>$key));
         if ($form->isValid())
         {
            // echo "<pre>"; var_dump($form->getValues(),$form->getTransactionFee(),$form->getTransactionFeeCurrency());
             // echo "OK";
             $form->getPaymentEmployerUser()->setFee($form->getTransactionFee(),$form->getTransactionFeeCurrency());
             $form->getPaymentEmployerUser()->accepted();           
             $form->getPaymentEmployerUser()->getOrder()->paid();       
             $this->getEventDispather()->notify(new mfEvent($form->getPaymentEmployerUser(),'payment.accepted'));
         }    
         else
         {                 
            $settings=new PaypalSettings();                
            $item=new PaymentEmployerUser(array('reference'=>$request->getGetParameter('key'),'payer'=>$settings->getPayerID()));     
