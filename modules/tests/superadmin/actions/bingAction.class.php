...
class tests_bingAction extends mfAction {
   ...
   //     {    
     ...
     //   curl_setopt($ch, CURLOPT_USERAGENT, self::user_agent);
        curl_setopt($ch, CURLOPT_URL, 'http://www.bing.fr/search?q='.$query.'&first='.$loop."&cc=fr");
...
//curl_setopt($ch, CURLOPT_COOKIEFILE, '/cookies.txt'); 
      ...
      //  echo $content;
     //    file_put_contents(__DIR__."/bing_".time().".html", $content);
     //   $content=file_get_contents(__DIR__."/../data/bing/bing.html");
     ...
     //    file_put_contents(__DIR__."/google4.html", $html);
      ...
      //  var_dump($dom->getElementById("ires")->tagName);
        ...
        $nodes=$finder->query('//li[@class="b_algo"]/h2/a'); // title
        $index=0;
         echo "<ul>";
        foreach ($nodes as $node)
        {
            echo "<li>---->"; echo $index++; echo $node->nodeValue; "</li>";
            //die(__METHOD__);
        }    
        echo "</ul>";
        $nodes=$finder->query('//li[@class="b_algo"]/h2/a/@href'); // lien
        foreach ($nodes as $node)
        {
        ...
        $nodes=$finder->query('//li[@class="b_algo"]/div/p'); // descriptif
     //   var_dump($nodes); 
    ...
    /*   echo "<ul>";
        $nodes=$finder->query('//div[@class="b_attribution"]/cite'); //links                        
        foreach ($nodes as $node)
        {           
            echo "<li>---->"; echo $node->nodeValue; 
