...
class CustomerAcademyStudentUserGuardValidator extends mfValidatorBase
{
  ...
  {      
    if (isset($values[$this->getOption('username_field')]) && isset($values[$this->getOption('password_field')]))
    {
      ...
      if ($user=CustomerAcademyStudentUserUtils::findByEmailOrUsernameAndPassword($username, $password)) 
      {                
