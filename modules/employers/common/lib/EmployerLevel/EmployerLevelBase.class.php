...
class EmployerLevelBase extends mfOrderedObject3 {
     
    protected static $fields=array('name','icon','color','position','created_at','updated_at');
    const table="t_employer_level";
    ...
    function hasI18n()
    {
     ...
     {       
         if ($this->i18n===null)
         {
             ...
             $this->i18n=new EmployerLevelI18n(array('lang'=>$lang,"level_id"=>$this->get('id')));
         }   
         return $this->i18n;
     } 
     ...
     static function getAllI18nForSelect()
    {
        ...
        $db=mfSiteDatabase::getInstance()
                ...
                ->setQuery("SELECT ".EmployerLevelI18n::getFieldsAndKeyWithTable()." FROM ".EmployerLevelI18n::getTable().
                           " WHERE lang='{lang}' ORDER BY title ASC;")               
                ->makeSqlQuery(); 
        if (!$db->getNumRows())
            return $values;
        while ($item=$db->fetchObject('EmployerLevelI18n'))
        { 
    ...
    static function getAllForSelect()
    {
        ...
        $db=mfSiteDatabase::getInstance()
                ->setParameters(array("lang"=>mfcontext::getInstance()->getUser()->getLanguage()))
                ->setObjects(array('EmployerLevelI18n','EmployerLevel'))
                ->setQuery("SELECT {fields} FROM ".EmployerLevel::getTable().
                           " INNER JOIN ".EmployerLevelI18n::getInnerForJoin('level_id').
                           " WHERE lang='{lang}' ORDER BY title ASC;")               
                ->makeSqlQuery(); 
        if (!$db->getNumRows())
            return $values;
        ...
        {             
            $values[$items->getEmployerLevel()->get('id')]=$items->getEmployerLevel()->setI18n($items->getEmployerLevelI18n());
        }              
        return $values;
    }
    ...
    static function getContractLevelsForSelect()
    {
        ...
        $db=mfSiteDatabase::getInstance()
                ->setParameters(array("lang"=>mfcontext::getInstance()->getUser()->getLanguage()))               
                ->setQuery("SELECT * FROM ".EmployerLevelI18n::getTable().                           
                           " WHERE lang='{lang}' ORDER BY title ASC;")               
                ->makeSqlQuery(); 
        if (!$db->getNumRows())
            return $values;
    ...
    {          
        ...
        $db=mfSiteDatabase::getInstance()               
                ...
                ->setObjects(array('EmployerLevel','EmployerLevelI18n'))
                ->setQuery("SELECT {fields} FROM ".self::getTable(). 
                           " INNER JOIN ".EmployerLevelI18n::getInnerForJoin('level_id')." AND lang='{lang}'".
       ...
       // var_dump($db->getQuery());
        ...
        {                  
           ...
           $item->setI18n($items->getEmployerLevelI18n());
   ...
   {
       return $this->formatter=$this->formatter===null?new EmployerLevelFormatter($this):$this->formatter;
   }
}
