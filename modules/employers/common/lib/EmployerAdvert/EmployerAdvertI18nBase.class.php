...
class EmployerAdvertI18nBase extends mfObjectI18n {
     
    protected static $fields=array('url','reference','is_validated','title','short_description','content','advert_id','revival_iteration','revival_at','lang','number_of_views','number_of_quotations','refused_at','published_at','state','status','created_at','updated_at');
    const table="t_employer_advert_i18n"; 
    protected static $foreignKeys=array('advert_id'=>'EmployerAdvert'); // By default
    protected static $fieldsNull=array('updated_at','refused_at','state','revival_at','published_at'); 
   ...
   {
       return $this->formatter=$this->formatter===null?new EmployerAdvertI18nFormatter($this):$this->formatter;
   }
   ...
   {       
    ...
    function publish()
    {
        $this->add(array('published_at'=>date("Y-m-d H:i:s"),
                         'refused_at'=>null))->save();
        return $this;
    }
   ...
   /* function getEmployeeUser()
    {
        if ($this->employee_user===null)
        {    
          $db=mfSiteDatabase::getInstance()
                ->setParameters(array('advert_id'=>$this->get('advert_id'),'lang'=>$this->get('lang')))
                ->setQuery("SELECT ".Employee::getFieldsAndKeyWithTable()." FROM ".SaleEmployeeQuotation::getTable().
                           ...
                           " WHERE employer_advert_id='{advert_id}' AND employer_advert_lang='{lang}'".
