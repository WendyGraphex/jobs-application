...
class EmployerBudgetUnitI18nBase extends mfObjectI18n {
     
    protected static $fields=array('title','unit_id','lang','created_at','updated_at');
    const table="t_recruiter_budget_unit_i18n"; 
    protected static $foreignKeys=array('unit_id'=>'EmployerBudgetUnit'); // By default
    ...
    function __construct($parameters=null) {
      ...
      {
          if (isset($parameters['lang']) && isset($parameters['unit_id']))
              return $this->loadByLangAndUnitId((string)$parameters['lang'],(string)$parameters['unit_id']); 
     ...
     protected function loadByLangAndUnitId($lang,$unit_id)
    {
       $this->set('unit_id',$unit_id);
       $this->set('lang',$lang);
       $db=mfSiteDatabase::getInstance()           
            ->setParameters(array('unit_id'=>$unit_id,"lang"=>$lang))              
            ->setQuery("SELECT * FROM ".self::getTable().                      
                       " WHERE lang='{lang}' AND unit_id={unit_id};")
            ->makeSqlQuery();  
    ...
    {     
       $db=mfSiteDatabase::getInstance()           
            ...
            ->setQuery("SELECT ".self::getFieldsAndKeyWithTable()." FROM ".self::getTable().  
                       " LEFT JOIN ".self::getOuterForJoin('unit_id').                      
                       " WHERE name='{name}';")
            ->makeSqlQuery();  
        return $this->rowtoObject($db);
    }
    ...
    {
        $db=mfSiteDatabase::getInstance()           
            ->setParameters(array("unit_id"=>$this->get('unit_id')))              
      ...
      function delete()
    {
     ...
     function getUnit()
    {
       return $this->_unit_id=$this->_unit_id===null?new EmployerBudgetUnit($this->get('unit_id')):$this->_unit_id;                 
    }    
