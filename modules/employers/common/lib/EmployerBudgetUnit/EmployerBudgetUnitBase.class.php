...
class EmployerBudgetUnitBase extends mfOrderedObject3 {
     
    protected static $fields=array('name','icon','color','position','created_at','updated_at');
    const table="t_recruiter_budget_unit";
    ...
    function hasI18n()
    {
     ...
     {       
         if ($this->i18n===null)
         {
             ...
             $this->i18n=new EmployerBudgetUnitI18n(array('lang'=>$lang,"unit_id"=>$this->get('id')));
         }   
         return $this->i18n;
     } 
     ...
     static function getAllI18nForSelect()
    {
        ...
        $db=mfSiteDatabase::getInstance()
                ...
                ->setQuery("SELECT ".EmployerBudgetUnitI18n::getFieldsAndKeyWithTable()." FROM ".EmployerBudgetUnitI18n::getTable().
                           " WHERE lang='{lang}' ORDER BY title ASC;")               
                ->makeSqlQuery(); 
        if (!$db->getNumRows())
            return $values;
        while ($item=$db->fetchObject('EmployerBudgetUnitI18n'))
        { 
            $values[$item->get('unit_id')]=new mfString($item->get('title'));
        }              
        return $values;
    }
    ...
    static function getAllForSelect()
    {
        ...
        $db=mfSiteDatabase::getInstance()
                ->setParameters(array("lang"=>mfcontext::getInstance()->getUser()->getLanguage()))
                ->setObjects(array('EmployerBudgetUnitI18n','EmployerBudgetUnit'))
                ->setQuery("SELECT {fields} FROM ".EmployerBudgetUnit::getTable().
                           " INNER JOIN ".EmployerBudgetUnitI18n::getInnerForJoin('unit_id').
                           " WHERE lang='{lang}' ORDER BY title ASC;")               
                ->makeSqlQuery(); 
        if (!$db->getNumRows())
            return $values;
        ...
        {             
            $values[$items->getEmployerBudgetUnit()->get('id')]=$items->getEmployerBudgetUnit()->setI18n($items->getEmployerBudgetUnitI18n());
        }              
        return $values;
    }
    ...
    static function getUnitsForSelect()
    {
        ...
        $db=mfSiteDatabase::getInstance()
                ->setParameters(array("lang"=>mfcontext::getInstance()->getUser()->getLanguage()))               
                ->setQuery("SELECT * FROM ".EmployerBudgetUnitI18n::getTable().                           
                           " WHERE lang='{lang}' ORDER BY title ASC;")               
                ->makeSqlQuery(); 
        if (!$db->getNumRows())
            return $values;
        while ($item=$db->fetchObject('EmployerBudgetUnitI18n'))
        {             
            $values[$item->get('unit_id')]=ucfirst($item->get('title'));
        }              
        return $values;
    } 
    ...
    {          
        ...
        $db=mfSiteDatabase::getInstance()               
                ...
                ->setObjects(array('EmployerBudgetUnit','EmployerBudgetUnitI18n'))
                ->setQuery("SELECT {fields} FROM ".self::getTable(). 
                           " INNER JOIN ".EmployerBudgetUnitI18n::getInnerForJoin('unit_id')." AND lang='{lang}'".
       ...
       // var_dump($db->getQuery());
        ...
        {                  
           ...
           $item->setI18n($items->getEmployerBudgetUnitI18n());
   ...
   {
       return $this->formatter=$this->formatter===null?new EmployerBudgetUnitFormatter($this):$this->formatter;
   }
}
