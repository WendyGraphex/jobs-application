          ...
          <div class="row admin-header-container">
            <div class="col-sm">
              ...
              <h3 class="admin-header-small-text">{__('Employer populars management')}</h3>
            </div>
          ...
          <div class="breadcrumb-title">
            <p>
                ...
                <i class="fa fa-angle-right" style="margin: 0 5px 0 5px; font-size: 1em;"></i>{__('Populars')}
            </p>
          </div>
          <!-- End breadcrumb -->
{alerts}
 <div class=" bg-white px-2 py-2">
      ...
      <tr class="list" id="{$item->get('id')}" role="row">
          ...
          <td>
              <div>{$item->getAdvert()->getI18n()->getFormatter()->getTitle()->ucfirst()}
              </div>
               <div>{__('Ref.')}:  <a href="{$item->getAdvert()->getI18n()->getUrl()}">{$item->getAdvert()->getI18n()->get('reference')} </a>
              </div>             
          </td>  
          <td>
              {if $item->hasEmployee()}{$item->getEmployee()->getFormatter()->getName()}{else}{__('---')}{/if}    
          </td>
          <td>
              {if $item->hasEmployer()}{$item->getEmployer()->getFormatter()->getName()}{else}{__('---')}{/if}    
          </td>
          <td>
              {$item->get('ip')}
          </td>
    ...
    var updated_dates = $( " .Range.UpdatedAtDate#updated_at_from, .Range.UpdatedAtDate#updated_at_to" ).datepicker({
     ...
     function getFilterParameters()
        ...
        $(".pager").click(function () {                     
                return $.ajax2({ data: getFilterParameters(), 
                                 url:"{url_to('employers_ajax',['action'=>'ListPartialPopular'])}?page="+$(this).attr('data-page'),                                
                                 target: "#actions"
                });
        });
        
         $("#init").click(function () {                     
                return $.ajax2({   url:"{url_to('employers_ajax',['action'=>'ListPartialPopular'])}",
                                 target: "#actions"
                });
        });
