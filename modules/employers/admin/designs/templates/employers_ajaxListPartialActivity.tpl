...
<div class="row admin-header-container">
    <div class="col-sm">
           <h3 class="admin-header-big-text">{__('Configurations|Activities')}</h3>
           <h3 class="admin-header-small-text">{__('Manage Activities of employers')}</h3>
    </div>
      ...
      <div class="col-sm-12 padd-col"> *}   
        <table class="table table-bordered table-hover table-striped" cellpadding="0" cellspacing="0"> 
         <thead class="bgSpec">
          ...
          <th><span>{__('Icon')}</span></th>
          <th><span>{__('Color')}</span></th>
         ...
         {foreach $pager as $item}
          <tr class="list" id="{$item->get('id')}"> 
              ...
              <td class="ModeDisplay">
                    {if $item->hasIcon()} 
                        <img class="classIcon" src="{$item->getIcon()->getURL()}" alt="{__('Icon')}"/>
                    {/if}
              </td>
              <td>
                  {if $item->hasColor()}
    ...
    var updated_dates = $( ".Range.UpdatedAtDate#updated_at_from, .Range.UpdatedAtDate#updated_at_to" ).datepicker({
        ...
        function updatePager(n)
        {
          ...
          {* =====================  P A G E R  A C T I O N S =============================== *}  
           ...
           $("#init").click(function() {                   
               return $.ajax2({ url:"{url_to('employers_ajax',['action'=>'ListPartialActivity'])}",                                    
                         target: "#actions"}); 
           }); 
          ...
          $("[name=nbitemsbypage]").change(function() { return updateFilter(); }); 
           ...
           $(".pager").click(function () {                      
                return $.ajax2({ data: getFilterParameters(), 
                                 url:"{url_to('employers_ajax',['action'=>'ListPartialActivity'])}?"+this.href.substring(this.href.indexOf("?")+1, this.href.length),
                                target: "#actions"
                });
        });
          {* =====================  A C T I O N S =============================== *}  
          ...
          $("#New").click( function () {             
            return $.ajax2({
                ...
                url: "{url_to('employers_ajax',['action'=>'NewActivityI18n'])}",              
                target: "#actions"
           });
         });
         
         $(".View").click( function () {                
                return $.ajax2({ data : { EmployerActivityI18n : { 
                                                activity_id: $(this).attr('id'),
                                                lang: $("#SelectListLanguages option:selected").attr('id')                                            
                                    } },                                
                                url:"{url_to('employers_ajax',['action'=>'ViewActivityI18n'])}",                              
                                target: "#actions"});
         });
       $(".list").dblclick( function () {                    
                  return $.ajax2({ data : { EmployerActivityI18n : { activity_id: $(this).attr('id'),lang: $("#SelectListLanguages option:selected").attr('id') } },                               
                                  url:"{url_to('employers_ajax',['action'=>'ViewActivityI18n'])}", 
                                  target: "#actions"});
          });
        ...
        $(".Delete").click( function () { 
                ...
                return $.ajax2({ data :{ EmployerActivity: $(this).attr('id') },
                                 url :"{url_to('employers_ajax',['action'=>'DeleteActivity'])}",                                
        ...
        $("#Positions").click( function () {                
                return $.ajax2({ url:"{url_to('employers_ajax',['action'=>'PositionsActivity'])}",
                                target: "#actions"});
         });   
