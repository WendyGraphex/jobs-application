...
<div class="breadcrumb-title">
    <p>
        ...
        <i class="fa fa-angle-right" style="margin: 0 5px 0 5px; font-size: 1em;"></i>{__('Employe Galleries')}
        ...
        <div class="card">
            <div class="card-content collapse show">
                <div class="card-body">
                        ...
                        <table id="EmployerGalleryEmployer" cellpadding="0" cellspacing="0" class="table  table-striped">
                            <thead>
                                <tr>
                                    <th width="10px"></th>
                                    <th>{__('Position')}</th>                                   
                            ...
                            <tbody>
                            {foreach $item_i18n->getGallery()->getGalleryEmployers() as $item}
                                <tr id="{$item->get('id')}" class="ui-state-default Employers">
                                    <td><i class="fa fa-arrows-alt"></i></td>
                                    <td class="position">{$item->get('position')}</td>                                   
                                    <td>
                                        <div class="pull-left">{$item->getEmployerUser()}</div>
                                    </td>
                                </tr>
                            {/foreach}
                            </tbody>
...
<script type="text/javascript">
    ...
    $("#EmployerGalleryEmployer tbody").sortable({
        cursor: 'move',
    ...
    $('#Save').click(function () {
        var params = {
            ...
            EmployerGalleryEmployerPositions: {
                positions: [],
                token: '{$form->getCSRFToken()}'
            }
        };
        $(".Employers").each(function () { params.EmployerGalleryEmployerPositions.positions.push($(this).attr('id')); });
    ...
    $("#Cancel").click(function () {
        return $.ajax2({
            ...
            url: "{url_to('employers_ajax',['action'=>'ListPartialEmployerForGalleryI18n'])}",
            target: "#actions"
        });
    });
