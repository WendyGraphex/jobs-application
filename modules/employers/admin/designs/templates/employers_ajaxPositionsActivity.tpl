...
<div class="row admin-header-container">
    <div class="col-sm">
        <h3 class="admin-header-big-text">{__('Activities')}</h3> 
        <h3 class="admin-header-small-text">{__("Activity management: Positions")}</h3> 
    </div>
                ...
                <div class="card-body">
                        ...
                        <table id="Activities" cellpadding="0" cellspacing="0" class="table  table-striped">
                            ...
                            {foreach $form->getOrdered() as $function}
                                <tr id="{$function->get('id')}" class="ui-function-default Activities">
...
<script activity="text/javascript">
    ...
    $('#Save').click(function () {
        var params = {
            EmployerActivityPositions: {
                positions: [],
                token: '{$form->getCSRFToken()}'
            }
        };
        $(".Activities").each(function () {
            params.EmployerActivityPositions.positions.push($(this).attr('id'));
        });
        // alert("Params="+params.toSource());   return ;
        return $.ajax2({
            data: params,
            url: "{url_to('employers_ajax',['action'=>'PositionsActivity'])}",
            target: "#actions"
        });
    });
