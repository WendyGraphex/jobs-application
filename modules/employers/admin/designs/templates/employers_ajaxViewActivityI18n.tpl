                    ...
                    <div class="form-group">
                        <label>{__('Icon')}</label>                  
                            <div id="pictureForm">                            
                                ...
                                <input id="item_file" type='file' hidden="" activity="file" class="form-control Files Fields" name="EmployerActivityI18n[activity][icon]" />
                            </div>
                    </div>
                </div> 
...
<script activity="text/javascript"> 
   ...
   $(".Color").minicolors({
      // position: relative,
       ...
       $("#EmployerActivity-ChangeIcon").click(function() {
           $("#EmployerActivity-icon").show();
           $(this).hide();
           $("#Save").show();  
        });           
      
       $('#EmployerActivity-DeleteIcon').click(function(){ 
            if (!confirm("{__("Icon will be deleted. Confirm ?")}")) return false; 
            return $.ajax2({ 
                              data : { EmployerActivity: "{$item_i18n->getActivity()->get('id')}" }, 
                              url: "{url_to('employers_ajax',['action'=>'DeleteIconActivity'])}",                              
                              success :function(response) {
                                          if (response.action=='DeleteIconActivity' && response.id=="{$item_i18n->getActivity()->get('id')}")
                                          {                                                     
                                                $("#EmployerActivity-icon-container").hide(); 
                                                $("span#EmployerActivity-text-icon").html("{__('Add icon')}");
                                          }
                              }
            });  
      }); 
      
    $(".EmployerActivity,.EmployerActivityI18n").click(function() {  $('#Save').show(); });  
         
     {* =================== L A N G U A G E ================================ *}
        
     {* =================== A C T I O N S ================================ *}
     $('#Cancel').click(function(){                
             return $.ajax2({ data: { filter: { lang:  '{$item_i18n->get('lang')}', token: "{mfForm::getToken('EmployerActivityFormFilter')}" } },                              
      ...
      $('#Save').click(function(){                             
        ...
        //      alert("Params="+params.toSource());   return ;      
          return $.ajax2({ data : params,  
                           files :'.Files',
                           url: "{url_to('employers_ajax',['action'=>'SaveActivityI18n'])}",
                           target: "#actions"}); 
        });                          
</script>
