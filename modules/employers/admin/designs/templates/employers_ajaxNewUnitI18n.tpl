...
<div class="row admin-header-container">
    <div class="col-sm">
        <h3 class="admin-header-big-text">{__("New Unit")}</h3>
...
<div class="breadcrumb-title">
    <p>
        ...
        <i class="fa fa-angle-right" style="margin: 0 5px 0 5px; font-size: 1em;"></i>{__('New Unit')}
    </p>
</div>
{alerts}
     ...
     <div class="the-box bg-white p-3">    
         <div class="row">
          <div class="form-group col-md-12">          
            <div>{$form.unit_i18n.lang->getError()}</div> 
            {component name="/site_languages/selectListLanguagesFrontend" selected=$form.unit_i18n.lang}
          </div>          
        </div>
        ...
        {*<div class="row">          
           ...
           <div class="form-group col-md-6">             
              {if $form->hasErrors()}<div class="alert alert-{if $form.unit.icon->hasError()}danger{else}light{/if}" role="alert">&nbsp;{$form.unit.icon->getError()}</div>{/if}
            <label for="icon">{__('Icon')}{if $form->unit.icon->getOption('required')}*{/if}</label> 
...
<script unit="text/javascript">  
   ...
   $(".Color").minicolors({
     ...
     $('#Cancel').click(function(){                
             return $.ajax2({ data: { filter: { lang:$("#SelectListLanguages option:selected").attr('id'), token: "{mfForm::getToken('EmployerBudgetUnitFormFilter')}" } },                              
      ...
      $('#Save').click(function(){                             
            var  params= {      iFrame:true,             
                                EmployerBudgetUnitI18n: { 
                                   unit_i18n : { lang: $("#SelectListLanguages option:selected").attr('id')  },
                                   unit : { },
        ...
        //      alert("Params="+params.toSource());   return ;      
          return $.ajax2({ data : params,  
                           files :'.Files',
                           url: "{url_to('employers_ajax',['action'=>'SaveNewUnitI18n'])}",
                           target: "#actions"}); 
        });                          
