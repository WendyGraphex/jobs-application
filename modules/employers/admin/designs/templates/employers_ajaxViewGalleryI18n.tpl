...
<script type="text/javascript">              
     ...
     $('#Cancel').click(function(){                
       ...
       $('#Save').click(function(){                             
            var  params= {  EmployerGalleryI18n: { 
                                   gallery_i18n : { lang: "{$item_i18n->get('lang')}",gallery_id: "{$item_i18n->get('gallery_id')}"    },
                                   gallery : { },
                                   token :'{$form->getCSRFToken()}'
                                } };
          $(".EmployerGalleryI18n.Input").each(function() { params.EmployerGalleryI18n.gallery_i18n[this.name]=$(this).val(); });
          $(".EmployerGallery.Input").each(function() {  params.EmployerGalleryI18n.gallery[this.name]=$(this).val();  });  // Get foreign key           
          //    alert("Params="+params.toSource());   return ;       
          return $.ajax2({ data : params,                           
                           url: "{url_to('employers_ajax',['action'=>'SaveGalleryI18n'])}",
                           target: "#actions" }); 
        });  
