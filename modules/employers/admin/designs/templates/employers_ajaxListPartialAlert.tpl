          ...
          <div class="row admin-header-container">
            <div class="col-sm">
              <h3 class="admin-header-big-text">{__('Alerts')}</h3>
              <h3 class="admin-header-small-text">{__('Employer alerts management')}</h3>
            </div>
            <div class="col-sm" style="padding: 0;">
                <div style="float: right;">
                    <a class="btn btn-success admin-header-button"  href="#" id="New"><i class="fa fa-plus"></i>  {__('New Alert')}</a>
...
<div class=" bg-white px-2 py-2">
      ...
      <thead class="bgGlobal">
         <tr role="row">
            ...
            <th class="sorting">{__('commercial')}</span>  
                <div style="float: right;">
      ...
      <tr class="list" id="{$item->get('id')}" role="row">
        ...
        <td>{$item->get('id')}</td>  
        <td>{if $item->hasI18n()}{$item->getI18n()->get('name')}{else}{__('---')}{/if}</td>
        <td>{if $item->hasI18n()}{$item->getI18n()->get('commercial')}{else}{__('---')}{/if}</td>
        <td>{if $item->hasI18n()}{$item->getI18n()->get('email')}{else}{__('---')}{/if}</td>
         <td>       
     ...
     function getFilterParameters()
        ...
        {          
           return $.ajax2({ data: getFilterParameters(), 
                            url:"{url_to('employers_ajax',['action'=>'ListPartialAlert'])}" ,                          
                            target: "#actions"
                             });
        }
...
{* =================================== PAGER/FILTER ======================================================== *}
        ...
        $(".pager").click(function () {                     
                return $.ajax2({ data: getFilterParameters(), 
                                 url:"{url_to('employers_ajax',['action'=>'ListPartialAlert'])}?page="+$(this).attr('data-page'),                                
                                 target: "#actions"
                });
        });
        
         $("#init").click(function () {                     
                return $.ajax2({   url:"{url_to('employers_ajax',['action'=>'ListPartialAlert'])}",
                                 target: "#actions"
                });
        });
...
{* =================================== ACTIONS ======================================================== *}
    ...
    $("#New").click( function () {             
            return $.ajax2({      
                ...
                url: "{url_to('employers_ajax',['action'=>'NewAlert'])}",               
                target: "#actions"
          });
     });
      ...
      $(".View").click( function () {                    
                  return $.ajax2({ data : { Employer : $(this).attr('id') },                               
                                  url:"{url_to('employers_ajax',['action'=>'ViewAlert'])}", 
     ...
     $(".Delete").click( function () { 
                ...
                return $.ajax2({ data :{ EmployerAlert: $(this).attr('id') },
                                 url :"{url_to('employers_ajax',['action'=>'DeleteAlert'])}",                               
