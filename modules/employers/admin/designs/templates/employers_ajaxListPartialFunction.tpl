...
<div class="row admin-header-container">
    <div class="col-sm">
           ...
           <h3 class="admin-header-small-text">{__('Manage Functions of employers')}</h3>
    </div>
  ...
  <a href="#" class="btn btn-success admin-header-button btn-color" id="Multiple" title="{__('New Multiple Functions')}">
                <i class="fa fa-list" style="margin-right: 5px;"></i>
                {__('New Multiple Functions')}
            </a>
    ...
    var updated_dates = $( ".Range.UpdatedAtDate#updated_at_from, .Range.UpdatedAtDate#updated_at_to" ).datepicker({
        ...
        function updatePager(n)
        {
          ...
          {* =====================  P A G E R  A C T I O N S =============================== *}  
           ...
           $("#init").click(function() {                   
               return $.ajax2({ url:"{url_to('employers_ajax',['action'=>'ListPartialFunction'])}",                                    
                         target: "#actions"}); 
           }); 
          ...
          $("[name=nbitemsbypage]").change(function() { return updateFilter(); }); 
           ...
           $(".pager").click(function () {                      
                return $.ajax2({ data: getFilterParameters(), 
                                 url:"{url_to('employers_ajax',['action'=>'ListPartialFunction'])}?"+this.href.substring(this.href.indexOf("?")+1, this.href.length),
                                target: "#actions"
                });
        });
          {* =====================  A C T I O N S =============================== *}  
          ...
          $("#New").click( function () {             
            return $.ajax2({
                ...
                url: "{url_to('employers_ajax',['action'=>'NewFunctionI18n'])}",              
                target: "#actions"
           });
         });
         
         $(".View").click( function () {                
                return $.ajax2({ data : { EmployerFunctionI18n : { 
                                                function_id: $(this).attr('id'),
                                                lang: $("#SelectListLanguages option:selected").attr('id')                                            
                                    } },                                
                                url:"{url_to('employers_ajax',['action'=>'ViewFunctionI18n'])}",                              
                                target: "#actions"});
         });
       $(".list").dblclick( function () {                    
                  return $.ajax2({ data : { EmployerFunctionI18n : { function_id: $(this).attr('id'),lang: $("#SelectListLanguages option:selected").attr('id') } },                               
                                  url:"{url_to('employers_ajax',['action'=>'ViewFunctionI18n'])}", 
                                  target: "#actions"});
          });
        ...
        $(".Delete").click( function () { 
                ...
                return $.ajax2({ data :{ EmployerFunction: $(this).attr('id') },
                                 url :"{url_to('employers_ajax',['action'=>'DeleteFunction'])}",                                
        ...
        $("#Positions").click( function () {                
                return $.ajax2({ url:"{url_to('employers_ajax',['action'=>'PositionsFunction'])}",
                                target: "#actions"});
         });   
     
 $("#Multiple").click( function () {                
                return $.ajax2({  data : { lang : { lang: $("#SelectListLanguages option:selected").attr('id'),token: "{mfForm::getToken('LanguageFrontendForm')}" } },     
                                url:"{url_to('employers_ajax',['action'=>'NewMultipleFunctionI18n'])}",
                                target: "#actions"});
         });
</script>    
