    ...
    <fieldset>
            <div class="row" style="margin: 0;padding-bottom: 10px;">
                <div class="col-sm-6 site-applications-header" style="padding: 0;">
                    <h3 class="admin-header-big-text" style="padding-top: 12px;">{__("View Employer")}</h3>
                </div>
                            ...
                            <div class="col-sm-2">
                                <div class="form-group">
                                    ...
                                    {foreach $form->getValidator('gender')->getOption('choices') as $name=>$gender}                                     
                                       <input type="radio"  class="Employer Input" name="gender" value="{$name}" {if $item->get('gender')==$name}checked="checked"{/if}> {$gender}
                                   {/foreach}
                                </div>
                            </div>
...
{*else}
    <span>{__("This candidate is invalid.")}</span> 
...
<script type="text/javascript">
    ...
    $(".Mode").click(function(){
        if ($(this).attr('name')=='NORMAL'){
         ...
         $(".Employer").click(function() { $("#Save").show(); });
         ...
         $('#Save').click(function(){ 
          ...
          $("input.Employer[type=radio]:checked").each(function() { params.Employer[this.name]=$(this).val(); });
            return $.ajax2({ data : params, 
                            files :'.Files',
                           errorTarget: ".Employer-errors",
                           url: "{url_to('employers_ajax',['action'=>'SaveEmployer'])}",
                           target: "#page-wrapper" }); 
