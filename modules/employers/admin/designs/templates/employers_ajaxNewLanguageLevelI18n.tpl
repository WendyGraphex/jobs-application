...
<div class="row admin-header-container">
    <div class="col-sm">
        <h3 class="admin-header-big-text">{__("New Employer Language Level")}</h3>
...
<div class="breadcrumb-title">
    <p>
        ...
        <i class="fa fa-angle-right" style="margin: 0 5px 0 5px; font-size: 1em;"></i>{__('New Employer Language Level')}
    </p>
</div>
{alerts}
                    ...
                    <div class="form-group">
                        <label>{__('Icon')}</label>                  
                            <div id="pictureForm">                            
                                ...
                                <input id="item_file" type='file' hidden="" type="file" class="form-control Files Fields" name="EmployerLanguageLevelI18n[level][icon]" /> {*name="SiteCityI18n[function][icon]"*}
                            </div>
                    </div>
                </div> 
              </div>
...
<script level="text/javascript"> 
   ...
   $(".Color").minicolors({
     ...
     $('#Cancel').click(function(){                
             return $.ajax2({ data: { filter: { lang:$("#SelectListLanguages option:selected").attr('id'), token: "{mfForm::getToken('EmployerLanguageLevelFormFilter')}" } },                              
      ...
      $('#Save').click(function(){                             
        ...
        //      alert("Params="+params.toSource());   return ;      
          return $.ajax2({ data : params,  
                           files :'.Files',
                           url: "{url_to('employers_ajax',['action'=>'SaveNewLanguageLevelI18n'])}",
                           target: "#actions"}); 
        });                          
