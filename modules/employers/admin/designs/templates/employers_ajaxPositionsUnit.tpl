...
<div class="row admin-header-container">
    <div class="col-sm">
        <h3 class="admin-header-big-text">{__('Units')}</h3> 
        <h3 class="admin-header-small-text">{__("Unit management: Positions")}</h3> 
    </div>
...
<div class="breadcrumb-title">
    <p>
        ...
        <i class="fa fa-angle-right" style="margin: 0 5px 0 5px; font-size: 1em;"></i>{__('Units')}
    </p>
</div>
        ...
        <div class="card">
            <div class="card-content collapse show">
                <div class="card-body">
                        ...
                        <table id="Units" cellpadding="0" cellspacing="0" class="table  table-striped">
                            <thead>
                                <tr>
                                    <th width="10px"></th>
                                    <th>{__('Position')}</th>
                            ...
                            {foreach $form->getOrdered() as $function}
                                <tr id="{$function->get('id')}" class="ui-function-default Units">
...
<script unit="text/javascript">
    ...
    $('#Save').click(function () {
        var params = {
            EmployerBudgetUnitPositions: {
                positions: [],
                token: '{$form->getCSRFToken()}'
            }
        };
        $(".Units").each(function () {
            params.EmployerBudgetUnitPositions.positions.push($(this).attr('id'));
        });
        // alert("Params="+params.toSource());   return ;
        return $.ajax2({
            data: params,
            url: "{url_to('employers_ajax',['action'=>'PositionsUnit'])}",
            target: "#actions"
        });
    });
