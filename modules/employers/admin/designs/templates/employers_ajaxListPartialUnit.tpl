...
<div class="row admin-header-container">
    <div class="col-sm">
           <h3 class="admin-header-big-text">{__('Configurations|Budget Units')}</h3>
           <h3 class="admin-header-small-text">{__('Manage Budget Units of employers')}</h3>
    </div>
...
<div class="breadcrumb-title">
    <p>
       <i class="fa fa-home" style="color: #37bc9b;"></i>
        ...
        <i class="fa fa-angle-right" style="margin: 0 5px 0 5px; font-size: 1em;"></i>{__('Budget Units')}
    </p>
</div>
...
<div class=" bg-white px-2 py-2">
    ...
    <tbody>
       <tr>
        ...
        <td>
            <input class="search form-control form-control-sm ml-1" name="name" value="{$formFilter.search.position}"/>
        </td>
        <td>
    ...
    var updated_dates = $( " .Range.UpdatedAtDate#updated_at_from, .Range.UpdatedAtDate#updated_at_to" ).datepicker({
        ...
        function updatePager(n)
        {
          ...
          {* =====================  P A G E R  A C T I O N S =============================== *}  
           ...
           $("#init").click(function() {                   
               return $.ajax2({ url:"{url_to('employers_ajax',['action'=>'ListPartialUnit'])}",                                    
                         target: "#actions"}); 
           }); 
          ...
          $("[name=nbitemsbypage]").change(function() { return updateFilter(); }); 
           ...
           $(".pager").click(function () {                      
                return $.ajax2({ data: getFilterParameters(), 
                                 url:"{url_to('employers_ajax',['action'=>'ListPartialUnit'])}?"+this.href.substring(this.href.indexOf("?")+1, this.href.length),
                                target: "#actions"
                });
        });
          {* =====================  A C T I O N S =============================== *}  
          ...
          $("#New").click( function () {             
            return $.ajax2({
                ...
                url: "{url_to('employers_ajax',['action'=>'NewUnitI18n'])}",              
                target: "#actions"
           });
         });
         
         $(".View").click( function () {                
                return $.ajax2({ data : { EmployerBudgetUnitI18n : { 
                                                unit_id: $(this).attr('id'),
                                                lang: $("#SelectListLanguages option:selected").attr('id')                                            
                                    } },                                
                                url:"{url_to('employers_ajax',['action'=>'ViewUnitI18n'])}",                              
                                target: "#actions"});
         });
       $(".list").dblclick( function () {                    
                  return $.ajax2({ data : { EmployerBudgetUnitI18n : { unit_id: $(this).attr('id'),lang: $("#SelectListLanguages option:selected").attr('id') } },                               
                                  url:"{url_to('employers_ajax',['action'=>'ViewUnitI18n'])}", 
                                  target: "#actions"});
          });
        ...
        $(".Delete").click( function () { 
                if (!confirm('{__("Contract Unit \"#0#\" will be deleted. Confirm ?")}'.format($(this).attr('name')))) return false; 
                return $.ajax2({ data :{ EmployerBudgetUnit: $(this).attr('id') },
                                 url :"{url_to('employers_ajax',['action'=>'DeleteUnit'])}",                                
                                 success : function(resp) {
                                          if (resp.action != 'DeleteUnit')  return ;                                                                              
        ...
        $("#Positions").click( function () {                
                return $.ajax2({ url:"{url_to('employers_ajax',['action'=>'PositionsUnit'])}",
                                target: "#actions"});
         });   
