    ...
    <div class="col-md-12 grid-margin classConducteurTop">
      <div class="d-flex justify-content-between flex-wrap">
        <div class="d-flex align-items-end flex-wrap">
          ...
          {component name="/site_help/help" help="employer-list-advert"}
    </div>
</div>
{alerts}
<div class="row admin-header-container btn-r">    
                      ...
                      <tbody>
                         {foreach $pager as $item} 
                            <tr class="list" id="{$item->get('id')}" role="row">
                                ...
                                <td><span class="EmployerAdvert" name="number_of_languages" id="{$item->get('id')}">
                                ...
                                <td>{$item->getFormatter()->getCreatedAt()->getText()}</td>                                                           
                                  <td class="EmployerAdvert" name="committed" id="{$item->get('id')}">
                                ...
                                <td>
                                  ...
                                  <td class="EmployerAdvert" name="state" id="{$item->get('id')}">
                                      {$item->getStateI18n()}
                                </td>
                                  <td>                                         
                            ...
                            <tr class="list-i18n" id="{$i18n->get('id')}" data-advert="{$item->get('id')}" role="row">
                                ...
                                <td>
                                 ...
                                 <td class="EmployerAdvertI18n" name="state" id="{$item->get('id')}">
                                    {if $i18n->hasPublishedAt()}
                                        {__('Approved')}
 ...
 <script type="text/javascript">
      ...
      function updatePager(n)
        ...
        $(".Delete").click( function () { 
         ...
         $(".DeleteI18n").click( function () { 
                ...
                return $.ajax2({ data :{ EmployerAdvertI18n: $(this).attr('id') },
                                 url :"{url_to('employers_ajax',['action'=>'DeleteAdvertI18n'])}",                               
                                 success : function(resp) {
                                       if (resp.action!='DeleteAdvertI18n') return ;                                          
                                          $(".list-i18n[id="+resp.id+"]").remove();                                           
                                          $(".EmployerAdvert[name=number_of_languages][id="+resp.advert_id+"]").html(resp.number_of_languages);
                                 }
                     });                                        
            });  
        ...
        $("#New").click( function () {                 
                return $.ajax2({ url :"{url_to('employers_ajax',['action'=>'NewAdvert'])}",                               
                                 target :"#actions"
                     });                                        
            });  
         ...
         $(".View").click( function () {                 
                return $.ajax2({ data :{ EmployerAdvert: $(this).attr('id') },
                                 url :"{url_to('employers_ajax',['action'=>'ViewAdvert'])}",                               
                                 target :"#actions"
                     });                                        
            });  
        ...
        $('#Dashboard').click(function(){                                       
         ...
         $('.Commit').click(function(){                                       
          return $.ajax2({ data :{ EmployerAdvert: $(this).attr('id') },
                           url: "{url_to('employers_ajax',['action'=>'CommitAdvert'])}",                           
                           success : function (resp)
                                {
                                    if (resp.action != 'CommitAdvert') return ;
                                    $(".Commit[id="+resp.id+"]").remove();
                                    $(".EmployerAdvert[name=committed][id="+resp.id+"]").html(resp.committed_at);
                                    $(".EmployerAdvert[name=state][id="+resp.id+"]").html(resp.state);
                                    $(".EmployerAdvertI18n[name=state][id="+resp.id+"]").html(resp.state_i18n);
