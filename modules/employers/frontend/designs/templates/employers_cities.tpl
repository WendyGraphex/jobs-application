...
<html lang="{$_request->getLanguage()}">
    <head>
        ...
        <title>{component name="/site_company/Display" field='name' COMMENT=false} - {__('All employer cities')}</title> 
            ...
            <div id="page-ctn" class="container">      
                <div id="page-wrapper" class="allAdvetsInCities">  
                    ...
                    {foreach $pager as $item}
                        <h3>{$item->getCountry()->getText()} - {$item->getCity()->lower()->ucfirst()}</h3>              
                        ...
                        {component name="/employers/cityLink" city=$item->getCity() country=$item->getCountry()}            
                    {/foreach}
                </div> 
            </div>
        </div> 
