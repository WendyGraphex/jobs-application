<div class="ModalLogin modal-dialog" style="width: 50%;">
        ...
        <div class="modal-body">
            <div class="FormCard card-container">
                ...
                <div class="row" style="padding-top: 10px;padding-bottom: 10px;">
                    <div class="col-md-6">
                        {component name="/facebook_oauth/oauthBtnEmployer" target="_blank"}
                    </div>
                    <div class="col-md-6">
                        {component name="/google_oauth/oauthBtnEmployer" target="_blank"}
                    </div>
                </div>
                <div class="row">
                    {*<div class="col-md-6">
                        {component name="/linkedin_oauth/oauthBtnEmployer" target="_blank"}  
                    </div>*}
                    <div class="col-md-6">
                        {component name="/instagram_oauth/oauthBtnEmployer" target="_blank"}  
                    </div>
                </div>

                <div class="text-center mt-4 font-weight-light">
                    {__("Don't have an account?")} <a href="{link_i18n('employers_signin')}" target="_blank" class="text-primary">{__('Create')}</a>
                </div>
            </div>                     
        </div>
    </div>
 ...
 <script type="text/javascript">
     ...
     //$(".close").click(function(){  $("#DialogEmployeeMessage,#DialogEmployeeQuote").modal('hide');  });
            
       
    $("#Login").click(function () { 
        var params = {  EmployerLogin : {  token : '{mfForm::getToken('EmployerUserGuardForm')}' } };
        $(".Login.Input").each(function () { params.EmployerLogin[$(this).attr('name')]=$(this).val(); });
        return $.ajax2({ data : params,               
                        url :"{url_to('employers_ajax',['action'=>'Login'])}",   
                        success : function (resp) { 
                            ...
                            $("#DialogEmployeeMessage,#DialogEmployeeQuote").modal('hide');   
                            $("#Employer-login-link").css('display','block');
                        }                         
          }); 
     });
</script>        