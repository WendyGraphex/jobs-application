        ...
        <div class="card classMargin">
                            ...
                            <div class="row mt-3">                       
                                <div class="form col-md-12">                                   
                                    <div class="form-group">                          
                                        ...
                                        <textarea cols="40" rows="5" class="Editor EmployerAdvertI18n Input" data-lang="{$language->get('code')}" name="short_description">{if $form->hasErrors()}{$form.adverts[$index].short_description|escape}{else}{$form->getAdvert()->getAdvertsI18n()->getItemByIndex($index)->get('short_description')|escape}{/if}</textarea>
                                    </div>                    
                                </div>  
                            </div>
                            <div class="row mt-3">                       
                                <div class="form col-md-12">                                   
                                    <div class="form-group">                          
                                        ...
                                        <textarea cols="40" rows="10" class="Editor EmployerAdvertI18n Textarea Input" data-lang="{$language->get('code')}" name="content">{if $form->hasErrors()}{$form.adverts[$index].content|escape}{else}{$form->getAdvert()->getAdvertsI18n()->getItemByIndex($index)->get('content')|escape}{/if}</textarea>
                                    </div>                    
                                </div>  
                            </div>                  
                        </div> 
                        ...
                        <div id="categories-ctn">
                            <div id="EmployerAdvertCategories" style="padding-top: 15px;">                               
                                {foreach $form->getCategories() as $category} 
                                    ...
                                    <a href="javascript:void(0);" title="{__('Delete')}" class="EmployerAdvertCategory-Delete" id="{$category->get('id')}"  name="{if $category->hasI18n()}{$category->getI18n()->get('title')}{else}{$category->get('name')}{/if}" style="color: #fff;">
                            ...
                            <div class="col-md-12" style="padding: 0;">
                                <div class="Addbtn px-2"> 
                                    <div style="width: 163px;margin: 0 auto;">
                                        <a href="#" id="AddCategories" class="btn btn-primary admin-header-button mb-2" style="line-height: 17px;padding: 14px;"><i class="fa fa-plus"></i><span style="vertical-align: text-top;margin-left: 10px;"> {__('Add categories')}</span></a>                
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <div class="tab-pane fade show" id="documents" role="tabpanel" aria-labelledby="documents-tab">                                                                                    
                <div class="row portfolio-grid">
                    <div class="EmployerDocuments-list col-lg-3 col-md-4">
...
<div style="margin-top: 20px;">                              
    {* <button id="EmployerSignin-Previous"  class="btn btn-success">{__('Previous')}</button> *}
    <button id="EmployerSignin-Send"  class="btn btn-primary">{__('Send')}</button>
</div> 
</div>
...
<script type="text/javascript">  
    ...
    $("#Modal-Categories").modal({
     ...
     $( '.Editor' ).ckeditor();
        ...
        $('.EmployerAdvertI18n').keyup(function () {   $(".EmployerAdvertI18n").removeClass('intro');  });
     
        $("#EmployerSignin-Send").click(function () { 
            $(".EmployerAdvertI18n").removeClass('intro');
           ...
           // console.log(params); return ;
            return $.ajax2({
                data : params,
                url: " {url_to('employers_ajax',['action'=>'SavePostAJobAdvertProfile'])}",                    
                success : function (resp)
                    {
                        if (resp.errors)
                        {
                            $.each(resp.errors,function (name,error) { 
                                alert(error);
                                $(".EmployerAdvertI18n[name="+name+"]").addClass('intro');
                            });
                            return ;
                        }
                        //$("#post-a-job-ctn").html(resp);
    ...
    $("#Modal-Categories-Close").click(function () { 
      ...
      $("#DialogCategories").click(function () { 
              return $.ajax2({      
                         ...
                         success : function () {
                            ...
                            /*$('#Modal-Categories').appendTo("body").modal({
                                backdrop: 'static',
                                keyboard: true, 
                                show: true
                            });  $('body').css('overflow', 'hidden');*/
        ...
        $("#AddCategories").click(function () { 
            ...
            return $.ajax2({
                ...
                success : function () {   $("#Modal-Categories").appendTo("body").modal('hide'); },
     ...
     $("#DropZone").dropzone({
            url: "{url_to("employers_ajax",['action'=>'UploadAdvertDocumentProfile'])}",
            clickable: false,
