        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div style="text-align: center;">
                    <h2 class="sharing-title">{__('Share %s',$employer_user->get('username'))}</h2>
                    <div class="sharing-description">
                        <p style="font-size: 18px;">{__('Spread the word about this employer on Graphex')}</p>
                    </div>
                    <ul class="shareTo-social-network">
                        {component name="/facebook_share/link" tpl='share' text=$text url=$employer_user->getUrl()}                      
                        {component name="/linkedin_share/link" tpl='share' text=$text url=$employer_user->getUrl()} 
                        {component name="/twitter_share/link" tpl='share' text=$text url=$employer_user->getUrl()} 
                        {component name="/whatsapp_share/link" tpl='share' text=$text url=$employer_user->getUrl()}
                        <li class="liLinkCopy" style="cursor: pointer;">
                            <p id="copyMe" style="display: none;">{$employer_user->getUrl()}</p>
                            ...
                            <label class="textcopy">Copy</label>
                    </ul>
                </div>
            </div>
        </div>
