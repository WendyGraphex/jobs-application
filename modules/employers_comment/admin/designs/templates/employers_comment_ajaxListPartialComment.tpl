<div class="row admin-header-container">
    <div class="col-sm">
        ...
        <h3 class="admin-header-small-text">{__('Employer comment management')}</h3>
    </div>
...
<div class=" bg-white px-2 py-2">
    ...
    <table id="Myliste" class="table table-striped table-bordered dt-responsive nowrap dataTable no-footer w-100 list-user"  role="grid" aria-describedby="Myliste_info">
        ...
        <tbody>
            <tr role="row">
                <td></td>
                <td><input class="form-control form-control-sm ml-1 search" placeholder="{__('Firstname, lastname, email, username')}" name="employer" value="{$formFilter.search.employer}"></td>
                <td><input class="form-control form-control-sm ml-1 search" placeholder="{__('Firstname, lastname, email, username')}" name="employee" value="{$formFilter.search.employee}"></td>
                <td></td>
                <td></td>
                <td></td>
                <td>
     ...
     function getFilterParameters()
        ...
        {          
           return $.ajax2({ data: getFilterParameters(), 
                            url:"{url_to('employers_comment_ajax',['action'=>'ListPartialComment'])}" ,                          
                            target: "#actions"
                             });
        }
        ...
        $(".pager").click(function () {                     
                return $.ajax2({ data: getFilterParameters(), 
                                 url:"{url_to('employers_comment_ajax',['action'=>'ListPartialComment'])}?page="+$(this).attr('data-page'),                                
                                 target: "#actions"
                });
        });
        
         $("#init").click(function () {                     
                return $.ajax2({   url:"{url_to('employers_comment_ajax',['action'=>'ListPartialComment'])}",
                                 target: "#actions"
                });
        });
      ...
      $(".View").click( function () {                    
                  return $.ajax2({ data : { Employer : $(this).attr('id') },                               
                                  url:"{url_to('employers_comment_ajax',['action'=>'ViewEmployer'])}", 
      ...
      $('.order').click(function() {
       ...
       $(".Comment").click( function () {                    
                  return $.ajax2({ data : { EmployerComment : $(this).attr('id') },                               
                                  url:"{url_to('employers_comment_ajax',['action'=>'DisplayComment'])}", 
