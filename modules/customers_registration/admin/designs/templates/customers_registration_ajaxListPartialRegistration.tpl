...
<div class="row admin-header-container">
    <div class="col-sm">
        <h3 class="admin-header-big-text">{__('Registrations')}</h3>
        <h3 class="admin-header-small-text">{__('Registration accounts management')}</h3>
    </div>
    <div class="col-sm" style="padding: 0;">
        <div style="float: right;"> 
            <a class="btn btn-warning admin-header-button" href="{url_to('customers_registration',['action'=>'ExportCsv'])}?{$formFilter->getParametersForUrl(['equal','in','begin','search','range'])}"><i class="fa fa-file-excel"></i> {__('Export XLS')}</a>
...
<div class="breadcrumb-title">
    <p>             
        ...
        <i class="fa fa-angle-right" style="margin: 0 5px 0 5px; font-size: 1em;"></i>{__('Registrations')}
...
{* =================================== PAGER/FILTER ======================================================== *}
     ...
     function getFilterParameters()
        ...
        $(".pager").click(function () {                     
                return $.ajax2({ data: getFilterParameters(), 
                                 url:"{url_to('customers_registration_ajax',['action'=>'ListPartialRegistration'])}?page="+$(this).attr('data-page'),                                
                                 target: "#actions"
                });
        });
        
         $("#init").click(function () {                     
                return $.ajax2({   url:"{url_to('customers_registration_ajax',['action'=>'ListPartialRegistration'])}",
                                 target: "#actions"
                });
        });
...
{* =================================== ACTIONS ======================================================== *}
      ...
      $("#Clear").click( function () { 
                if (!confirm('{__("Customer registration database will be cleared. Confirm ?")}')) return false; 
                return $.ajax2({ data :{ CustomerRegistration: $(this).attr('id') },
                                 url :"{url_to('customers_registration_ajax',['action'=>'ClearRegistration'])}",                               
                                 target: "#actions" 
                     });                                        
            });   
     ...
     $(".Delete").click( function () { 
                if (!confirm('{__("Customer registration \"#0#\" will be deleted. Confirm ?")}'.format($(this).attr('name')))) return false; 
                return $.ajax2({ data :{ CustomerRegistration: $(this).attr('id') },
                                 url :"{url_to('customers_registration_ajax',['action'=>'DeleteRegistration'])}",                               
                                 success : function(resp) {
                                       if (resp.action!='DeleteRegistration') return ;                                          
