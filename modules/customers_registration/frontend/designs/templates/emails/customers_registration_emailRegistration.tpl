        ...
        {if $model_i18n && $model_i18n->isLoaded()}
         ...
         {else}                   
            <div class="container email email-div">
                 {if in_array($_request->getLanguage(),['en','es','fr'])}
                {include file="./emails/i18n/`$_request->getLanguage()`/_emailRegistration.inc.tpl"}
                {else}
                {include file="./emails/i18n/en/_emailRegistration.inc.tpl"}
                {/if}
             </div>
         {/if}                       
    </body>
