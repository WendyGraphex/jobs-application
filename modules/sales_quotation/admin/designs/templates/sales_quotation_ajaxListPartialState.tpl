...
<div class="row admin-header-container">
    <div class="col-sm">
           ...
           <h3 class="admin-header-small-text">{__('Manage States of quotations')}</h3>
    </div>
...
<div class="breadcrumb-title">
    <p>
        ...
        <i class="fa fa-angle-right" style="margin: 0 5px 0 5px; font-size: 1em;"></i>{__('Quotation States')}
    </p>
</div>
...
<!-- Begin Add User -->
      ...
      <table class="table table-bordered table-hover table-striped" cellpadding="0" cellspacing="0"> 
        ...
        <th>{__('Position')}           
            <div style="float: right;">
                <a href="#" class="position-absolute order{$formFilter.order.position->getValueExist('asc','_active')}" id="asc" name="position"><i class="fas fa-sort-up {if $formFilter.order.position->getValue()=='asc'}text-dark{/if}" style=""></i></a>
        ...
        <th>{__('Title')}           
            <div style="float: right;">
                <a href="#" class="position-absolute order{$formFilter.order.title->getValueExist('asc','_active')}" id="asc" name="title"><i class="fas fa-sort-up {if $formFilter.order.title->getValue()=='asc'}text-dark{/if}" style=""></i></a>
    ...
    <tbody>
       <tr>
        ...
        <td>
            <input class="search form-control form-control-sm ml-1" placeholder="Name" name="name" value="{$formFilter.search.name}"/>
        </td>
        <td>
            
        </td>
        <td>
             <input class="search form-control form-control-sm ml-1" placeholder="Title" name="title" value="{$formFilter.search.title}"/>
        </td>
        <td>
     ...
     {* =================================== METHODS ======================================================== *}
        ...
        function updatePager(n)
        {
          ...
          {* =====================  P A G E R  A C T I O N S =============================== *}  
           ...
           $("#init").click(function() {                   
               return $.ajax2({ url:"{url_to('sales_quotation_ajax',['action'=>'ListPartialState'])}",                                    
                         target: "#actions"}); 
           }); 
          ...
          $("[name=nbitemsbypage]").change(function() { return updateFilter(); }); 
           ...
           $(".pager").click(function () {                      
                return $.ajax2({ data: getFilterParameters(), 
                                 url:"{url_to('sales_quotation_ajax',['action'=>'ListPartialState'])}?"+this.href.substring(this.href.indexOf("?")+1, this.href.length),
                                target: "#actions"
                });
        });
          {* =====================  A C T I O N S =============================== *}  
          ...
          $("#New").click( function () {             
            return $.ajax2({
                ...
                url: "{url_to('sales_quotation_ajax',['action'=>'NewStateI18n'])}",              
                target: "#actions"
           });
         });
         
         $(".View").click( function () {                
                return $.ajax2({ data : { QuotationStateI18n : { 
                                                state_id: $(this).attr('id'),
                                                lang: $("#SelectListLanguages option:selected").attr('id')                                            
                                    } },                                
                                url:"{url_to('sales_quotation_ajax',['action'=>'ViewStateI18n'])}",                              
                                target: "#actions"});
         });
       $(".list").dblclick( function () {                    
                  return $.ajax2({ data : { QuotationStateI18n : { state_id: $(this).attr('id'),lang: $("#SelectListLanguages option:selected").attr('id') } },                               
                                  url:"{url_to('sales_quotation_ajax',['action'=>'ViewStateI18n'])}", 
                                  target: "#actions"});
          });
        ...
        $(".Delete").click( function () { 
                ...
                return $.ajax2({ data :{ QuotationState: $(this).attr('id') },
                                 url :"{url_to('sales_quotation_ajax',['action'=>'DeleteState'])}",                                
        ...
        $("#Positions").click( function () {                
                return $.ajax2({ url:"{url_to('sales_quotation_ajax',['action'=>'PositionsState'])}",
                                target: "#actions"});
         });   
