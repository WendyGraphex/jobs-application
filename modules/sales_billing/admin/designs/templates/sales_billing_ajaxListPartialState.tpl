...
<div class="row admin-header-container">
    <div class="col-sm">
           ...
           <h3 class="admin-header-small-text">{__('Manage States of billings')}</h3>
    </div>
    <div class="col-sm" style="padding: 0;">
        <div style="float: right;">             
            <a href="#" class="btn btn-success admin-header-button btn-color" id="New" title="{__('New state')}">
...
<div class="breadcrumb-title">
    <p>
        ...
        <i class="fa fa-angle-right" style="margin: 0 5px 0 5px; font-size: 1em;"></i>{__('Billing States')}
    </p>
</div>
       ...
       <thead class="bgSpec">
        <tr class="list-header">
          ...
          <th class="footable-first-column" data-toggle="true">
              <span class="mr-1">{__('Name')}</span>  
              <div style="float: right;">
                <a href="#" class="position-absolute order{$formFilter.order.name->getValueExist('asc','_active')}" id="asc" name="name"><i class="fas fa-sort-up acs-desc {if $formFilter.order.name->getValue()=='asc'}text-dark{/if}"></i></a>
          ...
          <th data-hide="phone">
             <span class="mr-1">{__('Position')}</span>   
              <div style="float: right;">
                <a href="#" class="position-absolute order{$formFilter.order.position->getValueExist('asc','_active')}" id="asc" name="position"><i class="fas fa-sort-up acs-desc {if $formFilter.order.position->getValue()=='asc'}text-dark{/if}"></i></a>
          ...
          <th data-hide="phone">
            <span class="mr-1">{__('Title')}</span>  
              <div style="float: right;">
                <a href="#" class="position-absolute order{$formFilter.order.title->getValueExist('asc','_active')}" id="asc" name="title"><i class="fas fa-sort-up acs-desc {if $formFilter.order.title->getValue()=='asc'}text-dark{/if}"></i></a>
         ...
         {foreach $pager as $item}
          <tr class="list" id="{$item->get('id')}"> 
              ...
              <td>                            
               ...
               {if $item->hasIcon()} 
                   <img src="{$item->getIcon()->getURL('superadmin')}" style="margin-left: 5px;margin-right: 5px;" height="32" width="32" alt="{__('Icon')}"/>
               {/if}    
 ...
 {* =================================== METHODS ======================================================== *}
        ...
        function updatePager(n)
        {
          ...
          {* =====================  P A G E R  A C T I O N S =============================== *}  
           ...
           $("#init").click(function() {                   
               return $.ajax2({ url:"{url_to('sales_billing_ajax',['action'=>'ListPartialState'])}",                                    
                         target: "#actions"}); 
           }); 
          ...
          $("[name=nbitemsbypage]").change(function() { return updateFilter(); }); 
           ...
           $(".pager").click(function () {                      
                return $.ajax2({ data: getFilterParameters(), 
                                 url:"{url_to('sales_billing_ajax',['action'=>'ListPartialState'])}?"+this.href.substring(this.href.indexOf("?")+1, this.href.length),
                                target: "#actions"
                });
        });
          {* =====================  A C T I O N S =============================== *}  
          ...
          $("#New").click( function () {             
            return $.ajax2({
                ...
                url: "{url_to('sales_billing_ajax',['action'=>'NewStateI18n'])}",              
                target: "#actions"
           });
         });
         
         $(".View").click( function () {                
                return $.ajax2({ data : { SaleBillingStateI18n : { 
                                                state_id: $(this).attr('id'),
                                                lang: $("#SelectListLanguages option:selected").attr('id')                                            
                                    } },                                
                                url:"{url_to('sales_billing_ajax',['action'=>'ViewStateI18n'])}",                              
                                target: "#actions"});
         });
       $(".list").dblclick( function () {                    
                  return $.ajax2({ data : { SaleBillingStateI18n : { state_id: $(this).attr('id'),lang: $("#SelectListLanguages option:selected").attr('id') } },                               
                                  url:"{url_to('sales_billing_ajax',['action'=>'ViewStateI18n'])}", 
                                  target: "#actions"});
          });
        ...
        $(".Delete").click( function () { 
                ...
                return $.ajax2({ data :{ SaleBillingState: $(this).attr('id') },
                                 url :"{url_to('sales_billing_ajax',['action'=>'DeleteState'])}",                                
                                 success : function(resp) {
                                          if (resp.action != 'DeleteState')  return ;                                                                              
        ...
        $("#Positions").click( function () {                
                return $.ajax2({ url:"{url_to('sales_billing_ajax',['action'=>'PositionsState'])}",
                                target: "#actions"});
         });   
