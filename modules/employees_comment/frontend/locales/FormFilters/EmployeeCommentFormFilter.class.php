...
class EmployeeCommentFormFilter extends mfFormFilter2 {
    ...
    {  
       ...
       $this->setValidators(array( 
        ...
        $this->setQuery(mfQuery::getInstance()->select("{fields}")
                    ->from(EmployeeAdvertComment::getTable())                              
                    ->inner(EmployeeAdvertComment::getOuterForJoin('employer_user_id'))                          
                    ->inner(EmployeeAdvertCommentI18n::getInnerForJoin('comment_id'))       
                    ->where( EmployeeAdvertComment::getTableField('is_active')."='YES' AND sender='EMPLOYER' AND ".                         
                            EmployeeAdvertComment::getTableField('employee_user_id')."='{employee_user_id}'"));  
    }
