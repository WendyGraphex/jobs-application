          ...
          <div class="row admin-header-container">
            <div class="col-sm">
              ...
              <h3 class="admin-header-small-text">{__('Advert abuses management')}</h3>
            </div>
          ...
          <div class="breadcrumb-title">
            <p>
                ...
                <i class="fa fa-angle-right" style="margin: 0 5px 0 5px; font-size: 1em;"></i>{__('Advert abuses')}
            </p>
          </div>
          <!-- End breadcrumb -->
{alerts}
...
<div class=" bg-white px-2 py-2">
      ...
      <tr class="list" id="{$item->get('id')}" role="row">
          ...
          <td>
              {if $item->hasType()} 
                 {if $item->getType()->hasI18n()}
                     {$item->getType()->getI18n()}
                 {/if}    
             {else}
...
<script type="text/javascript">
     ...
     function getFilterParameters()
        ...
        {          
           return $.ajax2({ data: getFilterParameters(), 
                            url:"{url_to('employees_contact_ajax',['action'=>'ListPartialAbuse'])}" ,                          
                            target: "#actions"
                             });
        }
        ...
        $(".pager").click(function () {                     
                return $.ajax2({ data: getFilterParameters(), 
                                 url:"{url_to('employees_contact_ajax',['action'=>'ListPartialAbuse'])}?page="+$(this).attr('data-page'),                                
                                 target: "#actions"
                });
        });
        
         $("#init").click(function () {                     
                return $.ajax2({   url:"{url_to('employees_contact_ajax',['action'=>'ListPartialAbuse'])}",
                                 target: "#actions"
                });
        });
     ...
     $(".Delete").click( function () { 
                if (!confirm('{__("Employee advert abuse \"#0#\" will be deleted. Confirm ?")}'.format($(this).attr('name')))) return false; 
                return $.ajax2({ data :{ EmployeeAdvertI18nAbuse: $(this).attr('id') },
