...
class EmployeeContactEvents extends ProjectEventsBase {
   ...
   /* static function setSettingsForForm(mfEvent $event)
    ...
    {
        if (!mfModule::isModuleInstalled('employees_contact'))
             return ;       
        $form=$event->getSubject();
        $form->addValidators(array(                               
            "employees_contact_admin_recipients"=>new mfValidatorMultiple(new mfValidatorEmail(),array('required'=>false)),            
        ));
        $form->getSettings()->add(array( 
                                  ));
        $form->getComponents()->set("30_00_employees_contact_email","/employees_contact/emails");
    }   
}
