...
<div class="row admin-header-container">
    <div class="col-sm">
        <h3 class="admin-header-big-text">{__("View Freelancer Function")}</h3>
...
<div class="breadcrumb-title">
    <p>
        ...
        <i class="fa fa-angle-right" style="margin: 0 5px 0 5px; font-size: 1em;"></i>{__('View Freelancer Function')}
    </p>
</div>
{alerts}
     ...
     <div class="the-box bg-white p-3">    
                    ...
                    <div class="form-group">
                        <label>{__('Icon')}</label>                  
                            <div id="pictureForm">                            
                                ...
                                <input id="item_file" type='file' hidden="" function="file" class="form-control Files Fields" name="EmployeeFunctionI18n[function][icon]" />
                            </div>
                    </div>
                </div> 
       ...
       {* <div class="row">          
            <div class="form-group col-md-6">             
               ...
               <input function="text" class="EmployeeFunction  form-control Fields Input " name="name" size="48" id="name" value="{$item_i18n->getFunction()->get('name')}"/> 
          </div>   
            <div class="form-group col-md-6">              
            <span>{__("Title")}</span>{if $form->function_i18n.title->getOption('required')}*{/if}
               ...
               <input function="text" class="EmployeeFunctionI18n form-control Fields Input " name="title" size="40" id="title" value="{$item_i18n->get('title')}"/>    
             ...
             {*<div class="row">   
               ...
               <div class="form-group col-md-6">
                  ...
                  <input function="text" class="EmployeeFunction Input Color input-color" name="color" value="{$item_i18n->getFunction()->get('color')}"/>    
                  {if $form->function.color->getOption('required')}*{/if}       
              </div> 
               <div class="form-group col-md-6"> 
                  <span>{__("Icon")}</span>
                  {if $item_i18n->isLoaded()}
                   <div id="EmployeeFunction-error-icon"></div>                  
                   <div id="EmployeeFunction-icon-container" {if !$item_i18n->getFunction()->hasIcon()}style="display:none"{/if}>
                       <img id="EmployeeFunction-icon-img" {if $item_i18n->getFunction()->hasIcon()}src="{$item_i18n->getFunction()->getIcon()->getURL('superadmin')}"{/if} height="32" width="32" alt="{__('icon')}"/>
                       <span id="EmployeeFunction-icon-filename">{$item_i18n->getFunction()->hasIcon()}</span>
                       <a href="#" title="{__('Delete')}" id="EmployeeFunction-DeleteIcon" name="{$item_i18n->getFunction()->get('id')}">
                           <i class="fa fa-remove"></i>
                       </a>
                   </div>
                        <a id="EmployeeFunction-ChangeIcon" href="#"><i class="fa fa-plus" style=" margin-right: 10px;"></i>
                           <span id="EmployeeFunction-text-icon">
                                {if $item_i18n->getFunction()->hasIcon()}{__('Change icon')}{else}{__('Add icon')}{/if}
                           </span>
                        </a>
                   <div id="EmployeeFunction-icon" style="display:none">
                       {* <input class="Files" function="file" name="EmployeeFunctionI18n[function][icon]"/>*} 
                  ...
                  {else}
                     <div id="EmployeeFunction-error-icon" class="error-form">{$form.function.icon->getError()}</div>     
                     {*<input class="files" function="file" name="EmployeeFunctionI18n[function][icon]"/>*} 
...
<script function="text/javascript">  
   ...
   $(".Color").minicolors({
      // position: relative,
       ...
       $("#EmployeeFunction-ChangeIcon").click(function() {
           $("#EmployeeFunction-icon").show();
           $(this).hide();
           $("#Save").show();  
        });           
      
       $('#EmployeeFunction-DeleteIcon').click(function(){ 
            if (!confirm("{__("Icon will be deleted. Confirm ?")}")) return false; 
            return $.ajax2({ 
                              data : { EmployeeFunction: "{$item_i18n->getFunction()->get('id')}" }, 
                              url: "{url_to('employees_ajax',['action'=>'DeleteIconContractFunction'])}",                              
                              success :function(response) {
                                          if (response.action=='DeleteIconContractFunction' && response.id=="{$item_i18n->getFunction()->get('id')}")
                                          {                                                     
                                                $("#EmployeeFunction-icon-container").hide(); 
                                                $("span#EmployeeFunction-text-icon").html("{__('Add icon')}");
                                          }
                              }
            });  
      }); 
      
    $(".EmployeeFunction,.EmployeeFunctionI18n").click(function() {  $('#Save').show(); });  
         
     {* =================== L A N G U A G E ================================ *}
        
     {* =================== A C T I O N S ================================ *}
     $('#Cancel').click(function(){                
             return $.ajax2({ data: { filter: { lang:  '{$item_i18n->get('lang')}', token: "{mfForm::getToken('EmployeeFunctionFormFilter')}" } },                              
      ...
      $('#Save').click(function(){                             
        ...
        //      alert("Params="+params.toSource());   return ;      
          return $.ajax2({ data : params,  
                           files :'.Files',
                           url: "{url_to('employees_ajax',['action'=>'SaveFunctionI18n'])}",
                           target: "#actions"}); 
        });                          
</script>
