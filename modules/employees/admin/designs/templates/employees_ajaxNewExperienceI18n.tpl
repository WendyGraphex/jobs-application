...
<div class="row admin-header-container">
    <div class="col-sm">
        <h3 class="admin-header-big-text">{__("New Freelancer Experience")}</h3>
...
<div class="breadcrumb-title">
    <p>
        ...
        <i class="fa fa-angle-right" style="margin: 0 5px 0 5px; font-size: 1em;"></i>{__('New Freelancer Experience')}
    </p>
</div>
{alerts}
     ...
     <div class="the-box bg-white p-3">    
         <div class="row">
          <div class="form-group col-md-12">          
            <div>{$form.experience_i18n.lang->getError()}</div> 
            {component name="/site_languages/selectListLanguagesFrontend" selected=$form.experience_i18n.lang}
          </div>          
        </div>
                    ...
                    <div class="form-group">
                        <label>{__('Icon')}</label>                  
                            <div id="pictureForm">                            
                                ...
                                <input id="item_file" type='file' hidden="" function="file" class="form-control Files Fields" name="EmployeeExperienceI18n[experience][icon]" /> {*name="SiteCityI18n[experience][icon]"*}
                            </div>
                    </div>
                </div> 
              </div>
        ...
        {*<div class="row">          
           ...
           <div class="form-group col-md-6">             
              {if $form->hasErrors()}<div class="alert alert-{if $form.experience.icon->hasError()}danger{else}light{/if}" role="alert">&nbsp;{$form.experience.icon->getError()}</div>{/if}
            <label for="icon">{__('Icon')}{if $form->experience.icon->getOption('required')}*{/if}</label> 
...
<script function="text/javascript">
   ...
   $(".Color").minicolors({
     ...
     $('#Cancel').click(function(){                
             return $.ajax2({ data: { filter: { lang:$("#SelectListLanguages option:selected").attr('id'), token: "{mfForm::getToken('EmployeeExperienceFormFilter')}" } },                              
      ...
      $('#Save').click(function(){                             
            var  params= {      iFrame:true,             
                                EmployeeExperienceI18n: { 
                                   experience_i18n : { lang: $("#SelectListLanguages option:selected").attr('id')  },
                                   experience : { },
        ...
        //      alert("Params="+params.toSource());   return ;      
          return $.ajax2({ data : params,  
                           files :'.Files',
                           url: "{url_to('employees_ajax',['action'=>'SaveNewExperienceI18n'])}",
                           target: "#actions"}); 
        });                          
