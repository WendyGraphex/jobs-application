...
<div class="table-responsive bg-white px-2 py-2">            
      ...
      <tr class="list" id="{$item->get('id')}" role="row">
          ...
          <td>{$item->get('tpl')}</td> 
          <td>{$item->getNumberOfEmployees()}</td>
          <td>       
            ...
            <td>             
                  <a href="#" title="{__('Edit')}" class="View" id="{$item->get('id')}">
                                <i class="fa fa-pencil-alt icon-font"></i></a>   
                                 <a href="#" title="{__('Freelancers')}" class="Employees" id="{$item->get('id')}">
  ...
  var updated_at_dates = $( ".Range.updatedAtDate#updated_at_from, .Range.updatedAtDate#updated_at_to" ).datepicker({
        ...
        {          
           return $.ajax2({ data: getFilterParameters(), 
                            url:"{url_to('employees_ajax',['action'=>'ListPartialGallery'])}" ,                          
                            target: "#actions"
                             });
        }
...
{* =================================== PAGER/FILTER ======================================================== *}  
        ...
        $(".pager").click(function () {                     
                return $.ajax2({ data: getFilterParameters(), 
                                 url:"{url_to('employees_ajax',['action'=>'ListPartialGallery'])}?page="+$(this).attr('data-page'),                                
                                 target: "#actions"
                });
        });
...
{* =================================== ACTIONS ======================================================== *}
     ...
     $(".Delete").click( function () { 
                ...
                return $.ajax2({ data :{ EmployeeGallery: $(this).attr('id') },
                                 url :"{url_to('employees_ajax',['action'=>'DeleteGallery'])}",                               
      ...
      $(".View").click( function () {                    
                  return $.ajax2({  data : { EmployeeGalleryI18n : { gallery_id: $(this).attr('id'),lang: $("#SelectListLanguages option:selected").attr('id') } },                                                          
                                  url:"{url_to('employees_ajax',['action'=>'ViewGalleryI18n'])}", 
                                  target: "#actions"});
          });
      ...
      $("#New").click( function () {             
                return $.ajax2({       
                    ...
                    url: "{url_to('employees_ajax',['action'=>'NewGalleryI18n'])}",               
                    target: "#actions"
              });
            });
        ...
        $(".ChangeIsActive").click(function () {
            return $.ajax2({
                data : { EmployeeGallery: { id: $(this).attr('id') , value:$(this).attr('name'),token :'{mfForm::getToken('ChangeForm')}' } },                              
                url: "{url_to('employees_ajax',['action'=>'ChangeIsActiveGallery'])}",              
                success: function (resp) {
