          ...
          <div class="row admin-header-container">
            <div class="col-sm">
              ...
              <h3 class="admin-header-small-text">{__('Freelancer populars management')}</h3>
            </div>
          ...
          <div class="breadcrumb-title">
            <p>
                ...
                <i class="fa fa-angle-right" style="margin: 0 5px 0 5px; font-size: 1em;"></i>{__('Freelancer Populars')}
            </p>
          </div>
          <!-- End breadcrumb -->
{alerts}
...
<div class=" bg-white px-2 py-2">
      ...
      <tr class="list" id="{$item->get('id')}" role="row">
          ...
          <td>
              <a href="{$item->getUser()->getUrl()}">{$item->getUser()->getFormatter()->getname()}</a>
          </td>  
          <td>
...
<script type="text/javascript">
    ...
    var updated_dates = $( " .Range.UpdatedAtDate#updated_at_from, .Range.UpdatedAtDate#updated_at_to" ).datepicker({
     ...
     function getFilterParameters()
        ...
        {          
           return $.ajax2({ data: getFilterParameters(), 
                            url:"{url_to('employees_ajax',['action'=>'ListPartialEmployeePopular'])}" ,                          
                            target: "#actions"
                             });
        }
        ...
        $(".pager").click(function () {                     
                return $.ajax2({ data: getFilterParameters(), 
                                 url:"{url_to('employees_ajax',['action'=>'ListPartialEmployeePopular'])}?page="+$(this).attr('data-page'),                                
                                 target: "#actions"
                });
        });
        
         $("#init").click(function () {                     
                return $.ajax2({   url:"{url_to('employees_ajax',['action'=>'ListPartialEmployeePopular'])}",
                                 target: "#actions"
                });
        });
     ...
     $(".Delete").click( function () { 
                if (!confirm('{__("Freelancer popular \"#0#\" will be deleted. Confirm ?")}'.format($(this).attr('name')))) return false; 
                return $.ajax2({ data :{ EmployeePopular: $(this).attr('id') },
                                 url :"{url_to('employees_ajax',['action'=>'DeleteEmployeePopular'])}",                               
                                 success : function(resp) {
                                       if (resp.action!='DeletePopular') return ;                                          
