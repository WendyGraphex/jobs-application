...
<div class="row admin-header-container">
    <div class="col-sm">
           <h3 class="admin-header-big-text">{__('Configurations|Employee Skills')}</h3>
           <h3 class="admin-header-small-text">{__('Manage Skills of employees')}</h3>
    </div>
    <div class="col-sm" style="padding: 0;">
        <div style="float: right;"> 
            ...
            <a href="#" class="btn btn-success admin-header-button btn-color" id="New" title="{__('New')}">
             ...
             <a href="#" class="btn btn-success admin-header-button btn-color" id="Multiple" title="{__('New Multiple Skills')}">
                <i class="fa fa-list" style="margin-right: 5px;"></i>
                {__('New Multiple Skill')}
            </a>
             <a href="#" class="btn btn-success admin-header-button btn-color" id="Multiple" title="{__('Download Skills')}">
                <i class="fa fa-download" style="margin-right: 5px;"></i>
                {__('Import')}
            </a>
            
             <a class="btn btn-warning admin-header-button" href="{url_to('employees',['action'=>'ExportSkillCsv'])}?{$formFilter->getParametersForUrl(['equal','in','begin','search','range'])}"><i class="fa fa-file-excel"></i> {__('Export XLS')}</a> 
        </div>
    </div>
</div>
<div class="breadcrumb-title">
    <p>
        ...
        <i class="fa fa-angle-right" style="margin: 0 5px 0 5px; font-size: 1em;"></i>{__('Employee Skills')}
    </p>
</div>
...
<div class=" bg-white px-2 py-2">
        ...
        <table class="table table-bordered table-hover table-striped" cellpadding="0" cellspacing="0"> 
         <thead class="bgSpec">
          ...
          <th data-hide="phone">
            <span>{__('Title/Description')}</span>  
            <div style="float: right;">
         ...
         {foreach $pager as $item}
          <tr class="list" id="{$item->get('id')}"> 
              ...
              <td class="ModeDisplay">
                ...
                <td class="ModeDisplay">
                {if $item->hasPicture()} 
                     <img width="256px" src="{$item->getPicture()->getURL()}" class="classPicture" alt="{__('Picture')}"/>
                 {/if} 
               </td>
              <td>                
                        {$item->get('name')}
              ...
              <td>                            
                ...
                <div>
                    {if $item->hasI18n()}
                        <a href="{$item->getI18n()->getUrl()}" target="_blank">{$item->getI18n()->getFormatter()->getTitle()->ucfirst()}</a>
                    {else}
                        {__('----')}
                    {/if}    
                    </div>
                    <div>
                 {$item->getI18n()->getFormatter()->getDescription()->ucfirst()|truncate:80} 
                    </div>
              </td>
                <td>       
    ...
    var updated_dates = $( " .Range.UpdatedAtDate#updated_at_from, .Range.UpdatedAtDate#updated_at_to" ).datepicker({
        ...
        function updatePager(n)
        {
          ...
          {* =====================  P A G E R  A C T I O N S =============================== *}  
           ...
           $("#init").click(function() {                   
               return $.ajax2({ url:"{url_to('employees_ajax',['action'=>'ListPartialSkill'])}",                                    
                         target: "#actions"}); 
           }); 
          ...
          $("[name=nbitemsbypage]").change(function() { return updateFilter(); }); 
           ...
           $(".pager").click(function () {                      
                return $.ajax2({ data: getFilterParameters(), 
                                 url:"{url_to('employees_ajax',['action'=>'ListPartialSkill'])}?"+this.href.substring(this.href.indexOf("?")+1, this.href.length),
                                target: "#actions"
                });
        });
          {* =====================  A C T I O N S =============================== *}  
          ...
          $("#New").click( function () {             
            return $.ajax2({
                ...
                url: "{url_to('employees_ajax',['action'=>'NewSkillI18n'])}",              
                target: "#actions"
           });
         });
         
         $(".View").click( function () {                
                return $.ajax2({ data : { EmployeeSkillI18n : { 
                                                skill_id: $(this).attr('id'),
                                                lang: $("#SelectListLanguages option:selected").attr('id')                                            
                                    } },                                
                                url:"{url_to('employees_ajax',['action'=>'ViewSkillI18n'])}",                              
                                target: "#actions"});
         });
         
         
       $(".list").dblclick( function () {                    
                  return $.ajax2({ data : { EmployeeSkillI18n : { skill_id: $(this).attr('id'),lang: $("#SelectListLanguages option:selected").attr('id') } },                               
                                  url:"{url_to('employees_ajax',['action'=>'ViewSkillI18n'])}", 
                                  target: "#actions"});
          });
        ...
        $(".Delete").click( function () { 
                if (!confirm('{__("Freelancer Skill \"#0#\" will be deleted. Confirm ?")}'.format($(this).attr('name')))) return false; 
                return $.ajax2({ data :{ EmployeeSkill: $(this).attr('id') },
                                 url :"{url_to('employees_ajax',['action'=>'DeleteSkill'])}",                                
                                 success : function(resp) {
                                          if (resp.action != 'DeleteSkill')  return ;                                                                              
        ...
        $("#Positions").click( function () {                
                return $.ajax2({ url:"{url_to('employees_ajax',['action'=>'PositionsSkill'])}",
                                target: "#actions"});
         });   
        ...
        $("#Multiple").click( function () {                
                return $.ajax2({  data : { lang : { lang: $("#SelectListLanguages option:selected").attr('id'),token: "{mfForm::getToken('LanguageFrontendForm')}" } },     
                                url:"{url_to('employees_ajax',['action'=>'NewMultipleSkillI18n'])}",
                                target: "#actions"});
         });
</script>    
