...
<div class="row admin-header-container">
    <div class="col-sm">
           <h3 class="admin-header-big-text">{__('Configurations|Functions')}</h3>
           <h3 class="admin-header-small-text">{__('Manage Functions of employees')}</h3>
    </div>
...
<div class="breadcrumb-title">
    <p>
        ...
        <i class="fa fa-angle-right" style="margin: 0 5px 0 5px; font-size: 1em;"></i>{__('Functions')}
    </p>
</div>
    ...
    var updated_dates = $( ".Range.UpdatedAtDate#updated_at_from, .Range.UpdatedAtDate#updated_at_to" ).datepicker({
        ...
        function updatePager(n)
        {
          ...
          {* =====================  P A G E R  A C T I O N S =============================== *}  
           ...
           $("#init").click(function() {                   
               return $.ajax2({ url:"{url_to('employees_ajax',['action'=>'ListPartialFunction'])}",                                    
                         target: "#actions"}); 
           }); 
          ...
          $("[name=nbitemsbypage]").change(function() { return updateFilter(); }); 
           ...
           $(".pager").click(function () {                      
                return $.ajax2({ data: getFilterParameters(), 
                                 url:"{url_to('employees_ajax',['action'=>'ListPartialFunction'])}?"+this.href.substring(this.href.indexOf("?")+1, this.href.length),
                                target: "#actions"
                });
        });
          {* =====================  A C T I O N S =============================== *}  
          ...
          $("#New").click( function () {             
            return $.ajax2({
                ...
                url: "{url_to('employees_ajax',['action'=>'NewFunctionI18n'])}",              
                target: "#actions"
           });
         });
         
         $(".View").click( function () {                
                return $.ajax2({ data : { EmployeeFunctionI18n : { 
                                                function_id: $(this).attr('id'),
                                                lang: $("#SelectListLanguages option:selected").attr('id')                                            
                                    } },                                
                                url:"{url_to('employees_ajax',['action'=>'ViewFunctionI18n'])}",                              
                                target: "#actions"});
         });
       $(".list").dblclick( function () {                    
                  return $.ajax2({ data : { EmployeeFunctionI18n : { function_id: $(this).attr('id'),lang: $("#SelectListLanguages option:selected").attr('id') } },                               
                                  url:"{url_to('employees_ajax',['action'=>'ViewFunctionI18n'])}", 
                                  target: "#actions"});
          });
        ...
        $(".Delete").click( function () { 
                if (!confirm('{__("Contract Function \"#0#\" will be deleted. Confirm ?")}'.format($(this).attr('name')))) return false; 
                return $.ajax2({ data :{ EmployeeFunction: $(this).attr('id') },
                                 url :"{url_to('employees_ajax',['action'=>'DeleteFunction'])}",                                
                                 success : function(resp) {
                                          if (resp.action != 'DeleteFunction')  return ;                                                                              
        ...
        $("#Positions").click( function () {                
                return $.ajax2({ url:"{url_to('employees_ajax',['action'=>'PositionsFunction'])}",
                                target: "#actions"});
         });   
