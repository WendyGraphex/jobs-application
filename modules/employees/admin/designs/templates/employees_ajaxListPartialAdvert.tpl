<div class="row admin-header-container">
  <div class="col-sm">
    ...
    <h3 class="admin-header-small-text">{__('Freelancer adverts management')}</h3>
  </div>
                         ...
                         {foreach $pager as $item} 
                            <tr class="list" id="{$item->get('id')}" role="row">
                                <td class="count">{(($pager->getPage()-1)*$pager->getNbItemsByPage())+$item@iteration}</td>     
                                 <td>{$item->getEmployeeUser()->getFormatter()->getName()} ({$item->getEmployeeUser()->getFormatter()->getNumberOfAdverts()}) 
                                     <div>{$item->getEmployeeUser()->get('email')} / {$item->getEmployeeUser()->get('mobile')}</div>
                                 </td>
...
{* =================================== PAGER/FILTER ======================================================== *}             
     ...
     $("#GenerateRevivals").click(function() {                   
              return $.ajax2({ url:"{url_to('employees_ajax',['action'=>'GenerateRevival'])}",                                    
                         target: "#actions"}); 
        });  
</script> 
