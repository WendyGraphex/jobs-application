...
<div class="row admin-header-container">
    <div class="col-sm">
           <h3 class="admin-header-big-text">{__('Configurations | Freelancer Language Levels')}</h3>
           <h3 class="admin-header-small-text">{__('Manage Language Levels of employees')}</h3>
    </div>
...
<div class="breadcrumb-title">
    <p>
        ...
        <i class="fa fa-angle-right" style="margin: 0 5px 0 5px; font-size: 1em;"></i>{__('Language Levels')}
    </p>
</div>
    ...
    var updated_dates = $( " .Range.UpdatedAtDate#updated_at_from, .Range.UpdatedAtDate#updated_at_to" ).datepicker({
        ...
        function updatePager(n)
        {
          ...
          {* =====================  P A G E R  A C T I O N S =============================== *}  
           ...
           $("#init").click(function() {                   
               return $.ajax2({ url:"{url_to('employees_ajax',['action'=>'ListPartialLanguageLevel'])}",                                    
                         target: "#actions"}); 
           }); 
          ...
          $("[name=nbitemsbypage]").change(function() { return updateFilter(); }); 
           ...
           $(".pager").click(function () {                      
                return $.ajax2({ data: getFilterParameters(), 
                                 url:"{url_to('employees_ajax',['action'=>'ListPartialLanguageLevel'])}?"+this.href.substring(this.href.indexOf("?")+1, this.href.length),
                                target: "#actions"
                });
        });
          {* =====================  A C T I O N S =============================== *}  
          ...
          $("#New").click( function () {             
            return $.ajax2({
                ...
                url: "{url_to('employees_ajax',['action'=>'NewLanguageLevelI18n'])}",              
                target: "#actions"
           });
         });
         
         $(".View").click( function () {                
                return $.ajax2({ data : { EmployeeLanguageLevelI18n : { 
                                                level_id: $(this).attr('id'),
                                                lang: $("#SelectListLanguages option:selected").attr('id')                                            
                                    } },                                
                                url:"{url_to('employees_ajax',['action'=>'ViewLanguageLevelI18n'])}",                              
                                target: "#actions"});
         });
       $(".list").dblclick( function () {                    
                  return $.ajax2({ data : { EmployeeLanguageLevelI18n : { level_id: $(this).attr('id'),lang: $("#SelectListLanguages option:selected").attr('id') } },                               
                                  url:"{url_to('employees_ajax',['action'=>'ViewLanguageLevelI18n'])}", 
                                  target: "#actions"});
          });
        ...
        $(".Delete").click( function () { 
                if (!confirm('{__("Employee Language Level \"#0#\" will be deleted. Confirm ?")}'.format($(this).attr('name')))) return false; 
                return $.ajax2({ data :{ EmployeeLanguageLevel: $(this).attr('id') },
                                 url :"{url_to('employees_ajax',['action'=>'DeleteLanguageLevel'])}",                                
                                 success : function(resp) {
                                          if (resp.action != 'DeleteLanguageLevel')  return ;                                                                              
        ...
        $("#Positions").click( function () {                
                return $.ajax2({ url:"{url_to('employees_ajax',['action'=>'PositionsLanguageLevel'])}",
                                target: "#actions"});
         });   
