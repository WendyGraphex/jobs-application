          ...
          <div class="row admin-header-container">
            <div class="col-sm">
              ...
              <h3 class="admin-header-small-text">{__('Freelancers contact management')}</h3>
            </div>
            <div class="col-sm" style="padding: 0;">
                <div style="float: right;"> 
                  {*  <a class="btn btn-success admin-header-button"  href="#" id="New"><i class="fa fa-plus"></i> {__('New Contact')}</a>*}
...
<div class=" bg-white px-2 py-2">
      ...
      <thead class="bgSpec" {*class="bg-info thead-color"*}>
         <tr role="row">
            ...
            <th class="sorting">{__('Address2')}</th>
            <th class="sorting">{__('City')}  
              <div style="float: right;">
                <a href="#" class="position-absolute order{$formFilter.order.city->getValueExist('asc','_active')}" id="asc" name="city"><i class="fas fa-sort-up"></i></a>
                <a href="#" class="position-absolute order{$formFilter.order.city->getValueExist('desc','_active')}" id="desc" name="city"><i class="fas fa-sort-down"></i></a>            
              </div>
            </th>
      ...
      <tbody>
          <tr role="row">
              ...
              <td><input class="form-control form-control-sm ml-1 search" name="fax" value="{$formFilter.search.fax}"></td>
              <td><input class="form-control form-control-sm ml-1 search" name="address1" value="{$formFilter.search.address1}"></td>
              <td><input class="form-control form-control-sm ml-1 search" name="address2" value="{$formFilter.search.address2}"></td>
              <td>
...
<script type="text/javascript">
    ...
    var updated_dates = $( " .Range.UpdatedAtDate#updated_at_from, .Range.UpdatedAtDate#updated_at_to" ).datepicker({
     ...
     function getFilterParameters()
        ...
        {          
           return $.ajax2({ data: getFilterParameters(), 
                            url:"{url_to('candidates_ajax',['action'=>'ListPartialContact'])}" ,                          
                            target: "#actions"
                             });
        }
        ...
        $(".pager").click(function () {                     
                return $.ajax2({ data: getFilterParameters(), 
                                 url:"{url_to('candidates_ajax',['action'=>'ListPartialContact'])}?page="+$(this).attr('data-page'),                                
                                 target: "#actions"
                });
        });
        
         $("#init").click(function () {                     
                return $.ajax2({   url:"{url_to('candidates_ajax',['action'=>'ListPartialContact'])}",
                                 target: "#actions"
                });
        });
      ...
      $(".View").click( function () {                    
                  return $.ajax2({ data : { Employee : $(this).attr('id') },                               
                                  url:"{url_to('candidates_ajax',['action'=>'ViewEmployee'])}", 
     ...
     $(".Delete").click( function () { 
                ...
                return $.ajax2({ data :{ EmployeeContact: $(this).attr('id') },
                                 url :"{url_to('candidates_ajax',['action'=>'DeleteContact'])}",                               
