...
<div class="row admin-header-container">
    <div class="col-sm">
           <h3 class="admin-header-big-text">{__('Configurations|Experiences')}</h3>
           <h3 class="admin-header-small-text">{__('Manage Experiences of employees')}</h3>
    </div>
...
<div class="breadcrumb-title">
    <p>
        ...
        <i class="fa fa-angle-right" style="margin: 0 5px 0 5px; font-size: 1em;"></i>{__('Experiences')}
    </p>
</div>
...
<div class=" bg-white px-2 py-2">
    ...
    <tbody>
       <tr>
        ...
        <td>
            <input class="search form-control form-control-sm ml-1" placeholder="{__('Name')}" name="name" value="{$formFilter.search.name}"/>
        </td>
        <td>
            
        </td>
        <td>
             <input class="search form-control form-control-sm ml-1" placeholder="{__('Title')}" name="title" value="{$formFilter.search.title}"/>
        </td>
        <td>
    ...
    var updated_dates = $( ".Range.UpdatedAtDate#updated_at_from, .Range.UpdatedAtDate#updated_at_to" ).datepicker({
        ...
        function updatePager(n)
        {
          ...
          {* =====================  P A G E R  A C T I O N S =============================== *}  
           ...
           $("#init").click(function() {                   
               return $.ajax2({ url:"{url_to('employees_ajax',['action'=>'ListPartialExperience'])}",                                    
                         target: "#actions"}); 
           }); 
          ...
          $("[name=nbitemsbypage]").change(function() { return updateFilter(); }); 
           ...
           $(".pager").click(function () {                      
                return $.ajax2({ data: getFilterParameters(), 
                                 url:"{url_to('employees_ajax',['action'=>'ListPartialExperience'])}?"+this.href.substring(this.href.indexOf("?")+1, this.href.length),
                                target: "#actions"
                });
        });
          {* =====================  A C T I O N S =============================== *}  
          ...
          $("#New").click( function () {             
            return $.ajax2({
                ...
                url: "{url_to('employees_ajax',['action'=>'NewExperienceI18n'])}",              
                target: "#actions"
           });
         });
         
         $(".View").click( function () {                
                return $.ajax2({ data : { EmployeeExperienceI18n : { 
                                                experience_id: $(this).attr('id'),
                                                lang: $("#SelectListLanguages option:selected").attr('id')                                            
                                    } },                                
                                url:"{url_to('employees_ajax',['action'=>'ViewExperienceI18n'])}",                              
                                target: "#actions"});
         });
       $(".list").dblclick( function () {                    
                  return $.ajax2({ data : { EmployeeExperienceI18n : { experience_id: $(this).attr('id'),lang: $("#SelectListLanguages option:selected").attr('id') } },                               
                                  url:"{url_to('employees_ajax',['action'=>'ViewExperienceI18n'])}", 
                                  target: "#actions"});
          });
        ...
        $(".Delete").click( function () { 
                if (!confirm('{__("Contract Experience \"#0#\" will be deleted. Confirm ?")}'.format($(this).attr('name')))) return false; 
                return $.ajax2({ data :{ EmployeeExperience: $(this).attr('id') },
                                 url :"{url_to('employees_ajax',['action'=>'DeleteExperience'])}",                                
                                 success : function(resp) {
                                          if (resp.action != 'DeleteExperience')  return ;                                                                              
        ...
        $("#Positions").click( function () {                
                return $.ajax2({ url:"{url_to('employees_ajax',['action'=>'PositionsExperience'])}",
                                target: "#actions"});
         });   
