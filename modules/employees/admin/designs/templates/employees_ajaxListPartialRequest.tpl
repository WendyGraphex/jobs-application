          ...
          <div class="row admin-header-container">
            <div class="col-sm">
              ...
              <h3 class="admin-header-small-text">{__('Freelancer request management')}</h3>
            </div>
     ...
     function getFilterParameters()
        ...
        {          
           return $.ajax2({ data: getFilterParameters(), 
                            url:"{url_to('employees_ajax',['action'=>'ListPartialRequest'])}" ,                          
                            target: "#actions"
                             });
        }
...
{* =================================== PAGER/FILTER ======================================================== *}
        ...
        $(".pager").click(function () {                     
                return $.ajax2({ data: getFilterParameters(), 
                                 url:"{url_to('employees_ajax',['action'=>'ListPartialRequest'])}?page="+$(this).attr('data-page'),                                
                                 target: "#actions"
                });
        });
        
         $("#init").click(function () {                     
                return $.ajax2({   url:"{url_to('employees_ajax',['action'=>'ListPartialRequest'])}",
                                 target: "#actions"
                });
        });
...
{* =================================== ACTIONS ======================================================== *}
     ...
     $(".Delete").click( function () { 
                if (!confirm('{__("Freelancer contact \"#0#\" will be deleted. Confirm ?")}'.format($(this).attr('name')))) return false; 
                return $.ajax2({ data :{ EmployeeAdvertRequest: $(this).attr('id') },
                                 url :"{url_to('employees_ajax',['action'=>'DeleteAdvertRequest'])}",                               
                                 success : function(resp) {
                                       if (resp.action!='DeleteAdvertRequest') return ;                                          
