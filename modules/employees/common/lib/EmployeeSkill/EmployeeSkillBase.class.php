...
class EmployeeSkillBase extends mfOrderedObject3 {
     
    protected static $fields=array('name','picture','icon','color','position','created_at','updated_at');
    const table="t_employee_skill";
    ...
    function hasI18n()
    {
     ...
     {       
         if ($this->i18n===null)
         {
             ...
             $this->i18n=new EmployeeSkillI18n(array('lang'=>$lang,"language_id"=>$this->get('id')));
         }   
         return $this->i18n;
     } 
     ...
     static function getAllI18nForSelect()
    {
        ...
        { 
            $values[$item->get('skill_id')]=new mfString($item->get('title'));
        }              
        return $values;
    }
    ...
    static function getAllForSelect()
    {
        ...
        $db=mfSiteDatabase::getInstance()
                ->setParameters(array("lang"=>mfcontext::getInstance()->getUser()->getLanguage()))
                ->setObjects(array('EmployeeSkillI18n','EmployeeSkill'))
                ->setQuery("SELECT {fields} FROM ".EmployeeSkill::getTable().
        ...
        {             
            $values[$items->getEmployeeSkill()->get('id')]=$items->getEmployeeSkill()->setI18n($items->getEmployeeSkillI18n());
        }              
        return $values;
    }
    
    static function getSkillsForSelect()
    {
        $values=new mfArray();        
        $db=mfSiteDatabase::getInstance()
                ->setParameters(array("lang"=>mfcontext::getInstance()->getUser()->getLanguage()))               
                ->setQuery("SELECT * FROM ".EmployeeSkillI18n::getTable().                           
                           " WHERE lang='{lang}' ORDER BY title ASC;")               
                ->makeSqlQuery(); 
        if (!$db->getNumRows())
            return $values;
        while ($item=$db->fetchObject('EmployeeSkillI18n'))
        {             
            $values[$item->get('skill_id')]=ucfirst($item->get('title'));
        }              
        return $values;
    } 
    ...
    {          
        ...
        $db=mfSiteDatabase::getInstance()               
                ...
                ->setQuery("SELECT {fields} FROM ".self::getTable(). 
                           " INNER JOIN ".EmployeeSkillI18n::getInnerForJoin('skill_id')." AND lang='{lang}'".
    ...
    {
        return mfConfig::get('mf_sites_dir')."/".$this->getSiteName()."/frontend/view/data/employees/skills/".$this->get('id');
    }
    ...
    {    
        return $this->_icon=$this->_icon===null?new PictureObject3(array(
                 ...
                 "urlPath"=>url("/data/employees/skills/".$this->get('id'),"web","frontend"),
                 "url"=>url("/data/employees/skills/".$this->get('id')."/".$this->get('icon'),"web","frontend"),
                 "urlAdmin"=>url("/nocache/data/employees/skills/".$this->get('id')."/".$this->get('icon'),"web","frontend")
              )):$this->_icon;   
    }               
    ...
    {    
        return $this->_picture=$this->_picture===null?new PictureObject3(array(
                 ...
                 "urlPath"=>url("/data/employees/skills/".$this->get('id')."/","web","frontend"),
                 "url"=>url("/data/employees/skills/".$this->get('id')."/".$this->get('picture'),"web","frontend"),
                 "urlAdmin"=>url("/nocache/data/employees/skills/".$this->get('id')."/".$this->get('picture'),"web","frontend")
              )):$this->_picture;   
    }               
   ...
   {
       return $this->formatter=$this->formatter===null?new EmployeeSkillFormatter($this):$this->formatter;
   }
