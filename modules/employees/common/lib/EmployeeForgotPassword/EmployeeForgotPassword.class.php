...
class EmployeeForgotPassword extends mfObject3 {
   ...
   function __construct($parameters=null) {
    ...
    protected function loadByUser(Employee $user)
    {
        if ($user->isNotLoaded())
            return ;
    ...
    function toArray()
    {
        ...
        $values['uri']=link_i18n('employees_forgot_password_change')."?k=".$this->get('key');
        return $values;
    }
    
    function createKeyAndPassword()
    {
        if ($this->getEmployee()->isNotLoaded())
            return $this;
        $this->cleanup();
