...
class EmployeeFunctionBase extends mfOrderedObject3 {
     
    protected static $fields=array('name','icon','color','position','created_at','updated_at');
    const table="t_employee_function";
    ...
    function hasI18n()
    {
     ...
     {       
         if ($this->i18n===null)
         {
             ...
             $this->i18n=new EmployeeFunctionI18n(array('lang'=>$lang,"function_id"=>$this->get('id')));
         }   
         return $this->i18n;
     } 
     ...
     static function getAllI18nForSelect()
    {
        ...
        $db=mfSiteDatabase::getInstance()
                ...
                ->setQuery("SELECT ".EmployeeFunctionI18n::getFieldsAndKeyWithTable()." FROM ".EmployeeFunctionI18n::getTable().
                           " WHERE lang='{lang}' ORDER BY title ASC;")               
                ->makeSqlQuery(); 
        if (!$db->getNumRows())
            return $values;
        while ($item=$db->fetchObject('EmployeeFunctionI18n'))
        { 
    ...
    static function getAllForSelect()
    {
        ...
        $db=mfSiteDatabase::getInstance()
                ->setParameters(array("lang"=>mfcontext::getInstance()->getUser()->getLanguage()))
                ->setObjects(array('EmployeeFunctionI18n','EmployeeFunction'))
                ->setQuery("SELECT {fields} FROM ".EmployeeFunction::getTable().
                           " INNER JOIN ".EmployeeFunctionI18n::getInnerForJoin('function_id').
                           " WHERE lang='{lang}' ORDER BY title ASC;")               
                ->makeSqlQuery(); 
        if (!$db->getNumRows())
            return $values;
        ...
        {             
            $values[$items->getEmployeeFunction()->get('id')]=$items->getEmployeeFunction()->setI18n($items->getEmployeeFunctionI18n());
        }              
        return $values;
    }
    ...
    static function getContractFunctionsForSelect()
    {
        ...
        $db=mfSiteDatabase::getInstance()
                ->setParameters(array("lang"=>mfcontext::getInstance()->getUser()->getLanguage()))               
                ->setQuery("SELECT * FROM ".EmployeeFunctionI18n::getTable().                           
                           " WHERE lang='{lang}' ORDER BY title ASC;")               
                ->makeSqlQuery(); 
        if (!$db->getNumRows())
            return $values;
    ...
    {          
        ...
        $db=mfSiteDatabase::getInstance()               
                ...
                ->setObjects(array('EmployeeFunction','EmployeeFunctionI18n'))
                ->setQuery("SELECT {fields} FROM ".self::getTable(). 
                           " INNER JOIN ".EmployeeFunctionI18n::getInnerForJoin('function_id')." AND lang='{lang}'".
       ...
       // var_dump($db->getQuery());
        ...
        {                  
           ...
           $item->setI18n($items->getEmployeeFunctionI18n());
   ...
   {
       return $this->formatter=$this->formatter===null?new EmployeeFunctionFormatter($this):$this->formatter;
   }
}
