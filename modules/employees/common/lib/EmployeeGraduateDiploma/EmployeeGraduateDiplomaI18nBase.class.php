...
class EmployeeGraduateDiplomaI18nBase extends mfObjectI18n {
     
    protected static $fields=array('title','graduate_id','lang','created_at','updated_at');
    const table="t_employee_graduate_diploma_i18n"; 
    protected static $foreignKeys=array('graduate_id'=>'EmployeeGraduateDiploma'); // By default
    ...
    function __construct($parameters=null) {
      ...
      {
          if (isset($parameters['lang']) && isset($parameters['graduate_id']))
              return $this->loadByLangAndGraduateDiplomaId((string)$parameters['lang'],(string)$parameters['graduate_id']); 
     ...
     protected function loadByLangAndGraduateDiplomaId($lang,$graduate_id)
    {
       $this->set('graduate_id',$graduate_id);
       $this->set('lang',$lang);
       $db=mfSiteDatabase::getInstance()           
            ->setParameters(array('graduate_id'=>$graduate_id,"lang"=>$lang))              
            ->setQuery("SELECT * FROM ".self::getTable().                      
                       " WHERE lang='{lang}' AND graduate_id={graduate_id};")
            ->makeSqlQuery();  
    ...
    {     
       $db=mfSiteDatabase::getInstance()           
            ...
            ->setQuery("SELECT ".self::getFieldsAndKeyWithTable()." FROM ".self::getTable().  
                       " LEFT JOIN ".self::getOuterForJoin('graduate_id').                      
                       " WHERE name='{name}';")
            ->makeSqlQuery();  
        return $this->rowtoObject($db);
    }
    ...
    {
        $db=mfSiteDatabase::getInstance()           
            ->setParameters(array("language_id"=>$this->get('graduate_id')))              
