        ...
        <div class="d-flex justify-content-between flex-wrap">
            <div class="d-flex align-items-end flex-wrap">
                ...
                <div class="d-flex">
                    ...
                    <p class="text-muted mb-0">&nbsp;/&nbsp;{__('My jobs')}</p>
                </div>
            </div>     
        </div>
        {component name="/site_help/help" help="employee-list-advert"}
    </div>    
</div>
{alerts}
                    ...
                    <tbody>
                        {foreach $pager as $item} 
                            <tr class="list" id="{$item->get('id')}" role="row">
                                ...
                                <td class="EmployeeAdvert" name="state" id="{$item->get('id')}">
                                    {$item->getStateI18n()}
                                </td>
                                <td>                                         
                                    ...
                                    &nbsp;
                                    {if !$item->hasCommittedAt()}
                                        <button type="button" class="btn btn-primary btn-sm Commit" id="{$item->get('id')}" title="{__('Publish')}"><i class="fa fa-check icon-sm" style="font-size: 1.00rem;"></i>{__('Publish')}</button>
                                        {/if}
                                </td>
                            </tr>
                            {foreach $item->getAdvertsI18n() as $i18n}                                                                   
                                <tr class="list-i18n" id="{$i18n->get('id')}" data-advert="{$item->get('id')}" role="row">
                                    ...
                                    <td class="EmployeeAdvertI18n" name="state" id="{$item->get('id')}">
                                        {if $i18n->hasPublishedAt()}
                                            {__('Approved')}
 ...
 <script type="text/javascript">
        ...
        {          
           return $.ajax2({ data: getFilterParameters(), 
                            url:"{url_to('employees_ajax',['action'=>'ListPartialAdvert'])}" ,                          
                            target: "#actions"
                             });
        }
      ...
      function updatePager(n)
        ...
        $(".pager").click(function () {                     
                return $.ajax2({ data: getFilterParameters(), 
                                 url:"{url_to('employees_ajax',['action'=>'ListPartialAdvert'])}?page="+$(this).attr('data-page'),                                
                                 target: "#actions"
                });
        });
        ...
        $(".Delete").click( function () { 
                ...
                return $.ajax2({ data :{ EmployeeAdvert: $(this).attr('id') },
                                 url :"{url_to('employees_ajax',['action'=>'DeleteAdvert'])}",                               
                                 success : function(resp) {
                                       if (resp.action!='DeleteAdvert') return ;                                          
         ...
         $(".DeleteI18n").click( function () { 
                ...
                return $.ajax2({ data :{ EmployeeAdvertI18n: $(this).attr('id') },
                                 url :"{url_to('employees_ajax',['action'=>'DeleteAdvertI18n'])}",                               
                                 success : function(resp) {
                                       if (resp.action!='DeleteAdvertI18n') return ;                                          
                                          $(".list-i18n[id="+resp.id+"]").remove();                                           
                                          $(".EmployeeAdvert[name=number_of_languages][id="+resp.advert_id+"]").html(resp.number_of_languages);
                                 }
                     });                                        
            });  
        ...
        $("#New").click( function () {                 
                return $.ajax2({ url :"{url_to('employees_ajax',['action'=>'NewAdvert'])}",                               
                                 target :"#actions"
                     });                                        
            });  
         ...
         $(".View").click( function () {                 
                return $.ajax2({ data :{ EmployeeAdvert: $(this).attr('id') },
                                 url :"{url_to('employees_ajax',['action'=>'ViewAdvert'])}",                               
                                 target :"#actions"
                     });                                        
            });  
        ...
        $('#Dashboard').click(function(){                                       
         ...
         $('.Commit').click(function(){                                       
          return $.ajax2({ data :{ EmployeeAdvert: $(this).attr('id') },
                           url: "{url_to('employees_ajax',['action'=>'CommitAdvert'])}",                           
                           success : function (resp)
                                {
                                    if (resp.action != 'CommitAdvert') return ;
                                    $(".Commit[id="+resp.id+"]").remove();
                                    $(".EmployeeAdvert[name=committed][id="+resp.id+"]").html(resp.committed_at);
                                    $(".EmployeeAdvert[name=state][id="+resp.id+"]").html(resp.state);
                                    $(".EmployeeAdvertI18n[name=state][id="+resp.id+"]").html(resp.state_i18n);
        ...
        $(".Display").click( function () {                 
                return $.ajax2({ data :{ EmployeeAdvert: $(this).attr('id') },
                                 url :"{url_to('employees_ajax',['action'=>'DisplayAdvert'])}",                               
                                 target :"#actions"
                     });                                        
            });
</script> 
