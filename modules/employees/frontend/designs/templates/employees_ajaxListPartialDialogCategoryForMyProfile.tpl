      ...
      <thead  class="{*bg-info*}thead-color">
        ...
        <th>#</th>
            ...
            <th><span class="mr-1">{__('Title')}</span>
                          <div class="d-inline-block">
                             <a href="#" class="dialog-order{$formFilter.order.title->getValueExist('asc','_active')}" id="asc" name="title"><i class="fa fa-sort-up acs-desc"></i></a>
                             <a href="#" class="dialog-order{$formFilter.order.title->getValueExist('desc','_active')}" id="desc" name="title"><i class="fa fa-sort-down"></i></a>            
                          </div>
            </th>                                             
...
<script type="text/javascript">
     ...
     function getDialogFilterParameters()
        {
            var params={ EmployeeCategory : { node: '{$node->get('id')}',                                           
        ...
        $(".dialog-pager").click(function () {                     
                return $.ajax2({ data: getDialogFilterParameters(), 
                                 url:"{url_to('employees_ajax',['action'=>'ListPartialDialogCategoryForMyProfile'])}?page="+$(this).attr('data-page'),                                
                                 target: "#Modal-Categories-ctn"
                });
        });
          
        $("#dialog-init").click(function () {                     
                return $.ajax2({ data : { EmployeeCategory : {                                              
                                              token : '{$formFilter->getCSRFToken()}' ,
                                              selection : $("#DialogCategoryList").data('selection')
                                         } },
                                 url:"{url_to('employees_ajax',['action'=>'ListPartialDialogCategoryForMyProfile'])}",
     ...
     $(".EmployeeWorkCategoryPath-item").click(function () { 
            $('.DropMenuShowFoldersInside').hide();
             ...
             return $.ajax2({ 
                         ...
                         url:"{url_to('employees_ajax',['action'=>'GetChildrenCategoryMyProfile'])}",                                              
