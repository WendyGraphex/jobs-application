...
<div class="NewClassAtTable bg-white px-2 py-2">   
    ...
    <div class="col-md-12  padd-col">
        ...
        <div class="float-right w-50">                   
            <button  style="float: right;" id="dialog-filter" class="btn btn-outline-primary btn-f" >{__("Filter")}</button>   
            <button  style="float: right;margin-right: 10px;" id="dialog-init" class="btn btn-outline-primary btn-init btn-f">{__("Init")}</button>
        </div>
    </div>
    ...
    <table id="DialogCategoryList" class="NewClassAtTable table table-bordered table-hover table-striped list-user"  role="grid" aria-describedby="Myliste_info">
...
<script type="text/javascript">
     ...
     function getDialogFilterParameters()
        {
            var params={ EmployeeAdvertCategory : { node: '{$node->get('id')}',                                           
        ...
        $(".dialog-pager").click(function () {                     
                return $.ajax2({ data: getDialogFilterParameters(), 
                                 url:"{url_to('employees_ajax',['action'=>'ListPartialDialogCategoryForAdvert'])}?page="+$(this).attr('data-page'),                                
                                 target: "#Modal-Categories-ctn"
                });
        });
          
        $("#dialog-init").click(function () {                     
                return $.ajax2({ data : { EmployeeAdvertCategory : {                                              
                                              token : '{$formFilter->getCSRFToken()}' ,
                                              selection : $("#DialogCategoryList").data('selection')
                                         } },
                                 url:"{url_to('employees_ajax',['action'=>'ListPartialDialogCategoryForAdvert'])}",
...
{*  $(".DropMenuShowFoldersInside").mouseleave(function(){
    ...
    $(".ProductCategoryPath-item").click(function () { 
         $('.DropMenuShowFoldersInside').hide();
             var item=$(this);
             return $.ajax2({ 
                         data : { ProductCategoryI18n : { category_id: $(this).attr('id'),lang : '{$formFilter->getLanguage()}' } },
                         url:"{url_to('products_ajax',['action'=>'GetChildrenDialog'])}",                                              
        ...
        $(".EmployeeWorkCategoryPath-item").click(function () { 
            $('.DropMenuShowFoldersInside').hide();
             var item=$(this);
             return $.ajax2({ 
                         data : { EmployeeWorkCategoryI18n : { category_id: $(this).attr('id'),lang : '{$formFilter->getLanguage()}' } },
                         url:"{url_to('employees_ajax',['action'=>'GetChildrenCategory'])}",                                              
                         success : function (resp)
                                {
                                       $(".DropMenuShowFoldersInside").css({
                                        position: "absolute",
    ...
    $(".EmployeeWorkCategory-item").click(function () {        
              var params={  EmployeeAdvertCategory : { node: $(this).attr('id'), 
