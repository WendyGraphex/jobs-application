        ...
        <div class="row w-100 mx-0">
  		...
  		<div class="form-group size center-block">
              ...
              {* <div class="form-group btnSize center-block text-center" style="display:none;">                        
                   <a href="{link_i18n('suppliers_contact')}" class="btn btn-warning btn-lg btn-perspective btn-block btn-pw-Contact">{__('Contact')}</a>
               </div> *}
             </div>        
          </div>        
        </div>  
...
<script type="text/javascript">
       {JqueryScriptsReady}
           ...
           $("#Send").click(function () { 
                            var params = { EmployeeForgotPasswordChange :  { 
                                        key: '{$forgot->get('key')}',
                                        token : "{mfForm::getToken('EmployeeForgotPasswordChangeForm')}"
                                    } };
                          $(".Fields.Input").each(function () { params.EmployeeForgotPasswordChange[$(this).attr('name')]=$(this).val(); });                            
                          //  alert('Params='+params.toSource());
                          $("#Send").button('loading');
                            return $.ajax2({ 
                                data : params,                              
                                url:"{url_to('employees_ajax',['action'=>'ForgotPasswordChange'])}",
                                 error : function (resp) {
                                     $("#Send").button('reset');
