...
<html lang="{$_request->getLanguage()}">
    <head>
        ...
        <title>{component name="/site_company/Display" field='name' COMMENT=false} - {__('All best employee skills')}</title>  
        ...
        <section class="ftco-section bg-light sectionCategoriesTop sectionMarginTop">
            <div id="page-ctn" class="container">      
                <div id="page-wrapper" class="allAdvetsInCities">  
                    <h1>{__('All best skills')}</h1>
                    {foreach $pager as $skill_employees}
                        <h3>{$skill_employees->getSkill()->getI18n()}</h3>              
                        <div class="advertsInCities">
                            <div class="row">
                                {foreach $skill_employees->getEmployees() as $item}                                  
                                    {component name="/employees/employeeDisplay" employee_user=$item}
                                {/foreach}    
                            </div>
                        </div>
                        {component name="/employees/skillEmployeesLink" skill=$skill_employees->getSkill()}                       
                    {/foreach}
                </div> 
            </div>
        </div> 
