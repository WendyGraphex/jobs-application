<div class="row RowTop">
    <div class="col-md-12 grid-margin classConducteurTop">
        ...
        {component name="/site_help/help" help="employee-view-job"}
    </div>
</div>
{component name="/site_warning/warning" warning="employee-view-job"}
            ...
            <div class="tab-pane fade show" id="documents" role="tabpanel" aria-labelledby="documents-tab">   
                ...
                <div class="row portfolio-grid">
                    ...
                    {foreach $advert->getDocuments() as $document}
                        <div class="EmployeeDocuments-list col-xl-2 col-lg-3 col-md-3 col-sm-6 col-12" id="{$document->get('id')}">   
                            <figure class="effect-text-in classBorderSolid">
                                <div class="div-img h-100 dz-image">
                                    ...
                                    {/if}
                                    <label class="PublicPrivate btn btn-success EmployeeAdvertDocumentPublic-Text" id="{$document->get('id')}" {if !$document->isOpened()}style="display:none"{/if}>{__('Public')}</label>
                                    <label class="PublicPrivate btn btn-danger EmployeeAdvertDocumentPrivate-Text" id="{$document->get('id')}" {if $document->isOpened()}style="display:none"{/if}>{__('Private')}</label>
                                    <div class="dropdown DropPublicPrivate">
                                        ...
                                        <ul class="dropdown-menu" style="min-width: 50px;padding: 10px;margin-top: 2px;">
                                            <li><a href="javascript:void();" class="EmployeeAdvertDocumentPublic-btn" {if $document->isOpened()}style="display:none"{/if} id="{$document->get('id')}">{__('Public')}</a></li>
                                            <li><a href="javascript:void();" class="EmployeeAdvertDocumentPrivate-btn" {if !$document->isOpened()}style="display:none"{/if} id="{$document->get('id')}">{__('Private')}</a></li>
                                        </ul>
                                    </div>  
                                </div>
...
<script type="text/javascript">                  
      ...
      $("#Save").click(function () { 
        var params = {  EmployeeAdvert : '{$advert->get('id')}',
                        EmployeeAdvertMultiple :  {  adverts : [ ], categories : [ ],token : '{$form->getCSRFToken()}' } };
       ...
       // console.log(params); return ;
        return $.ajax2({
            data : params,
            url: "{url_to('employees_ajax',['action'=>'SaveAdvert'])}",           
            target: "#actions"
        });
      ...
      $("#DialogCategories").click(function () { 
           var params = { EmployeeAdvertCategory : {   selection : [ ],  token : '{mfForm::getToken('EmployeePartnerWorkCategorySelectionForm')}' } };      
            $(".EmployeeAdvertCategories-list").each(function() { params.EmployeeAdvertCategory.selection.push($(this).attr('id'));  }); 
             return $.ajax2({      
                         data : params,
       ...
       $("#AddCategories").click(function () { 
            var params = { AddCategoriesForAdvert : {  selection: $("#DialogCategoryList").data('selection'), token : '{mfForm::getToken('EmployeeAddCategoriesForNewAdvertForm')}' } };           
            return $.ajax2({
                  data : params,
    ...
    {*$(".EmployeeDocuments-Delete").click(function () {            
     ...
     $(".EmployeeDocuments-Delete").click(function () {            
      ...
      $(".EmployeeAdvertDocumentPublic-btn").click(function () {                     
           return $.ajax2({      
                          data : { EmployeeAdvertDocument : $(this).attr('id') },
                         url:"{url_to('employees_ajax',['action'=>'PublicAdvertDocument'])}",                                              
                         success : function (resp) { 
                                if (resp.action !='PublicAdvertDocument') return ;
                                $(".EmployeeAdvertDocumentPrivate-btn[id="+resp.id+"]").show();
                                $(".EmployeeAdvertDocumentPublic-btn[id="+resp.id+"]").hide(); 
                                $(".EmployeeAdvertDocumentPrivate-Text[id="+resp.id+"]").hide();
                                $(".EmployeeAdvertDocumentPublic-Text[id="+resp.id+"]").show(); 
                            }                          
                       });                      
     });   
     
     $(".EmployeeAdvertDocumentPrivate-btn").click(function () {                     
           return $.ajax2({      
                          data : { EmployeeAdvertDocument : $(this).attr('id') },
                         url:"{url_to('employees_ajax',['action'=>'PrivateAdvertDocument'])}",                                              
                         success : function (resp) { 
                                if (resp.action !='PrivateAdvertDocument') return ;
                                $(".EmployeeAdvertDocumentPrivate-btn[id="+resp.id+"]").hide();
                                $(".EmployeeAdvertDocumentPublic-btn[id="+resp.id+"]").show(); 
                                $(".EmployeeAdvertDocumentPrivate-Text[id="+resp.id+"]").show();
                                $(".EmployeeAdvertDocumentPublic-Text[id="+resp.id+"]").hide();    
                            }                          
                       });                      
     });   
</script>           
