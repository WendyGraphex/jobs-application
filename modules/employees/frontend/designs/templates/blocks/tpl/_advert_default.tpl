     ...
     {* ==={$item_i18n->get('id')}==={$item_i18n->getAdvert()->get('id')}===  *}
            <div class="blog-entry align-self-stretch">
                <div class="ViewsQuotes">
                    <label class="AdViews"><i class="mdi mdi-cart"></i>{$item_i18n->getFormatter()->getNumberOfQuotations()->getChoicesText('[0]no quote|[1]one quote|(1,Inf]%s quotes')}</label>
                    ...
                    {else}
                         {foreach $item_i18n->getAdvert()->getDocuments() as $document}
                          <div class="item">
                            ...
                            {*<p id="copyMe-{$item_i18n->get('id')}" style="display: none;">{$item_i18n->getUrl()}</p>
                            <input class="copied-{$item_i18n->get('id')}" type="text" id="Link" aria-hidden="true" style="position: absolute;left: -999em;"/>*}
                          </div>
                        {/foreach} 
                    {/if}    
                </div>  
                ...
                <div class="seller-info row">
                    <div class="col-md-6 AdvertDate">
                        <span class="AdDate">{$item_i18n->getFormatter()->getPublishedAt()->getDateFormatter()->getText('P')}</span>
                    </div>
                    <div class="col-md-6 nameAndContact">
                        <div class="ImageTitle">
                                <span class="seller-image">
                                    <span class="is-online" {if $item_i18n->getAdvert()->getEmployeeUser()->isOnline()}style="background: #1dbf73;"{/if} style="background: #62646a;"></span>
                                    <a href="{$item_i18n->getAdvert()->getEmployeeUser()->getUrl()}" class="spanImage">
                            ...
                            <div class="nameAndDate" style="padding-left: 42px;">
                                <div class="UserNameAndSecond">
                                    <a href="{$item_i18n->getAdvert()->getEmployeeUser()->getUrl()}" class="UserName" style="display: inline;">
                                        {$item_i18n->getAdvert()->getEmployeeUser()->getFormatter()->getUsername()->ucfirst()}
                                    </a>
                                     {component name="/employees_messager/sendBtn" item_i18n=$item_i18n}                                                                        
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="UserInfo">         
                    ...
                    <div class="UserVotes">
                       ...
                       {component name="/employees_contact/abuseBtn" item_i18n=$item_i18n class='abuse-user'}    
                    </div>
                    <div class="wsihAndStarting startingAndPrice row">
                        ...
                        <span class="starting starting-price" style="float:right;">{__('Starting at')} <label class="workPrice">{$item_i18n->getAdvert()->getFormatter()->getBudget()->getAmount()}</label></span>
                        </div>
                    </div>
                    <div class="social-network-sharing">
                        ...
                        <label class="social-network">
                            {component name="/facebook_share/link" text=$item_i18n->getFormatter()->getTitle() url=$item_i18n->getUrl()}
                                ...
                                {component name="/employees_contact/shareBtn" item_i18n=$item_i18n}
                                    ...
                                    <div class="modal-dialog">
                                      <div class="modal-content">
                                        ...
                                        <div class="modal-body" style="text-align:center;color: #109310;">
                                            <i class="mdi mdi-check-circle" style="font-size: 45px;"></i><p style="font-size: 19px;">{__('Your Message has been sent')}</p>
                                        </div>
                                      </div>
                                    </div>
                                </div>*}
                       ...
                       <button class="BtnOfferQuote btn btn-primary EmployeeAdvertQuote" id="{$item_i18n->get('id')}">{__('Get a quote')}</button>                       
                    </div>
                </div>
                {component name="/employees_contact/abuseBtn" item_i18n=$item_i18n class='abuse-user-second'}    
