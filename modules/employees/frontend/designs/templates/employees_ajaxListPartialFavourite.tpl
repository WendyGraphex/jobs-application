      ...
      <div class="d-flex justify-content-between flex-wrap">
        <div class="d-flex align-items-end flex-wrap">
          ...
          <div class="d-flex">
            ...
            <p class="text-primary mb-0 hover-cursor">{__('My favorites')}</p>
          </div>
        </div>      
      </div>
          {component name="/site_help/help" help="employee-list-favourite"}
    </div>
</div>
 ...
 <script type="text/javascript">
        ...
        {          
           return $.ajax2({ data: getFilterParameters(), 
                            url:"{url_to('employees_ajax',['action'=>'ListPartialFavourite'])}" ,                          
                            target: "#actions"
                             });
        }
        ...
        $(".pager").click(function () {                     
                return $.ajax2({ data: getFilterParameters(), 
                                 url:"{url_to('employees_ajax',['action'=>'ListPartialFavourite'])}?page="+$(this).attr('data-page'),                                
                                 target: "#actions"
                });
        });
        
        $(".Delete").click( function () { 
                if (!confirm('{__("Favourite \"#0#\" will be deleted. Confirm ?")}'.format($(this).attr('name')))) return false; 
                return $.ajax2({ data :{ EmployeeFavourite: $(this).attr('id') },
                                 url :"{url_to('employees_ajax',['action'=>'DeleteFavourite'])}",                               
                                 success : function(resp) {
                                       if (resp.action!='DeleteFavourite') return ;                                          
