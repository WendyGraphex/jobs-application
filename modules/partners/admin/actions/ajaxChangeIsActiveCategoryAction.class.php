...
class partners_ajaxChangeIsActiveCategoryAction extends mfAction
{
    ...
    function execute(mfWebRequest $request) {   
      ...
      {        
        $form=new ChangeForm();
        $form->bind($request->getPostParameter('PartnerWorkCategory'));
        if (!$form->isValid())
        {
           // var_dump($form->getErrorSchema()->getErrorsMessage());
             throw new mfException(__('Form has some errors'));    
        }
        $item= new PartnerWorkCategory($form->getValue('id'));    
        if ($item->isNotLoaded())
