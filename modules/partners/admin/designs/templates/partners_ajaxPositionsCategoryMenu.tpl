...
<script type="text/javascript">
     $("#Menus tbody").sortable({
        ...
        stop: function (event, ui) {
           ...
           var params = {
                        PartnerWorkCategoryMenuPositions: {
                            node : ui.item.attr('id'),
                            sibling_id : ui.item.next().attr('id'),
                            token: '{mfForm::getToken('PartnerWorkCategoryMenuMoveForm')}'
                        }
               };
             $.ajax2({
            data: params,
            url: "{url_to('partners_ajax',['action'=>'MoveCategoryMenu'])}"         
        });
        },
    ...
    $("#Cancel").click(function () {
        return $.ajax2({
            data : { PartnerWorkCategoryMenuNode : { node: '{$item_i18n->get('menu_id')}', lang: '{$item_i18n->get('lang')}',token : '{mfForm::getToken('PartnerWorkCategoryMenuNodeForm')}'  } },                                
