<div class="row admin-header-container">
   <div class="col-sm">
     ...
     <h3 class="admin-header-small-text">{__('Category management')}</h3>
   </div>
   <div class="col-sm" style="padding: 0;">
       <div style="float: right;"> 
         <a class="btn btn-success admin-header-button btn-color"  href="#" id="New"><i class="fa fa-plus"></i> {__('New Category')}</a>
           ...
           <a class="btn btn-warning admin-header-button" href="{url_to('partners',['action'=>'ExportCsv'])}?{$formFilter->getParametersForUrl(['equal','in','begin','search','range'])}"><i class="fa fa-file-excel"></i> {__('Export XLS')}</a> 
       </div> 
   </div>
</div>
...
{* ============================================ METHODS ============================================================== *}
  ...
  var updated_at_dates = $( ".Range.updatedAtDate#updated_at_from, .Range.updatedAtDate#updated_at_to" ).datepicker({
     ...
     function getFilterParameters()
        {
            var params={ PartnerWorkCategoryNode : { node: '{$node->get('id')}', 
                                             lang : $("#SelectListLanguages option:selected").attr('id'), 
                                             token : '{mfForm::getToken('PartnerWorkCategoryNodeForm')}' },
                          filter: {  order : { }, 
                                    search : { },
                                    equal: {
        ...
        $(".pager").click(function () {                     
                return $.ajax2({ data: getFilterParameters(), 
                                 url:"{url_to('partners_ajax',['action'=>'ListPartialCategory'])}?page="+$(this).attr('data-page'),                                
                                 target: "#actions"
                });
        });
      ...
      $(".View").click( function () {                    
                  return $.ajax2({  data : { PartnerWorkCategoryI18n : { 
                                                category_id: $(this).attr('id'),
                                                lang: $("#SelectListLanguages option:selected").attr('id')                                            
                                    } },                            
                                  url:"{url_to('partners_ajax',['action'=>'ViewCategoryI18n'])}", 
                                  target: "#actions"});
          });
    
      $("#New").click( function () {             
                return $.ajax2({      
                    data : { PartnerWorkCategoryNode : { node : '{$node->get('id')}', 
                                                 lang: $("#SelectListLanguages option:selected").attr('id'),
                                                 token: "{mfForm::getToken('PartnerWorkCategoryNodeForm')}" 
                                               } },                
                    url: "{url_to('partners_ajax',['action'=>'NewCategoryI18n'])}",               
                    target: "#actions"
              });
            });
     ...
     $(".Delete").click( function () { 
                ...
                return $.ajax2({ data :{ Category: $(this).attr('id') },
                                 url :"{url_to('partners_ajax',['action'=>'DeleteCategory'])}",                               
    ...
    $(".PartnerWorkCategory-item").click(function () { 
    ...
    $(".PartnerWorkCategoryPath-item").click(function () { 
         $('.DropMenuShowFoldersInside').hide();
             var item=$(this);
             return $.ajax2({ 
                         data : { PartnerWorkCategoryI18n : { category_id: $(this).attr('id'),lang : '{$formFilter->getLanguage()}' } },
                         url:"{url_to('partners_ajax',['action'=>'GetChildrenCategory'])}",                                              
                         success : function (resp)
                                {
       ...
       $("#Positions").click( function () {                
                return $.ajax2({  data : { PartnerWorkCategoryI18n : { category_id: '{$node->get('id')}', lang: $("#SelectListLanguages option:selected").attr('id') } }, 
                                url:"{url_to('partners_ajax',['action'=>'PositionsCategory'])}",
                                target: "#actions"});
         }); 
         
         $(".ChangeIsActive").click(function () {
            return $.ajax2({
                 data : { PartnerWorkCategory : { id: $(this).attr('id') , value:$(this).attr('name'),token :'{mfForm::getToken('ChangeForm')}' } },                  
                url: "{url_to('partners_ajax',['action'=>'ChangeIsActiveCategory'])}",              
                success: function (resp) {
