<div class="row admin-header-container">
   <div class="col-sm">
     ...
     <h3 class="admin-header-small-text">{__('Category menu management')}</h3>
   </div>
         ...
         {* <a class="btn btn-warning admin-header-button" href="#"><i class="fa fa-file-excel"></i> {__('Export XLS')}</a> *}
          ...
          <div class="breadcrumb-title">
            <p>                
                ...
                <i class="fa fa-angle-right" style="margin: 0 5px 0 5px; font-size: 1em;"></i>{__('Partner Works')}
                <i class="fa fa-angle-right" style="margin: 0 5px 0 5px; font-size: 1em;"></i>{__('Category Menu')}
            </p>
          </div>
          <!-- End breadcrumb -->
{alerts}
...
<div class="bg-white px-2 py-2">   
    <div class="row">       
     {component name="/partners/pathMenu"}  
    </div>
     <div>                 
      ...
      <thead  class="bgSpec">
        <tr role="row">
            ...
            <th><span class="mr-1">{__('Title')}</span>
             ...
             <th><span class="mr-1">{__('Category')}</span>
...
{* ============================================ METHODS ============================================================== *}
  ...
  var updated_at_dates = $( ".Range.updatedAtDate#updated_at_from, .Range.updatedAtDate#updated_at_to" ).datepicker({
     ...
     function getFilterParameters()
        {
            var params={ PartnerWorkCategoryMenuNode : { node: '{$node->get('id')}', 
                                             lang : $("#SelectListLanguages option:selected").attr('id'), 
                                             token : '{mfForm::getToken('PartnerWorkCategoryMenuNodeForm')}' },
                          filter: {  order : { }, 
                                    search : { },
                                    equal: {
        ...
        $(".pager").click(function () {                     
                return $.ajax2({ data: getFilterParameters(), 
                                 url:"{url_to('partners_ajax',['action'=>'ListPartialCategoryMenu'])}?page="+$(this).attr('data-page'),                                
                                 target: "#actions"
                });
        });
          
        $("#init").click(function () {                     
                return $.ajax2({ data : { PartnerWorkCategoryMenuNode : { node: '{$node->get('id')}', 
                                             lang : $("#SelectListLanguages option:selected").attr('id'), 
                                             token : '{mfForm::getToken('PartnerWorkCategoryMenuNodeForm')}' } },
                                 url:"{url_to('partners_ajax',['action'=>'ListPartialCategoryMenu'])}",
                                 target: "#actions"
      ...
      $(".View").click( function () {                    
                  return $.ajax2({  data : { PartnerWorkCategoryMenuI18n : { 
                                                menu_id: $(this).attr('id'),
                                                lang: $("#SelectListLanguages option:selected").attr('id')                                            
                                    } },                            
                                  url:"{url_to('partners_ajax',['action'=>'ViewCategoryMenuI18n'])}", 
                                  target: "#actions"});
          });
    
      $("#New").click( function () {             
                return $.ajax2({      
                    data : { PartnerWorkCategoryMenuNode : { node : '{$node->get('id')}', 
                                                 lang : $("#SelectListLanguages option:selected").attr('id'),
                                                 token: "{mfForm::getToken('PartnerWorkCategoryMenuNodeForm')}" 
                                               } },                
                    url: "{url_to('partners_ajax',['action'=>'NewCategoryMenuI18n'])}",               
                    target: "#actions"
              });
            });
         
     $(".Delete").click( function () { 
                if (!confirm('{__("Category Menu \"#0#\" will be deleted. Confirm ?")}'.format($(this).attr('name')))) return false; 
                return $.ajax2({ data :{ Category: $(this).attr('id') },
                                 url :"{url_to('partners_ajax',['action'=>'DeleteCategoryMenu'])}",                               
                                 success : function(resp) {
                                     alert(resp);
                                       if (resp.action!='DeleteCategoryMenu') return ;                                          
     ...
     $(".Menus").click( function () {                    
                  return $.ajax2({  data : { PartnerWorkCategoryMenuNode : { 
                                                node: $(this).attr('id'),
                                                lang: $("#SelectListLanguages option:selected").attr('id'),
                                                token : '{mfForm::getToken('PartnerWorkCategoryMenuNodeForm')}'
                                    } },                            
    ...
    $(".PartnerWorkCategoryMenu-item").click(function () { 
    ...
    $(".PartnerWorkCategoryMenuPath-item").click(function () { 
         $('.DropMenuShowFoldersInside').hide();
             var item=$(this);
             return $.ajax2({ 
                         data : { PartnerWorkCategoryMenuI18n : { menu_id: $(this).attr('id'),lang : '{$formFilter->getLanguage()}' } },
                         url:"{url_to('partners_ajax',['action'=>'GetChildrenCategoryMenu'])}",                                              
                         success : function (resp)
                                {
       ...
       $("#Positions").click( function () {                
                return $.ajax2({  data : { PartnerWorkCategoryMenuI18n : { menu_id: '{$node->get('id')}', lang: $("#SelectListLanguages option:selected").attr('id') } }, 
                                url:"{url_to('partners_ajax',['action'=>'PositionsCategoryMenu'])}",
                                target: "#actions"});
         }); 
         
         $(".ChangeIsActive").click(function () {
            return $.ajax2({
                 data : { PartnerWorkCategoryMenu : { id: $(this).attr('id') , value:$(this).attr('name'),token :'{mfForm::getToken('ChangeForm')}' } },                  
                url: "{url_to('partners_ajax',['action'=>'ChangeIsActiveCategoryMenu'])}",              
                success: function (resp) {
                    if (resp.action != 'ChangeIsActiveCategoryMenu') return ;
