...
<div class="row admin-header-container">
    ...
    <div class="col-sm" style="padding: 0;">
        <div style="float: right;"> 
            <a class="btn btn-warning admin-header-button" href="{url_to('employees_invitation',['action'=>'ExportCsv'])}?{$formFilter->getParametersForUrl(['equal','in','begin','search','range'])}"><i class="fa fa-file-excel"></i> {__('Export XLS')}</a>
     ...
     function getFilterParameters()
        ...
        {          
           return $.ajax2({ data: getFilterParameters(), 
                            url:"{url_to('employees_invitation_ajax',['action'=>'ListPartialInvitation'])}" ,                          
                            target: "#actions"
                             });
        }
        ...
        $(".pager").click(function () {                     
                return $.ajax2({ data: getFilterParameters(), 
                                 url:"{url_to('employees_invitation_ajax',['action'=>'ListPartialInvitation'])}?page="+$(this).attr('data-page'),                                
                                 target: "#actions"
                });
        });
        
         $("#init").click(function () {                     
                return $.ajax2({   url:"{url_to('employees_invitation_ajax',['action'=>'ListPartialInvitation'])}",
                                 target: "#actions"
                });
        });
...
{* =================================== ACTIONS ======================================================== *}
      ...
      $("#Clear").click( function () { 
                if (!confirm('{__("Employee invitation database will be cleared. Confirm ?")}')) return false; 
                return $.ajax2({ data :{ EmployeeInvitation: $(this).attr('id') },
                                 url :"{url_to('employees_invitation_ajax',['action'=>'ClearInvitation'])}",                               
                                 target: "#actions" 
                     });                                        
            });   
     ...
     $(".Delete").click( function () { 
                if (!confirm('{__("Employee invitation \"#0#\" will be deleted. Confirm ?")}'.format($(this).attr('name')))) return false; 
                return $.ajax2({ data :{ EmployeeInvitation: $(this).attr('id') },
                                 url :"{url_to('employees_invitation_ajax',['action'=>'DeleteInvitation'])}",                               
