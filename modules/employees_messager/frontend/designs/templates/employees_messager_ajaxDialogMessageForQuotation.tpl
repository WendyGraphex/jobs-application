...
<script type="text/javascript">                    
        ...
        $("#Send").click(function () { 
            var params = { EmployeeMessage : { quotation : '{$item->get('id')}', id : $("#Message").attr('data-id'), token :'{mfForm::getToken('SendMessageForQuotationForm')}' } };
            $(".CustomerEmployeeMessage.Input").each(function () { params.EmployeeMessage[$(this).attr('name')]=$(this).val(); });
            return $.ajax2({      data :params,
                                  url :"{url_to('employees_messager_ajax',['action'=>'SendMessageForQuotation'])}",                                                                
                                  success: function (resp) 
                                        { 
                                            if (resp.action !='SendMessageForQuotation') return ;
                                            if (resp.errors)
                                            {
                                            ...
                                            $("#ModalQuotationSuccess").modal('show');
                                            $(".NameUserQuotation").html("{$item->getEmployerUser()->getFormatter()->getUsername()->ucfirst()}");
                                        }
                     });   
        });
</script> 