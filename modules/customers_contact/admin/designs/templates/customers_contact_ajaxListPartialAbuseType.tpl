...
<div class="row admin-header-container">
    <div class="col-sm">
           ...
           <h3 class="admin-header-small-text">{__('Manage Abuse Types of customers')}</h3>
    </div>
    ...
    var updated_dates = $( " .Range.UpdatedAtDate#updated_at_from, .Range.UpdatedAtDate#updated_at_to" ).datepicker({
        ...
        function updatePager(n)
        {
          ...
          {* =====================  P A G E R  A C T I O N S =============================== *}  
           ...
           $("#init").click(function() {                   
               return $.ajax2({ url:"{url_to('customers_contact_ajax',['action'=>'ListPartialAbuseType'])}",                                    
                         target: "#actions"}); 
           }); 
          ...
          $("[name=nbitemsbypage]").change(function() { return updateFilter(); }); 
           ...
           $(".pager").click(function () {                      
                return $.ajax2({ data: getFilterParameters(), 
                                 url:"{url_to('customers_contact_ajax',['action'=>'ListPartialAbuseType'])}?"+this.href.substring(this.href.indexOf("?")+1, this.href.length),
                                target: "#actions"
                });
        });
          {* =====================  A C T I O N S =============================== *}  
          ...
          $("#New").click( function () {             
            return $.ajax2({
                ...
                url: "{url_to('customers_contact_ajax',['action'=>'NewAbuseTypeI18n'])}",              
                target: "#actions"
           });
         });
         
         $(".View").click( function () {                
                return $.ajax2({ data : { CustomerAbuseTypeI18n : { 
                                                abuse_id: $(this).attr('id'),
                                                lang: $("#SelectListLanguages option:selected").attr('id')                                            
                                    } },                                
                                url:"{url_to('customers_contact_ajax',['action'=>'ViewAbuseTypeI18n'])}",                              
                                target: "#actions"});
         });
         
         
       $(".list").dblclick( function () {                    
                  return $.ajax2({ data : { CustomerAbuseTypeI18n : { abuse_id: $(this).attr('id'),lang: $("#SelectListLanguages option:selected").attr('id') } },                               
                                  url:"{url_to('customers_contact_ajax',['action'=>'ViewAbuseTypeI18n'])}", 
                                  target: "#actions"});
          });
        ...
        $(".Delete").click( function () { 
                if (!confirm('{__("Customer Abuse Type \"#0#\" will be deleted. Confirm ?")}'.format($(this).attr('name')))) return false; 
                return $.ajax2({ data :{ CustomerAbuseType: $(this).attr('id') },
                                 url :"{url_to('customers_contact_ajax',['action'=>'DeleteAbuseType'])}",                                
                                 success : function(resp) {
                                          if (resp.action != 'DeleteAbuseType')  return ;                                                                              
        ...
        $("#Positions").click( function () {                
                return $.ajax2({ url:"{url_to('customers_contact_ajax',['action'=>'PositionsAbuseType'])}",
                                target: "#actions"});
         });   
