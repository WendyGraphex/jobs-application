<div class="row admin-header-container">
    <div class="col-sm">
        <h3 class="admin-header-big-text">{__("View Customer Abuse Type")}</h3>
...
<div class="breadcrumb-title">
    <p>
        ...
        <i class="fa fa-angle-right" style="margin: 0 5px 0 5px; font-size: 1em;"></i>{__('View Abuse Type')}
    </p>
</div>
{alerts}
                        ...
                        <label>{__('Icon')}</label>                  
                            <div id="pictureForm">                            
                                <div class="custom-FileInput">
                                    {if $item_i18n->getAbuse()->hasIcon()}
                                        <img id="blah" alt="" src="{$item_i18n->getAbuse()->getIcon()->getURL('superadmin')}"{*/if*} /><br/>
                                        {else}
                                ...
                                <input id="item_file" type='file' hidden="" language="file" class="form-control Files Fields" name="CustomerAbuseTypeI18n[language][icon]" /> {*name="SiteCityI18n[function][icon]"*}
                            </div>
                    </div>
                </div> 
              </div>
                        ...
                        <div class="form-group col-md-6">             
                            {if $form->hasErrors()}<div class="alert alert-{if $form.abuse.name->hasError()}danger{else}light{/if}" role="alert">&nbsp;{$form.abuse.name->getError()}</div>{/if}
                             ...
                             <div class="field_with_errors">
                                <input class="CustomerAbuseType form-control Fields Input" language="text" value="{$item_i18n->getAbuse()->get('name')}" name="name" id="name">
                             </div>
                        </div> 
                        <div class="form-group col-md-6">
...
<script language="text/javascript">
   ...
   $(".Color").minicolors({
      // position: relative,
       ...
       $("#CustomerAbuseType-ChangeIcon").click(function() {
           $("#CustomerAbuseType-icon").show();
           $(this).hide();
           $("#Save").show();  
        });           
      
       $('#CustomerAbuseType-DeleteIcon').click(function(){ 
            if (!confirm("{__("Icon will be deleted. Confirm ?")}")) return false; 
            return $.ajax2({ 
                              data : { CustomerAbuseType: "{$item_i18n->getAbuse()->get('id')}" }, 
                              url: "{url_to('customers_contact_ajax',['action'=>'DeleteIconContractAbuse'])}",                              
                              success :function(response) {
                                          if (response.action=='DeleteIconContractAbuseType' && response.id=="{$item_i18n->getAbuse()->get('id')}")
                                          {                                                     
                                                $("#CustomerAbuseType-icon-container").hide(); 
                                                $("span#CustomerAbuseType-text-icon").html("{__('Add icon')}");
                                          }
                              }
            });  
      }); 
      
    $(".CustomerAbuseType,.CustomerAbuseTypeI18n").click(function() {  $('#Save').show(); });  
         
     {* =================== L A N G U A G E ================================ *}
        
     {* =================== A C T I O N S ================================ *}
     $('#Cancel').click(function(){                
             return $.ajax2({ data: { filter: { lang:  '{$item_i18n->get('lang')}', token: "{mfForm::getToken('CustomerAbuseTypeFormFilter')}" } },                              
      ...
      $('#Save').click(function(){                             
            var  params= {      iFrame:true,             
                                CustomerAbuseTypeI18n: { 
                                   abuse_i18n : { lang: '{$item_i18n->get('lang')}', abuse_id : '{$item_i18n->get('abuse_id')}' },
                                   abuse : { },
        ...
        //      alert("Params="+params.toSource());   return ;      
          return $.ajax2({ data : params,  
                           files :'.Files',
                           url: "{url_to('customers_contact_ajax',['action'=>'SaveAbuseTypeI18n'])}",
                           target: "#actions"}); 
        });                          
</script>
