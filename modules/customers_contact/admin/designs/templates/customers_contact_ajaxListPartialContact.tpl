          ...
          <div class="row admin-header-container">
            ...
            <div class="col-sm" style="padding: 0;">
                <div style="float: right;"> 
                  <a class="btn btn-warning admin-header-button" href="{url_to('customers_contact',['action'=>'ExportCsv'])}?{$formFilter->getParametersForUrl(['equal','in','begin','search','range'])}"><i class="fa fa-file-excel"></i> {__('Export XLS')}</a>
                </div> 
            </div>
          </div>
     ...
     function getFilterParameters()
        ...
        {          
           return $.ajax2({ data: getFilterParameters(), 
                            url:"{url_to('customers_contact_ajax',['action'=>'ListPartialContact'])}" ,                          
                            target: "#actions"
                             });
        }
        ...
        $(".pager").click(function () {                     
                return $.ajax2({ data: getFilterParameters(), 
                                 url:"{url_to('customers_contact_ajax',['action'=>'ListPartialContact'])}?page="+$(this).attr('data-page'),                                
                                 target: "#actions"
                });
        });
        
         $("#init").click(function () {                     
                return $.ajax2({   url:"{url_to('customers_contact_ajax',['action'=>'ListPartialContact'])}",
                                 target: "#actions"
                });
        });
...
{* =================================== ACTIONS ======================================================== *}
      ...
      $(".View").click( function () {                    
                  return $.ajax2({ data : { Customer : $(this).attr('id') },                               
                                  url:"{url_to('customers_contact_ajax',['action'=>'ViewCustomer'])}", 
     ...
     $(".Delete").click( function () { 
                ...
                return $.ajax2({ data :{ CustomerContact: $(this).attr('id') },
                                 url :"{url_to('customers_contact_ajax',['action'=>'DeleteCustomerContact'])}",                               
                                 success : function(resp) {
                                       if (resp.action!='DeleteCustomerContact') return ;                                          
