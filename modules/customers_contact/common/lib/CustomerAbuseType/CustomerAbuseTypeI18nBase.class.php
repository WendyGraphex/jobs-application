...
class CustomerAbuseTypeI18nBase extends mfObjectI18n {
     
    protected static $fields=array('title','abuse_id','lang','created_at','updated_at');
    const table="t_customer_abuse_type_i18n"; 
    protected static $foreignKeys=array('abuse_id'=>'CustomerAbuseType'); // By default
    ...
    function __construct($parameters=null) {
      ...
      {
          if (isset($parameters['lang']) && isset($parameters['abuse_id']))
              return $this->loadByLangAndAbuseTypeId((string)$parameters['lang'],(string)$parameters['abuse_id']); 
     ...
     protected function loadByLangAndAbuseTypeId($lang,$language_id)
    {
       $this->set('abuse_id',$language_id);
       $this->set('lang',$lang);
       $db=mfSiteDatabase::getInstance()           
            ->setParameters(array('abuse_id'=>$language_id,"lang"=>$lang))              
            ->setQuery("SELECT * FROM ".self::getTable().                      
                       " WHERE lang='{lang}' AND abuse_id={abuse_id};")
            ->makeSqlQuery();  
    ...
    {     
       $db=mfSiteDatabase::getInstance()           
            ...
            ->setQuery("SELECT ".self::getFieldsAndKeyWithTable()." FROM ".self::getTable().  
                       " LEFT JOIN ".self::getOuterForJoin('abuse_id').                      
                       " WHERE name='{name}';")
            ->makeSqlQuery();  
        return $this->rowtoObject($db);
    }
    ...
    {
        $db=mfSiteDatabase::getInstance()           
            ->setParameters(array("language_id"=>$this->get('abuse_id')))              
      ...
      function delete()
    {
        if (parent::delete()===false)       
            return $this;
        if (!$this->hasSibbling())
            $this->getAbuse()->delete();
        return $this;
    }  
   
     function getAbuse()
    {
       return $this->_abuse_id=$this->_abuse_id===null?new CustomerAbuseType($this->get('abuse_id')):$this->_abuse_id;                 
    }    
