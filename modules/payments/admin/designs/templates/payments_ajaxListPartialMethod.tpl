...
{alerts}
      ...
      <tr class="list" id="{$item->get('id')}" role="row">
          ...
          <td>{$item->get('id')}</td>        
         {* <td>{$item->getI18n()->ucfirst()}</td>*}  
         <td>{$item->getFormatter()->getI18n()->ucfirst()}</td>
           <td class="align-middle text-center"> <a href="#" class="ChangeIsActive" name="{$item->get('is_active')}" id="{$item->get('id')}"><i class="fa fa-circle text-{if $item->isActive()}success{else}danger{/if}"></i></a>
            ...
            <td>            
              <a href="#" class="list-action Text" id="{$item->get('id')}" title="{__('Texts')}"><i class="fa fa-font"></i></a>
     ...
     function getFilterParameters()
        ...
        {                   
           return $.ajax2({ data: getFilterParameters(), 
                            url:"{url_to('payments_ajax',['action'=>'ListPartialMethod'])}" ,                          
                            target: "#actions"
                             });
        }
        
        $("#init").click(function() {                   
            $.ajax2({ url:"{url_to('payments_ajax',['action'=>'ListPartialMethod'])}",                                    
                      target: "#actions"}); 
        }); 
       ...
       $('.order').click(function() {           
        ...
        $(".pager").click(function () {                     
                return $.ajax2({ data: getFilterParameters(), 
                                 url:"{url_to('payments_ajax',['action'=>'ListPartialMethod'])}?page="+$(this).attr('data-page'),                                
                                 target: "#actions"
                });
        });
      ...
      $(".View").click( function () {                    
                  return $.ajax2({ data : { PaymentMethod : $(this).attr('id') },                               
                                  url:"{url_to('payments_ajax',['action'=>'ViewMethod'])}", 
                                  target: "#actions"});
          });
    
      $("#New").click( function () {             
                return $.ajax2({                    
                    url: "{url_to('payments_ajax',['action'=>'NewMethod'])}",               
                    target: "#actions"
              });
            });
         
     $(".Delete").click( function () { 
                if (!confirm('{__("Method \"#0#\" will be deleted. Confirm ?")}'.format($(this).attr('name')))) return false; 
                return $.ajax2({ data :{ PaymentMethod: $(this).attr('id') },
                                 url :"{url_to('payments_ajax',['action'=>'DeleteMethod'])}",                               
                                 success : function(resp) {
                                       if (resp.action!='DeleteMethod') return ;                                          
        ...
        $(".ChangeIsActive").click(function () {
            return $.ajax2({
                 data : { PaymentMethod : { id: $(this).attr('id') , value:$(this).attr('name'),token :'{mfForm::getToken('ChangeForm')}' } },                  
                url: "{url_to('payments_ajax',['action'=>'ChangeIsActivePayment'])}",              
                success: function (resp) {
                    if (resp.action != 'ChangeIsActivePayment') return ;
