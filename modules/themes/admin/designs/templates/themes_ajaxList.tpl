...
<script type="text/javascript">   
 function changeThemeIsActiveState(resp) 
        ...
        {
           return $.ajax2({ data: getThemesFilterParameters(), url:"{url_to('themes_admin_ajax',['action'=>'List'])}" , target: "#middle" });      
        }
        ...
        $(".pager").click(function () {
            return $.ajax2({ data: getThemesFilterParameters(), url:"{url_to('themes_admin_ajax',['action'=>'List'])}?"+this.href.substring(this.href.indexOf("?")+1, this.href.length),target: "#middle"});
        });
        
        $("#init").click(function() { 
            $.ajax2({ url:"{url_to('themes_admin_ajax',['action'=>'List'])}",target: "#middle"});
        });
        ...
        $(".ChangeIsActive").click( function () {                 
                return $.ajax2({ data: { Theme : { id: $(this).attr('id'), value: $(this).attr('name'), token: '{mfForm::getToken("ChangeForm")}' } },
                                 url: "{url_to('themes_admin_ajax',['action'=>'ChangeIsActive'])}",
        ...
        $(".Delete").click( function () { 
                ...
                return $.ajax2({ data :{ Theme: $(this).attr('id') },
                                 url :"{url_to('themes_admin_ajax',['action'=>'Delete'])}",
                                 success : function(resp) {
                                       if (resp.action=='deleteTheme')
                                       {    
                                          $("tr#themes-"+resp.id).remove();  
                                          if ($('.themes').length==0)
                                              return $.ajax2({ url:"{url_to('themes_admin_ajax',['action'=>'List'])}",target: "#middle"});
                                          updatePager(1);
                                        }       
                                 }
                     });                         
