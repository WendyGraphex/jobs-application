...
<div class="bg-white px-2 py-2">            
  ...
  <table class="table table-bordered table-hover table-striped" cellpadding="0" cellspacing="0"> 
        ...
        <th>#</th>           
            ...
            <th>{__('Language')}</th>  
            <th>{__('IP')}</th>    
            <th><span class="mr-1">{__('Number of items')}</span>
                 
            </th>
             <th><span class="mr-1">{__('Number of events')}</span>
      ...
      <tr class="list" id="{$item->get('id')}" role="row">
          ...
          <td>{$item->get('ip')}</td>
          <td>{$item->getFormatter()->getNumberOfItems()}</td>       
          <td>{$item->getFormatter()->getNumberOfEvents()}</td>       
...
{* =================================== PAGER/FILTER ======================================================== *}
        ...
        {          
           return $.ajax2({ data: getFilterParameters(), 
                            url:"{url_to('site_event_cart_ajax',['action'=>'ListPartial'])}" ,                          
                            target: "#actions"
                             });
        }
        ...
        $("#init").click(function () { 
                return $.ajax2({ url:"{url_to('site_event_cart_ajax',['action'=>'ListPartial'])}",                                
                         target: "#actions"
                });
        });
...
{* =================================== ACTIONS ======================================================== *}
        ...
        $(".pager").click(function () {                     
                return $.ajax2({ data: getFilterParameters(), 
                                 url:"{url_to('site_event_cart_ajax',['action'=>'ListPartial'])}?page="+$(this).attr('data-page'),                                
                                 target: "#actions"
                });
        });
      ...
      $(".Products").click( function () {                    
                  return $.ajax2({ data : { SiteEventCart : $(this).attr('id') },                               
                                  url:"{url_to('site_event_cart_ajax',['action'=>'ListPartialProductForCart'])}", 
                                  target: "#actions"});
          });
    ...
    $(".Display").click( function () {                    
                  return $.ajax2({ data : { SiteEventCart : $(this).attr('id') },                               
                                  url:"{url_to('site_event_cart_ajax',['action'=>'DisplayCart'])}", 
                                  target: "#actions"});
          });
