...
class SiteEventCartBase extends mfObject3 {
     
    protected static $fields=array( 'event_user_id', 'number_of_items',
                                   'culture','currency_id','state','lang','reference',
                                   'number_of_events',
    ...
    protected static $fieldsNull=array('updated_at','event_user_id'); // By default
  ...
  /*  protected function executeIsExistQuery($db)    
    ...
    {
        return $this->formatter=$this->formatter===null?new SiteEventCartFormatter($this):$this->formatter;
    }
    
    function hasEventUser()
    {
        return (boolean)$this->event_user_id;
    }
  ...
  /*  function addEvent(SiteEventCartEvent $item)
    {
        if ($this->cart_events===null)          
            $this->cart_events = new SiteEventCartEventCollection();                           
        $item->set('cart_id',$this);          
        $this->cart_events[]=$item;        
        return $this;
    }*/
  ...
  /*  function deleteQuotation(CartQuotation $cart_quotation)
    ...
    function getEngine()
    {
        return $this->engine=$this->engine===null?new SiteEventCartEngine($this):$this->engine;
    }
    
    function clean()
    {
        $this->cart_events = null; 
        $this->cart_events_with_tax= null;
        return $this;
    }
    ...
    function getCartEvents()
    {
         if ($this->cart_events===null)          
         {    
            $this->cart_events = new SiteEventCartEventCollection($this);             
         }
         return $this->cart_events;
    }
   ...
   /*  function getCartProductsWithTax()
    ...
    {
        return $this->settings=$this->settings===null?new SiteEventCartSettings():$this->settings;
    }
    
      function setReference()
    {        
        $format_reference=$this->getSettings()->get('site_event_cart_format_reference','EVC{year}{month}{id}');       
        $parameters=array('{year}'=>$this->get('year'),
                          '{month}'=>$this->get('month'),
                          '{id}'=>format_number($this->get('id'),$this->getSettings()->get('site_event_cart_format_number','0000')));                
    ...
    function getCommissionWithTax()
     ...
     function getTaxAmount()
     {
      ...
      function getTaxes()
     {
         return  $this->_taxes = $this->_taxes ===null ?new SiteEventCartTaxes(new mfJson($this->get('taxes'))):$this->_taxes;
     }
