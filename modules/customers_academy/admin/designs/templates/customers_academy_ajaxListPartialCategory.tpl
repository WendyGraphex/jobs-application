<div class="row admin-header-container">
   <div class="col-sm">
     ...
     <h3 class="admin-header-small-text">{__('Course Category management')}</h3>
   </div>
   <div class="col-sm" style="padding: 0;">
       <div style="float: right;"> 
         <a class="btn btn-success admin-header-button btn-color"  href="#" id="New"><i class="fa fa-plus"></i> {__('New Category')}</a>
         ...
         {* <a class="btn btn-warning admin-header-button" href="#"><i class="fa fa-file-excel"></i> {__('Export XLS')}</a> *}
          ...
          <div class="breadcrumb-title">
            <p>                
                ...
                <i class="fa fa-angle-right" style="margin: 0 5px 0 5px; font-size: 1em;"></i>{__('Category')}
            </p>
          </div>
          <!-- End breadcrumb -->
{alerts}
...
<div class="bg-white px-2 py-2">   
    <div class="row">       
     {component name="/customers_academy/path"}  
    </div>
     <div>                 
     ...
     {*<div class="row float-left mt-2 w-100 padd-row">
      ...
      <thead  class="bgSpec">
        <tr role="row">
            ...
            {*<th><span class="mr-1">{__('Number Of CustomerCourseCategorys')}</span>
      ...
      <tr class="list" id="{$item->get('id')}" role="row">
          ...
          <td>{$item->getFormatter()->getCreatedAt()->getText()}</td>
            <td>
              ...
              <a href="#" class="list-action text-info Topics" id="{$item->get('id')}" title="{__('Topics')}"><i class="fa fa-font icon-font"></i></a>
               <a href="#" class="list-action text-info Softwares" id="{$item->get('id')}" title="{__('Softwares')}"><i class="fab fa-microsoft icon-font"></i></a>
              <a href="#" class="list-action text-danger Delete" id="{$item->get('id')}" name="{if $item->hasI18n()}{$item->getI18n()->get('title')}{else}{__('---')}{/if}" title="{__('Delete')}"><i class="fa fa-trash-alt icon-font delete-color"></i></a>
            </td>
        </tr>
        {/foreach}
         {if !$pager->hasItems()}
  ...
  var updated_at_dates = $( ".Range.updatedAtDate#updated_at_from, .Range.updatedAtDate#updated_at_to" ).datepicker({
     ...
     function getFilterParameters()
        ...
        $(".pager").click(function () {                     
                return $.ajax2({ data: getFilterParameters(), 
                                 url:"{url_to('customers_academy_ajax',['action'=>'ListPartialCategory'])}?page="+$(this).attr('data-page'),                                
                                 target: "#actions"
                });
        });
      ...
      $(".View").click( function () {                    
                  return $.ajax2({  data : { CustomerCourseCategoryI18n : { 
                                                category_id: $(this).attr('id'),
                                                lang: $("#SelectListLanguages option:selected").attr('id')                                            
                                    } },                            
                                  url:"{url_to('customers_academy_ajax',['action'=>'ViewCategoryI18n'])}", 
                                  target: "#actions"});
          });
    
      $("#New").click( function () {             
                return $.ajax2({      
                    data : { CustomerCourseCategoryNode : { node : '{$node->get('id')}', 
                                                 lang: $("#SelectListLanguages option:selected").attr('id'),
                                                 token: "{mfForm::getToken('CustomerCourseCategoryNodeForm')}" 
                                               } },                
                    url: "{url_to('customers_academy_ajax',['action'=>'NewCategoryI18n'])}",               
                    target: "#actions"
              });
            });
     ...
     $(".Delete").click( function () { 
                ...
                return $.ajax2({ data :{ Category: $(this).attr('id') },
                                 url :"{url_to('customers_academy_ajax',['action'=>'DeleteCategory'])}",                               
                                 success : function(resp) {                                   
                                       if (resp.action!='DeleteCategory') return ;                                          
     ...
     $(".Categories").click( function () {                    
                  return $.ajax2({  data : { CustomerCourseCategoryNode : { 
                                                node: $(this).attr('id'),
                                                lang: $("#SelectListLanguages option:selected").attr('id'),
                                                 token : '{mfForm::getToken('CustomerCourseCategoryNodeForm')}'
                                    } },                            
    ...
    $(".CustomerCourseCategory-item").click(function () { 
    ...
    $(".CustomerCourseCategoryPath-item").click(function () { 
         $('.DropMenuShowFoldersInside').hide();
             var item=$(this);
             return $.ajax2({ 
                         data : { CustomerCourseCategoryI18n : { category_id: $(this).attr('id'),lang : '{$formFilter->getLanguage()}' } },
                         url:"{url_to('customers_academy_ajax',['action'=>'GetChildrenCategory'])}",                                              
                         success : function (resp)
                                {
       ...
       $("#Positions").click( function () {                
                return $.ajax2({  data : { CustomerCourseCategoryI18n : { category_id: '{$node->get('id')}', lang: $("#SelectListLanguages option:selected").attr('id') } }, 
                                url:"{url_to('customers_academy_ajax',['action'=>'PositionsCategory'])}",
                                target: "#actions"});
         }); 
         
         $(".ChangeIsActive").click(function () {
            return $.ajax2({
                 data : { CustomerCourseCategory : { id: $(this).attr('id') , value:$(this).attr('name'),token :'{mfForm::getToken('ChangeForm')}' } },                  
                url: "{url_to('customers_academy_ajax',['action'=>'ChangeIsActiveCategory'])}",              
                success: function (resp) {
                    if (resp.action != 'ChangeIsActiveCategory') return ;
