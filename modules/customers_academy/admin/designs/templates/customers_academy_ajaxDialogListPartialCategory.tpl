...
<div class="bg-white px-2 py-2">   
    <div class="row">    
     ...
     {component name="/partners/path" class='dialog'}  
     {/if}
    </div>    
      ...
      <table id="DialogCustomerCourseCategoryList" class="table table-striped table-bordered dt-responsive nowrap dataTable no-footer w-100 list-user"  role="grid" aria-describedby="Myliste_info">
      <thead  class="bgSpec">
        <tr role="row">
            <th>#</th>                                       
      ...
      <tr class="dialog-list {if $item->get('id') == $form->getSelected()}Selected{/if}" id="{$item->get('id')}" role="row">
          ...
          <td>{if !$item->isLeaf()}
                 <a href="#" class="list-action text-info dialog-categories" id="{$item->get('id')}" title="{__('Categories')}"><i class="fa fa-list icon-font"></i></a>
               {/if}
          </td>
        </tr>
        {/foreach}
...
<script type="text/javascript">
    ...
    $("#DialogCustomerCourseCategoryList").data('selected',{$form->getSelected()});
     ...
     function getDialogFilterParameters()
        {
            var params={ CustomerCourseCategoryNode : { node: '{$node->get('id')}', 
                                             ...
                                             selected:  $("#DialogCustomerCourseCategoryList").data('selected'),
        ...
        {          
           return $.ajax2({ data: getDialogFilterParameters(), 
                            url:"{url_to('customers_academy_ajax',['action'=>'DialogListPartialCategory'])}" ,                          
                            target: "#CustomerCourseCategoryListDialog-ctn"
                             });
        }
        ...
        $(".dialog-pager").click(function () {                     
                return $.ajax2({ data: getDialogFilterParameters(), 
                                 url:"{url_to('customers_academy_ajax',['action'=>'DialogListPartialCategory'])}?page="+$(this).attr('data-page'),                                
                                 target: "#CustomerCourseCategoryListDialog-ctn"
                });
        });
        ...
        $(".dialog-list").click(function() { 
             $("#DialogCustomerCourseCategoryList").data('selected',$(this).attr('id'));
             $(".dialog-list").removeClass('Selected');
             $(this).addClass('Selected');
        });
        
         $(".dialog-categories,.dialog-CustomerCourseCategory-item").click( function () {                    
