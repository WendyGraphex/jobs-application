<div class="row admin-header-container">
    <div class="col-sm">
           <h3 class="admin-header-big-text">{__('Configurations|Levels')}</h3>
           <h3 class="admin-header-small-text">{__('Manage Levels of customers_academy')}</h3>
    </div>
    <div class="col-sm" style="padding: 0;">
        <div style="float: right;"> 
            ...
            {/if}
            <a href="#" class="btn btn-success admin-header-button btn-color" id="Multiple" title="{__('New Multiple levels')}">
                <i class="fa fa-list" style="margin-right: 5px;"></i>
                {__('New Multiple levels')}
            </a>
...
<div class="breadcrumb-title">
    <p>
        ...
        <i class="fa fa-angle-right" style="margin: 0 5px 0 5px; font-size: 1em;"></i>{__('Levels')}
    </p>
</div>
    ...
    var updated_dates = $( " .Range.UpdatedAtDate#updated_at_from, .Range.UpdatedAtDate#updated_at_to" ).datepicker({
        ...
        function updatePager(n)
        {
          ...
          {* =====================  P A G E R  A C T I O N S =============================== *}  
           ...
           $("#init").click(function() {                   
               return $.ajax2({ url:"{url_to('customers_academy_ajax',['action'=>'ListPartialLevel'])}",                                    
                         target: "#actions"}); 
           }); 
          ...
          $("[name=nbitemsbypage]").change(function() { return updateFilter(); }); 
           ...
           $(".pager").click(function () {                      
                return $.ajax2({ data: getFilterParameters(), 
                                 url:"{url_to('customers_academy_ajax',['action'=>'ListPartialLevel'])}?page="+$(this).attr('data-page'),                 
                                target: "#actions"
                });
        });
          {* =====================  A C T I O N S =============================== *}  
          ...
          $("#New").click( function () {             
            return $.ajax2({
                ...
                url: "{url_to('customers_academy_ajax',['action'=>'NewLevelI18n'])}",              
                target: "#actions"
           });
         });
         
         $(".View").click( function () {                
                return $.ajax2({ data : { CustomerAcademyLevelI18n : { 
                                                level_id: $(this).attr('id'),
                                                lang: $("#SelectListLanguages option:selected").attr('id')                                            
                                    } },                                
                                url:"{url_to('customers_academy_ajax',['action'=>'ViewLevelI18n'])}",                              
                                target: "#actions"});
         });
       $(".list").dblclick( function () {                    
                  return $.ajax2({ data : { CustomerAcademyLevelI18n : { level_id: $(this).attr('id'),lang: $("#SelectListLanguages option:selected").attr('id') } },                               
                                  url:"{url_to('customers_academy_ajax',['action'=>'ViewLevelI18n'])}", 
                                  target: "#actions"});
          });
        ...
        $(".Delete").click( function () { 
                if (!confirm('{__("Contract Level \"#0#\" will be deleted. Confirm ?")}'.format($(this).attr('name')))) return false; 
                return $.ajax2({ data :{ CustomerAcademyLevel: $(this).attr('id') },
                                 url :"{url_to('customers_academy_ajax',['action'=>'DeleteLevel'])}",                                
                                 success : function(resp) {
                                          if (resp.action != 'DeleteLevel')  return ;                                                                              
        ...
        $("#Positions").click( function () {                
                return $.ajax2({ url:"{url_to('customers_academy_ajax',['action'=>'PositionsLevel'])}",
                                target: "#actions"});
         });   
      
        $("#Multiple").click( function () {                
                return $.ajax2({  data : { lang : { lang: $("#SelectListLanguages option:selected").attr('id'),token: "{mfForm::getToken('LanguageFrontendForm')}" } },     
                                url:"{url_to('customers_academy_ajax',['action'=>'NewMultipleLevelI18n'])}",
                                target: "#actions"});
         });
</script>    
