<div class="row admin-header-container">
    <div class="col-sm">
           <h3 class="admin-header-big-text">{__('Configurations|Softwares')}</h3>
           <h3 class="admin-header-small-text">{__('Manage Softwares of Academy')}</h3>
    </div>
    <div class="col-sm" style="padding: 0;">
        <div style="float: right;"> 
            ...
            {/if}
            <a href="#" class="btn btn-success admin-header-button btn-color" id="Multiple" title="{__('New Multiple softwares')}">
                <i class="fa fa-list" style="margin-right: 5px;"></i>
                {__('New Multiple softwares')}
            </a>
        ...
        function updatePager(n)
        {
          ...
          $("[name=nbitemsbypage]").change(function() { return updateFilter(); }); 
           ...
           $(".pager").click(function () {                      
                return $.ajax2({ data: getFilterParameters(), 
                                 url:"{url_to('customers_academy_ajax',['action'=>'ListPartialSoftwareForCategoryI18n'])}?page="+$(this).attr('data-page'),                 
                                target: "#actions"
                });
        });
          {* =====================  A C T I O N S =============================== *}  
          ...
          $("#New").click( function () {             
            return $.ajax2({
                ...
                url: "{url_to('customers_academy_ajax',['action'=>'NewSoftwareI18nForCategoryI18n'])}",              
                target: "#actions"
           });
         });
         
         $(".View").click( function () {                
                return $.ajax2({ data : { CustomerAcademySoftwareI18n : { 
                                                software_id: $(this).attr('id'),
                                                lang: $("#SelectListLanguages option:selected").attr('id')                                            
                                    } },                                
                                url:"{url_to('customers_academy_ajax',['action'=>'ViewSoftwareI18nForCategoryI18n'])}",                              
                                target: "#actions"});
         });
         
       $(".list").dblclick( function () {                    
                  return $.ajax2({ data : { CustomerAcademySoftwareI18n : { software_id: $(this).attr('id'),lang: $("#SelectListLanguages option:selected").attr('id') } },                               
                                  url:"{url_to('customers_academy_ajax',['action'=>'ViewSoftwareI18nForCategoryI18n'])}", 
                                  target: "#actions"});
          });
        ...
        $(".Delete").click( function () { 
                if (!confirm('{__("Contract Software \"#0#\" will be deleted. Confirm ?")}'.format($(this).attr('name')))) return false; 
                return $.ajax2({ data :{ CustomerAcademySoftware: $(this).attr('id') },
                                 url :"{url_to('customers_academy_ajax',['action'=>'DeleteSoftware'])}",                                
                                 success : function(resp) {
                                          if (resp.action != 'DeleteSoftware')  return ;                                                                              
        ...
        $("#Positions").click( function () {                
                return $.ajax2({ data : { CustomerAcademySoftwareI18n : { 
                                ...
                                url:"{url_to('customers_academy_ajax',['action'=>'PositionsSoftwareForCategoryI18n'])}",
                                target: "#actions"});
         });   
        ...
        $("#Multiple").click( function () {                
                return $.ajax2({   data : { CustomerCourseCategoryI18n : { category_id: '{$node->get('id')}',lang : $("#SelectListLanguages option:selected").attr('id') } },                
                                url:"{url_to('customers_academy_ajax',['action'=>'NewMultipleSoftwareI18nForCategoryI18n'])}",
                                target: "#actions"});
         });
