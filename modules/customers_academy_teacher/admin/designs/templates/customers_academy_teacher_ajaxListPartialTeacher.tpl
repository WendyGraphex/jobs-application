    <div class="row admin-header-container">
            <div class="col-sm">
              <h3 class="admin-header-big-text">{__('CustomerAcademyTeacherUsers')}</h3>
              <h3 class="admin-header-small-text">{__('Accounts')}</h3>
            </div>
            <div class="col-sm" style="padding: 0;">
                <div style="float: right;"> 
                  {*<a class="btn btn-success admin-header-button"  href="#" id="New"><i class="fa fa-plus"></i> {__('Add CustomerAcademyTeacherUser')}</a>*}
          ...
          <div class="breadcrumb-title">
            <p>              
                ...
                <i class="fa fa-angle-right" style="margin: 0 5px 0 5px; font-size: 1em;"></i>{__('CustomerAcademyTeacherUsers')}
...
<div class="table-responsive bg-white px-1 py-2">
    <div class="col-md-12">
      ...
      <table id="tableCustomerAcademyTeacherUser" class="table table-bordered table-hover table-striped" cellpadding="0" cellspacing="0"> 
        <thead class="bgSpec">
    ...
    <tr class="Type list" id="{$item->get('id')}">
                ...
                <td colspan="3">
                    ...
                    {/if}
                    <div>{$item->getFormatter()->getUsername()}</div>
                ...
                <td>    
                    <a href="#" class="list-action text-info Display" id="{$item->get('id')}" title="{__('Edit')}"><i class="fa fa-search"></i></a>
                   {* <a href="#" class="list-action text-info View" id="{$item->get('id')}"  title="{__('Edit')}"><i class="fa fa-pencil-alt"></i></a> *}
                    <a href="#" class="list-action text-danger Delete" id="{$item->get('id')}" name="{$item->get('firstname')} {$item->get('lastname')}" title="{__('Delete')}"><i class="fa fa-trash-alt"></i></a>
                </td>       
          </tr>
      {/foreach}    
         {if !$pager->hasItems()}
    ...
    {* =================================== METHODS ======================================================== *}
        ...
        {          
           return $.ajax2({ data: getFilterParameters(), 
                            url:"{url_to('customers_academy_teacher_ajax',['action'=>'ListPartialCustomerAcademyTeacherUser'])}" ,                          
                            target: "#actions"
                             });
        }
...
{* =================================== PAGER/FILTER ======================================================== *}             
        ...
        $("#init").click(function() {                   
              return $.ajax2({ url:"{url_to('customers_academy_teacher_ajax',['action'=>'ListPartialCustomerAcademyTeacherUser'])}",                                    
                         target: "#actions"}); 
        }); 
        ...
        $(".pager").click(function () {                     
                return $.ajax2({ data: getFilterParameters(), 
                                 url:"{url_to('customers_academy_teacher_ajax',['action'=>'ListPartialCustomerAcademyTeacherUser'])}?page="+$(this).attr('data-page'),                                
                                 target: "#actions"
                });
        });
...
{* =================================== ACTIONS ======================================================== *}
        ...
        $(".View").click( function () {                    
                  return $.ajax2({ data : { CustomerAcademyTeacherUser : $(this).attr('id') },                               
                                  url:"{url_to('customers_academy_teacher_ajax',['action'=>'ViewCustomerAcademyTeacherUser'])}", 
                                  target: "#actions"});
        });
        $(".Display").click( function () {                    
                  return $.ajax2({ data : { CustomerAcademyTeacherUser : $(this).attr('id') },                               
                                  url:"{url_to('customers_academy_teacher_ajax',['action'=>'DisplayCustomerAcademyTeacherUser'])}", 
                                  target: "#actions"});
        });
    
        $("#New").click( function () {             
                return $.ajax2({                    
                    url: "{url_to('customers_academy_teacher_ajax',['action'=>'NewCustomerAcademyTeacherUser'])}",               
                    target: "#actions"
              });
        });
        ...
        $(".ChangeIsActive").click(function () {
            return $.ajax2({
                 data : { CustomerAcademyTeacherUser : { id: $(this).attr('id') , value:$(this).attr('name'),token :'{mfForm::getToken('ChangeForm')}' } },                  
                url: "{url_to('customers_academy_teacher_ajax',['action'=>'ChangeIsActiveCustomerAcademyTeacherUser'])}",              
                success: function (resp) {
                    if (resp.action != 'ChangeIsActiveCustomerAcademyTeacherUser') return ;
        ...
        $(".ChangeIsValidated").click(function () {
            return $.ajax2({
                 data : { CustomerAcademyTeacherUser : { id: $(this).attr('id') , value:$(this).attr('name'),token :'{mfForm::getToken('ChangeForm')}' } },                  
                url: "{url_to('customers_academy_teacher_ajax',['action'=>'ChangeIsValidatedCustomerAcademyTeacherUser'])}",              
                success: function (resp) {
                    if (resp.action != 'ChangeIsValidatedCustomerAcademyTeacherUser') return ;
        ...
        $(".ChangeIsCompleted").click(function () {
            return $.ajax2({
                 data : { CustomerAcademyTeacherUser : { id: $(this).attr('id') , value:$(this).attr('name'),token :'{mfForm::getToken('ChangeForm')}' } },                  
                url: "{url_to('customers_academy_teacher_ajax',['action'=>'ChangeIsCompletedCustomerAcademyTeacherUser'])}",              
                success: function (resp) {
                    if (resp.action != 'ChangeIsCompletedCustomerAcademyTeacherUser') return ;
        ...
        $(".Delete").click( function () { 
                if (!confirm('{__("CustomerAcademyTeacherUser \"#0#\" will be deleted. Confirm ?")}'.format($(this).attr('name')))) return false; 
                return $.ajax2({ data :{ CustomerAcademyTeacherUser: $(this).attr('id') },
                                 url :"{url_to('customers_academy_teacher_ajax',['action'=>'DeleteCustomerAcademyTeacherUser'])}",                               
                                 success : function(resp) {
                                       if (resp.action!='DeleteCustomerAcademyTeacherUser') return ;                                          
