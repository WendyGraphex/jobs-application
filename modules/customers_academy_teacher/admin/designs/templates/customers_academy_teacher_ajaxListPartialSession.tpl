  ...
  <div class="col-sm">
   ...
   <div class="col-sm" style="padding: 0;">
                <div style="float: right;"> 
                 <a class="btn btn-success admin-header-button"  href="#" id="New"><i class="fa fa-plus"></i> {__('New Connection')}</a>
...
<div class="bg-white px-2 py-2">
    ...
    <th>{__('ID')}           
        ...
        <th>
              <span>{__('IP')}</span>  
                          <div style="float: right;">
                             <a href="#" class="position-absolute order{$formFilter.order.ip->getValueExist('asc','_active')}" id="asc" name="ip"><i class="fas fa-sort-up"></i></a>
                             <a href="#" class="position-absolute order{$formFilter.order.ip->getValueExist('desc','_active')}" id="desc" name="ip"><i class="fas fa-sort-down"></i></a>            
                          </div>  
        </th>
...
<tbody>
    {* search/equal/range *}
     <tr class="input-list">      
       ...
       <td></td>  
       <td>   
             <input class="form-control form-control-sm ml-1 search" name="email" value="{$formFilter.search.email}">
       </td>    
        <td>
             <input class="form-control form-control-sm ml-1 search" name="ip" value="{$formFilter.search.ip}">
        </td> 
     ...
     function getFilterParameters()
        ...
        {          
           return $.ajax2({ data: getFilterParameters(), 
                            url:"{url_to('customers_academy_teacher_ajax',['action'=>'ListPartialSession'])}" ,                                              
                            target: "#actions"
                             });
        }
        ...
        function updatePager(n)
        {
          ...
          {* =====================  P A G E R  A C T I O N S =============================== *}  
           ...
           $("#init").click(function() {                  
               $.ajax2({ url:"{url_to('customers_academy_teacher_ajax',['action'=>'ListPartialSession'])}",                                              
                         target: "#actions"}); 
           }); 
          ...
          $("[name=nbitemsbypage]").change(function() { return updateFilter(); }); 
           ...
           $(".pager").click(function () {                     
                return $.ajax2({ data: getFilterParameters(), 
                                 url:"{url_to('customers_academy_teacher_ajax',['action'=>'ListPartialSession'])}?page="+$(this).attr('data-page'),   
                                 target: "#actions"
                });
        });
        ...
        $("#New").click( function () {             
            return $.ajax2({      
                ...
                url: "{url_to('customers_academy_teacher_ajax',['action'=>'NewSession'])}",               
                target: "#actions"
          });
        });
