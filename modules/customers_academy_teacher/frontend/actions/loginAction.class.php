...
class customers_academy_teacher_loginAction extends mfAction {
    ...
    function execute(mfWebRequest $request) {
        ...
        $this->form = new CustomerAcademyTeacherUserGuardForm($request->getPostParameter('Login'));
        ...
        try 
        {
            ...
            if ($request->isMethod('POST') && $request->getPostParameter('Login')) 
            {
                ...
                if ($this->form->isValid()) {
                    ...
                    $this->getEventDispather()->notify(new mfEvent($values['user'], 'customers_academy_teacher.user.connected')); 
                    $this->redirect(to_link_i18n(mfConfig::get('mf_customers_academy_teacher_redirect_account')));    
                } 
                else 
                { 
                    // var_dump($this->form->getErrorSchema()->getErrorsMessage());
                    $this->getEventDispather()->notify(new mfEvent($this, 'customers_academy_teacher.user.failed.login', array_merge($request->getPostParameter('Login'),array('ip'=>$request->getIp()))));
                }    
            }
        } 
        catch (mfException $e) {
