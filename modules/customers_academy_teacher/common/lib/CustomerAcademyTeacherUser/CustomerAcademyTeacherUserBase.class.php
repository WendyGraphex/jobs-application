...
class CustomerAcademyTeacherUserBase extends mfObject3 {
     
    protected static $fields=array( 'password','gender','firstname','lastname','email','phone','mobile','fax','address1',
                                   'address2','avatar','postcode','city','country','is_completed','web','linkedin_user',
                                   'is_validated','is_locked','username', 'instagram_user', 'state', 'number_of_courses', 'linkedin_user',                 
                                   'lastlogin', 'lastpassword', 'is_active','status', 'created_at','updated_at');    
    const table="t_customer_academy_teacher_user"; 
   ...
   {
       return $this->formatter=$this->formatter===null?new CustomerAcademyTeacherUserFormatter($this):$this->formatter;
   }
    ...
    {               
         if ($this->isNotLoaded())
            return null;
        $session=new CustomerAcademyTeacherUserSession($session_id);
    ...
    function updateRememberMe($ip)
    {
        ...
        CustomerAcademyTeacherUserRemember::cleanup(date('Y-m-d H:i:s', time() - $this->getExpirationAge()), $this);       
        $remember=new CustomerAcademyTeacherUserRemember();
    ...
    {
        return CustomerAcademyTeacherUserRemember::getCookieName();
    }
    ...
    {
        return intval($this->get('number_of_courses'));
    }
    ...
    {        
     ...
     function getAvatar()
    {    
        return $this->_avatar=$this->_avatar===null?new CustomerAcademyTeacherUserAvatarFrame($this):$this->_avatar;   
    }
