...
<div id="Activity-ctn"  data-id="{$item_i18n->getActivity()->get('id')}" class="tablewhiteback" style="padding: 20px;">
      <div class="row">
                                ...
                                {if $item_i18n->getActivity()->hasPicture()}
                                    <div class="PictureExist" id="Picture"> 
                                       <div class="div-img h-100">
                                          <img id="Picture-img" class="img-dpzone img-product" src="{$item_i18n->getActivity()->getPicture()->getMedium()->getUrl()}"/>
                                       </div> 
                        ...
                        <div id="DropZoneIcon" class="dropzone dp-addFile pt-1">                                                  
                                ...
                                {if $item_i18n->getActivity()->hasIcon()}
                                    <div class="IconExist" id="Icon"> 
                                       <div class="div-img h-100">
                                          <img id="Icon-img" class="img-dpzone img-product" src="{$item_i18n->getActivity()->getIcon()->getMedium()->getUrl()}"/>
                                       </div> 
                                       <div class="dz-message" data-dz-message><span>{__('Drop file here to upload change')}</span></div>
                                   </div>                               
...
<script activity="text/javascript"> 
   ...
   $(".Color").minicolors({
      // position: relative,
       ...
       $("#CustomerBlogActivity-ChangeIcon").click(function() {
           $("#CustomerBlogActivity-icon").show();
           $(this).hide();
           $("#Save").show();  
        });           
      
       $('#CustomerBlogActivity-DeleteIcon').click(function(){ 
            if (!confirm("{__("Icon will be deleted. Confirm ?")}")) return false; 
            return $.ajax2({ 
                              data : { CustomerBlogActivity: "{$item_i18n->getActivity()->get('id')}" }, 
                              ...
                              success :function(response) {
                                          ...
                                          {                                                     
                                                $("#CustomerBlogActivity-icon-container").hide(); 
                                                $("span#CustomerBlogActivity-text-icon").html("{__('Add icon')}");
                                          }
                              }
            });  
      }); 
      
    $(".CustomerBlogActivity,.CustomerBlogActivityI18n").click(function() {  $('#Save').show(); });  
         
     {* =================== L A N G U A G E ================================ *}
        
     {* =================== A C T I O N S ================================ *}
      $('#Cancel').click(function() {                
             return $.ajax2({ data : { CustomerBlogActivityNode : { node: '{$item_i18n->getActivity()->getFather()->get('id')}', 
      ...
      $('#Save').click(function(){                             
          ...
          $(".CustomerBlogActivityI18n.Input").each(function() { params.CustomerBlogActivityI18n.activity_i18n[$(this).attr('name')]=$(this).val(); });
          $(".CustomerBlogActivity.Input").each(function() {  params.CustomerBlogActivityI18n.activity[$(this).attr('name')]=$(this).val();  });  // Get foreign key  
