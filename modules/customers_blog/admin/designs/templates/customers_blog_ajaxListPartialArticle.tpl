          ...
          <div class="row admin-header-container">
            ...
            <div class="col-sm" style="padding: 0;">
                <div style="float: right;"> 
                  <a class="btn btn-success admin-header-button"  href="#" id="New"><i class="fa fa-plus"></i> {__('New article')}</a> 
          ...
          <div class="breadcrumb-title">
            <p>             
                ...
                <i class="fa fa-angle-right" style="margin: 0 5px 0 5px; font-size: 1em;"></i>{__('Articles')}
                {*<i class="fa fa-angle-right" style="margin: 0 5px 0 5px; font-size: 1em;"></i>{__('Articles')} *}
            </p>
          </div>
          <!-- End breadcrumb -->
{alerts}
      ...
      <table class="table table-bordered table-hover table-striped" cellpadding="0" cellspacing="0"> 
        ...
        <th>#</th>
            ...
            <th>{__('Start/End')}</span>  
             ...
             <th style="width: 230px;">{__('Published at')}</span>  
      ...
      <tr class="list" id="{$item->get('id')}" role="row">
          ...
          <td>{$item->get('id')}</td>        
          <td>
              {if $item->hasStartedAt()}
                  {$item->getFormatter()->getStartedAt()->getText()}
              {/if}  
               {if $item->hasExpiredAt()}
                  - {$item->getFormatter()->getExpiredAt()->getText()}
              {/if}  
          </td>
         <td>              
             {if $item->hasI18n()}
                 <a href="{$item->getI18n()->getUrl()}" target="_blank">{$item->getI18n()->getFormatter()->getMetaTitle()->ucfirst()}</a>
                  <i class="list-action fa fa-eye"></i> {$item->getI18n()->getFormatter()->getNumberOfViews()->getText()} 
                    {else}
                        {__('----')}
                    {/if}  
           ...
           <td class="align-middle text-center"> <a href="#" class="ChangeIsActive" name="{$item->get('is_active')}" id="{$item->get('id')}"><i class="fa fa-circle text-{if $item->isActive()}success{else}danger{/if}"></i></a>
             ...
             <td class="Article" name="published_at" id="{$item->get('id')}">
                {if $item->isPublished()}
                {$item->getFormatter()->getPublishedAt()->getFormatted()}  
                {else}
                <button class="btn btn-primary btn-sm Publish"  id="{$item->get('id')}">{__('Publish')}</button>
                {/if}
            </td> 
            <td>       
...
<script type="text/javascript">
    {* =================================== METHODS ======================================================== *}
     ...
     function getFilterParameters()
        ...
        $("#init").click(function() {                   
            $.ajax2({ url:"{url_to('customers_blog_ajax',['action'=>'ListPartialArticle'])}",                                    
                      target: "#actions"}); 
        }); 
       ...
       $('.order').click(function() {           
        ...
        $(".search").keypress(function(article) {  if (article.keyCode==13)   return updateFilter(); });

        $("[name=nbitemsbypage],.equal").change(function () {  return updateFilter();  });
    
        $(".pager").click(function () {                     
                return $.ajax2({ data: getFilterParameters(), 
                                 url:"{url_to('customers_blog_ajax',['action'=>'ListPartialArticle'])}?page="+$(this).attr('data-page'),                                
                                 target: "#actions"
                });
        });
      ...
      $(".View").click( function () {                    
                  return $.ajax2({ data : { CustomerBlogArticleI18n : { 
                                                article_id: $(this).attr('id'),
                                                lang: $("#SelectListLanguages option:selected").attr('id')                                            
                                    } },                               
                                  url:"{url_to('customers_blog_ajax',['action'=>'ViewArticleI18n'])}", 
                                  target: "#actions"});
          });
      ...
      $("#New").click( function () {             
                return $.ajax2({        
                   data : { lang : { lang: $("#SelectListLanguages option:selected").attr('id'),token: "{mfForm::getToken('LanguageFrontendForm')}" } },    
                    url: "{url_to('customers_blog_ajax',['action'=>'NewArticleI18n'])}",               
                    target: "#actions"
              });
            });
         
     $(".Delete").click( function () { 
                if (!confirm('{__("Article \"#0#\" will be deleted. Confirm ?")}'.format($(this).attr('name')))) return false; 
                return $.ajax2({ data :{ CustomerBlogArticle: $(this).attr('id') },
                                 url :"{url_to('customers_blog_ajax',['action'=>'DeleteArticle'])}",                               
                                 success : function(resp) {
                                          if (resp.action!='DeleteArticle') return ;                                          
        ...
        $(".ChangeIsActive").click(function () {
            return $.ajax2({
                 data : { CustomerBlogArticle : { id: $(this).attr('id') , value:$(this).attr('name'),token :'{mfForm::getToken('ChangeForm')}' } },                  
                url: "{url_to('customers_blog_ajax',['action'=>'ChangeIsActiveArticle'])}",              
                success: function (resp) {
                    if (resp.action != 'ChangeIsActiveArticle') return ;
 ...
 $(".Publish").click( function () {                    
                  return $.ajax2({ data : { CustomerBlogArticle  : $(this).attr('id') },                               
                                  url:"{url_to('customers_blog_ajax',['action'=>'PublishArticle'])}", 
                                  success : function (resp)
                                        {
                                                if (resp.action !='PublishArticle') return ;                                              
                                                $(".Article[name=published_at][id="+resp.id+"]").html(resp.published_at);
                                        }
                    });
          });
</script>
