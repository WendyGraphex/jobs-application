...
<div class="breadcrumb-title">
    <p>
        ...
        <i class="fa fa-angle-right" style="margin: 0 5px 0 5px; font-size: 1em;"></i>{__('View Article')}
    </p>
</div>
{alerts}
{if $article_i18n->getArticle()->isLoaded()}
<div id="Article-ctn" class="tablewhiteback" style="padding: 20px;">
      <div class="row">
            <div class="form-group col-md-6">                                                              
             {component name="/site_languages/selectListLanguagesFrontend" selected=$article_i18n->get('lang')}
            </div>  
      </div>
         ...
         {* ============== C O N T R O L ========================================= *}
        <div class="tab-content" id="myTabContentViewArticle">         
        <div class="tab-pane fade show active" id="t-control" role="tabpanel" aria-labelledby="t-control-tab">  
            <table cellpadding="0" cellspacing="0">                   
               <tr style="height: 60px;">
                   <td>{__('Starting at')}:{if $form->article.started_at->getOption('required')}*{/if}</td>
                   <td>
         ...
         <div class="tab-pane fade" id="t-activity" role="tabpanel" aria-labelledby="t-activity-tab">
                        <div class="row mt-3" style="height: 20px;">                       
                            <div class="form col-md-12">                                   
                                <div class="form-group">                                                    
                                    <a href="javascript:void(0);" id="DialogActivities"><i class="fa fa-plus"></i></a>
                                </div>                    
                            </div>  
                        </div> 
                        ...
                        <div id="activities-ctn">
                            <div id="CustomerBlogActivities">                               
                                {foreach $article_i18n->getArticle()->getArticleActivities()->getActivities() as $activity} 
                                    <div class="CustomerBlogActivities-list" id="{$activity->get('id')}" style="display: inline-block;">                                              
                                    ...
                                    <a href="javascript:void(0);" title="{__('Delete')}" class="CustomerBlogArticleActivity-Delete" id="{$activity->get('id')}"  name="{if $activity->hasI18n()}{$activity->getI18n()->get('title')}{else}{$activity->get('name')}{/if}">
                 ...
                 <div id="Modal-Activities" class="modal " role="dialog">
                    <div class="modal-dialog modal-lg model-cat-product">    
                        <div class="modal-content">
                            <div class="px-2"><span class="close" id="Modal-Activities-Close">&times;</span></div>
         ...
         <div class="tab-pane fade" id="t-picture" role="tabpanel" aria-labelledby="t-picture-tab">                     
              <div class="row portfolio-grid">
                            ...
                            {if $article_i18n->getArticle()->hasPicture()}
                                    <div class="PictureExist" id="Picture"> 
                                       <div class="div-img h-100">
                                          <img id="Picture-img" class="img-dpzone img-product" src="{$article_i18n->getArticle()->getPicture()->getMedium()->getUrl()}"/>
                                       </div> 
                ...
                <div class="col-2">  
                    ...
                    <a href="#" {if !$article_i18n->getArticle()->hasPicture()}style="display:none"{/if} id="DeletePicture" name="{$article_i18n->getArticle()->get('picture')}" class="list-action"><i class="fa fa-trash"></i></a>
                </div>           
              </div>
         </div>           
         {* ============== DESCRIPTION ========================================= *}
...
<script type="text/javascript">
        ...
        $('#Save').click(function(){          
          var params= { CustomerBlogArticleI18n : { 
                               ...
                               article :{ },
                               article_i18n: {  
                                    article_id: '{$article_i18n->get('article_id')}',
                                    lang: $("#SelectListLanguages option:selected").attr('id')                              
                                         },
         ...
         // alert("Params="+params.toSource());   return;
          return $.ajax2({ 
                            data : params,
                            url: "{url_to('customers_blog_ajax',['action'=>'SaveArticleI18n'])}",
                            target: "#actions"                            
               }); 
   ...
   $("#DeletePicture").click( function () { 
            ...
            return $.ajax2({ data :{ CustomerBlogArticle: '{$article_i18n->get('article_id')}' },
...
if ($("#DropZonePicture").find('.dz-default').length)
     ...
     $("#DropZonePicture").dropzone({
      url: "{url_to("customers_blog_ajax",['action'=>'UploadPictureArticle'])}",
     // clickable: false,
      params: {               
                "CustomerBlogArticle" : '{$article_i18n->get('article_id')}',
                "UploadPictureArticle[token]" : '{mfForm::getToken('UploadPictureArticleForm')}' 
              },   
      ...
      $("#DialogActivities").click(function () { 
           var params = { CustomerBlogActivityNode : {  lang: $("#SelectListLanguages option:selected").attr('id'), selection : {$article_i18n->getArticle()->getArticleActivities()->getActivities()->getKeys()->toJson()},  token : '{mfForm::getToken('CustomerBlogActivitySelectionForm')}' } };      
            $(".CustomerBlogArticleActivities-list").each(function() { params.CustomerBlogActivity.selection.push($(this).attr('id'));  }); 
             return $.ajax2({      
                         data : params,
    ...
    $(".CustomerBlogArticleActivity-Delete").click(function () {  
        $(".CustomerBlogActivities-list[id="+$(this).attr('id')+"]").remove();
    }); 
      
</script>
{else}
    {__('Article is invalid')}
{/if}    