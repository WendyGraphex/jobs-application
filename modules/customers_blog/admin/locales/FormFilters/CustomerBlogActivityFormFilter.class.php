...
class CustomerBlogActivityFormFilter extends mfFormFilterBase {
    ...
    {                         
       ...
       $this->setClass('CustomerBlogActivity');
       
       $this->setFields(array(          
              'name'=>array(              
              'search'=>array('conditions'=>
                                                 "(".
               CustomerBlogActivity::getTableField('name')." COLLATE UTF8_GENERAL_CI LIKE '%%{name}%%'".
                                                 ")")
                                ),
           'title'=>array(
              ...
              'search'=>array('conditions'=>
                                                 "(".
               CustomerBlogActivityI18n::getTableField('title')." COLLATE UTF8_GENERAL_CI LIKE '%%{title}%%'".
                                                 ")")
                                ),  
           ));
       $this->setQuery("SELECT {fields} FROM ".CustomerBlogActivity::getTable(). 
                       " LEFT JOIN ".CustomerBlogActivityI18n::getInnerForJoin('activity_id')." AND lang='{lang}'".                     
                       " WHERE level={levelplusone} AND  lb >= {lb} AND rb <= {rb} ".
                            // " AND state IS NULL ".
                       " ORDER BY ".CustomerBlogActivity::getTableField('rb')." ASC".
                       ";");        
