          ...
          <div class="breadcrumb-title">
            <p>               
                ...
                <i class="fa fa-angle-right" style="margin: 0 5px 0 5px; font-size: 1em;"></i>{__('Credits')}
     ...
     function getFilterParameters()
        ...
        {                   
           return $.ajax2({ data: getFilterParameters(), 
                            url:"{url_to('payments_event_user_ajax',['action'=>'ListPartialCreditMethod'])}" ,                          
                            target: "#actions"
                             });
        }
        
        $("#init").click(function() {                   
            $.ajax2({ url:"{url_to('payments_event_user_ajax',['action'=>'ListPartialCreditMethod'])}",                                    
                      target: "#actions"}); 
        }); 
       ...
       $('.order').click(function() {           
        ...
        $(".pager").click(function () {                     
                return $.ajax2({ data: getFilterParameters(), 
                                 url:"{url_to('payments_event_user_ajax',['action'=>'ListPartialCreditMethod'])}?page="+$(this).attr('data-page'),                                
                                 target: "#actions"
                });
        });
         ...
         $("#Add").click(function() {                   
            $.ajax2({ url:"{url_to('payments_event_user_ajax',['action'=>'AddCreditMethod'])}",                                    
                      target: "#actions"}); 
        });
        
        $(".ChangeIsActive").click(function () {
            return $.ajax2({
                 data : { PaymentCreditEventUserMethod : { id: $(this).attr('id') , value:$(this).attr('name'),token :'{mfForm::getToken('ChangeForm')}' } },                  
                url: "{url_to('payments_event_user_ajax',['action'=>'ChangeIsActiveCreditMethod'])}",              
                success: function (resp) {
                    if (resp.action != 'ChangeIsActiveCreditMethod') return ;
