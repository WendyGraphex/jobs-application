...
<script type="text/javascript">
        ...
        $("#Methods").click(function() {                   
            return $.ajax2({ url:"{url_to('payments_event_user_ajax',['action'=>'ListPartialDebitMethod'])}",                                    
                      target: "#actions"}); 
        }); 
     
         $("#Save").click(function() {                   
            var params = { AddPaymentDebitMethod : {  methods: [], token : '{$form->getCSRFToken()}' }  };
            $(".PaymentMethod:checked").each(function () { params.AddPaymentDebitMethod.methods.push($(this).val()); });
            return $.ajax2({   data :params,
                                url:"{url_to('payments_event_user_ajax',['action'=>'AddDebitMethod'])}",                                    
                                target: "#actions"}); 
        });
</script>
