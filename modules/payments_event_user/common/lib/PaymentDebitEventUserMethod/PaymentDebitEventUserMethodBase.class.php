...
class PaymentDebitEventUserMethodBase extends mfOrderedObject3 {
    
    protected static $fields=array('method_id','is_active','position','created_at','updated_at');
    const table="t_payment_debit_event_user_method"; 
   ...
   {
       $list=new PaymentDebitEventUserMethodCollection();
       $db=mfSiteDatabase::getInstance()
                    ->setParameters(array())   
                    ->setObjects(array('PaymentDebitEventUserMethod','PaymentMethod'))               
                    ->setQuery("SELECT {fields} FROM ".PaymentDebitEventUserMethod::getTable().    
                               " INNER JOIN ". PaymentDebitEventUserMethod::getOuterForJoin('method_id').    
                               ";")
                    ->makeSqlQuery();
        if (!$db->getNumRows())
                return $list;         
        ...
        {                                   
           $list[$items->getPaymentDebitEventUserMethod()->get('id')]=$items->getPaymentDebitEventUserMethod()->set('method_id',$items->getPaymentMethod());
        }  
       return $list; 
   }
   ...
   {   
       ...
       $db=mfSiteDatabase::getInstance();       
         ...
         $db=mfSiteDatabase::getInstance()
                    ->setParameters(array())                      
                    ->setQuery("SELECT method_id FROM ".PaymentDebitEventUserMethod::getTable().    
         ...
         $collection = new PaymentDebitEventUserMethodCollection();
         foreach ($methods as $index=>$method_id)
         {    
             $item= new PaymentDebitEventUserMethod();
   ...
   static function getMethods()
   {
       ...
       $db=mfSiteDatabase::getInstance()
                    ->setParameters(array())                               
                    ->setQuery("SELECT ".PaymentMethod::getFieldsAndKeyWithTable()." FROM ".PaymentDebitEventUserMethod::getTable().   
                               " INNER JOIN ". PaymentDebitEventUserMethod::getOuterForJoin('method_id').    
                              " WHERE ".PaymentDebitEventUserMethod::getTableField('is_active')."='YES' AND ".  
