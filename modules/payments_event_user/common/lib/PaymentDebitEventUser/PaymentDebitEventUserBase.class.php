...
class PaymentDebitEventUserBase extends mfObject3 {
    ...
    const table="t_payment_debit_event_user"; 
    ...
    function getSettings()
    {
      ...
      function getState()
    {
        return $this->_state=$this->_state===null?new PaymentDebitEventUserState($this->get('state')):$this->_state;
    } 
    
     function loadReference()
    {        
        $format_reference=$this->getSettings()->get('payment_debit_event_reference_format','P{year}{month}{day}{id}');
        $parameters=array('{year}'=>date("Y"),'{month}'=>date("m"), '{day}'=>date("d"),
                          '{id}'=>format_number($this->get('id'),$this->getSettings()->get('payment_debit_event_format_number','00000')));
    ...
    {      
        return $this->_order_id=$this->_order_id===null?new SiteEventOrder($this->get('order_id'),$this->getEventUser()):$this->_order_id;
    }
   ...
   {
       return $this->formatter=$this->formatter===null?new PaymentDebitEventUserFormatter($this):$this->formatter;
   }
    ...
    function getEngine()
   {
       if ($this->engine===null)
       {
            $class = "PaymentMethodEvent".$this->getMethod()->get('name')."Engine";
            if (!class_exists($class))
                $class = "PaymentMethodEventEngine";
    ...
    function create()
    {        
        if (!$this->get('order_id') || !$this->get('event_user_id'))
          return $this;                    
    ...
    function validate()
    {
        ...
        $this->save();
        $this->getOrder()->set('state', SiteEventOrderState::PAID)->save();
        return $this;
    }
