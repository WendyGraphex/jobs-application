...
class ProjectDocumentFileManagerPictures extends Pictures {
     
     function __construct(ProjectDocumentFileManager $item) {
        ...
        if ($this->original===null)
        {
            $this->original=new PictureObject3(array(
               ...
               //  "url"=>url($this->getFileManager()->getPathForUrl()."/".$this->getFileManager()->getItem()->get('file'),'web',"frontend"),
                  "urlPath"=>url_to("projects_documents_download",array('file'=>'','id'=>''),mfConfig::get('mf_app')),
                  "url"=>url_to("projects_documents_download",array('file'=>$this->getFileManager()->getItem()->get('file'),'id'=>$this->getFileManager()->getItem()->get('id')),mfConfig::get('mf_app'))
            ));
        }   
    ...
    {               
       return new PictureObject3(array(
             ...
             "urlPath"=>url_to("projects_documents_download_picture",array('file'=>'','id'=>'','type'=>$type),mfConfig::get('mf_app')),
                  "url"=>url_to("projects_documents_download_picture",array('file'=>$this->getFileManager()->getItem()->get('file'),'type'=>$type,'id'=>$this->getFileManager()->getItem()->get('id')),mfConfig::get('mf_app'))
        ));
    }
