...
class ProjectDocumentFileManager {
     
    protected $item=null;
     
    function __construct(ProjectDocument $item) {
        $this->item=$item;
    }
   ...
   /* function escapeFilename(mfString $filename)
    ...
    function getImages()
    {
        return $this->images=$this->images===null?new ProjectDocumentFileManagerPictures($this):$this->images;
    }
     ...
     function getFile()
    {
        return new FileObject2(array(
              ...
              "urlPath"=>url_to("projects_documents_download",array('file'=>'','id'=>''),mfConfig::get('mf_app')),
                  "url"=>url_to("projects_documents_download",array('file'=>$this->getItem()->get('file'),'id'=>$this->getItem()->get('id'),),mfConfig::get('mf_app'))
              ));
    }
    
    function getFlavicons()
    {     
        return $this->flavicons=$this->flavicons===null?new ProjectDocumentFileFlavicons($this->getItem()->get('extension')):$this->flavicons;
    }
