...
<script type="text/javascript">
        ...
        $("#Methods").click(function() {                   
            return $.ajax2({ url:"{url_to('payments_employee_ajax',['action'=>'ListPartialMethod'])}",                                    
                      target: "#actions"}); 
        }); 
     
         $("#Save").click(function() {                   
            var params = { AddPaymentMethod : {  methods: [], token : '{$form->getCSRFToken()}' }  };
            $(".PaymentMethod:checked").each(function () { params.AddPaymentMethod.methods.push($(this).val()); });
            return $.ajax2({   data :params,
                                url:"{url_to('payments_employee_ajax',['action'=>'AddMethod'])}",                                    
                                target: "#actions"}); 
        });
</script>
