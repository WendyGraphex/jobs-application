     ...
     function getFilterParameters()
        ...
        {                   
           return $.ajax2({ data: getFilterParameters(), 
                            url:"{url_to('payments_employee_ajax',['action'=>'ListPartialMethod'])}" ,                          
                            target: "#actions"
                             });
        }
        
        $("#init").click(function() {                   
            $.ajax2({ url:"{url_to('payments_employee_ajax',['action'=>'ListPartialMethod'])}",                                    
                      target: "#actions"}); 
        }); 
       ...
       $('.order').click(function() {           
        ...
        $(".pager").click(function () {                     
                return $.ajax2({ data: getFilterParameters(), 
                                 url:"{url_to('payments_employee_ajax',['action'=>'ListPartialMethod'])}?page="+$(this).attr('data-page'),                                
                                 target: "#actions"
                });
        });
         ...
         $("#Add").click(function() {                   
            $.ajax2({ url:"{url_to('payments_employee_ajax',['action'=>'AddMethod'])}",                                    
                      target: "#actions"}); 
        });
        
        $(".ChangeIsActive").click(function () {
            return $.ajax2({
                 data : { PaymentEmployeeMethod : { id: $(this).attr('id') , value:$(this).attr('name'),token :'{mfForm::getToken('ChangeForm')}' } },                  
                url: "{url_to('payments_employee_ajax',['action'=>'ChangeIsActiveMethod'])}",              
