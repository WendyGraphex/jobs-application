...
<div class="row admin-header-container">
    <div class="col-sm">
        <h3 class="admin-header-big-text">{__("New  Type")}</h3>
...
<div class="breadcrumb-title">
    <p>
        ...
        <i class="fa fa-angle-right" style="margin: 0 5px 0 5px; font-size: 1em;"></i>{__('New Type')}
    </p>
</div>
{alerts}
            ...
            <label for="icon">{__('Icon')}{if $form->type.icon->getOption('required')}*{/if}</label> 
                <div class="input-group">
                  ...
                  <span class="btn btn-default btn-file bg-primary">
                     <a href="#" id="item_file_browse" class="file-btn">{__('Browse…')}</a><input type="file" class="Files Fields" style="display:none" name="CustomerClaimTypeI18n[type][icon]" id="item_file">
                  </span>
                          </span>
...
<script type="text/javascript">     
   ...
   $(".Color").minicolors({
     ...
     $('#Cancel').click(function(){                
             return $.ajax2({ data: { filter: { lang:$("#SelectListLanguages option:selected").attr('id'), token: "{mfForm::getToken('CustomerClaimTypeFormFilter')}" } },                              
      ...
      $('#Save').click(function(){                             
        ...
        //      alert("Params="+params.toSource());   return ;      
          return $.ajax2({ data : params,  
                           files :'.Files',
                           url: "{url_to('customers_ajax',['action'=>'SaveNewTypeI18n'])}",
                           target: "#actions"}); 
        });                          
