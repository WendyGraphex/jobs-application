                        ...
                        {foreach $pager as $item} 
                            <tr class="list" id="{$item->get('id')}" role="row">
                                ...
                                <td>
                                 ...
                                 <td id="{$item->get('id')}" class="SaleOrder" name="state">
                                    {$item->getStateI18n()}
                                </td> 
                                <td>
 ...
 <script type="text/javascript">      
      ...
      function getFilterParameters()
        ...
        {          
           return $.ajax2({ data: getFilterParameters(), 
                            url:"{url_to('sales_order_ajax',['action'=>'ListPartialOrderForEmployer'])}" ,                          
                            target: "#actions"
                             });
        }
      ...
      function updatePager(n)
        ...
        $(".pager").click(function () {                     
                return $.ajax2({ data: getFilterParameters(), 
                                 url:"{url_to('sales_order_ajax',['action'=>'ListPartialOrderForEmployer'])}?page="+$(this).attr('data-page'),                                
                                 target: "#actions"
                });
        });
    ...
    $(".ProceedToPayment").click(function () {          
        return $.ajax2({ data : { SaleOrder : $(this).attr('id') },
                         url :"{url_to('sales_order_ajax',['action'=>'Order'])}",            
                         target : "#actions"
             });
    });
   ...
   /* $(".OpenDetailsMoadl").click(function () {    
    ...
    $(".Details").click(function () { 
             return $.ajax2({ data : { SaleOrder : $(this).attr('id') },
                         url :"{url_to('sales_order_ajax',['action'=>'ListPartialDetailForEmployer'])}",            
    ...
    $(".CancelPayment").click(function () { 
             return $.ajax2({ data : { SaleOrder : $(this).attr('id') },
                         url :"{url_to('sales_order_ajax',['action'=>'CancelPaymentForEmployer'])}",                                     
                         success : function (resp) { 
                              if (resp.action !='CancelPaymentForEmployer') return ;
                              $(".Actions[id="+resp.id+"]").remove();
                              $(".SaleOrder[id="+resp.id+"][name=state]").html(resp.state);
                         }
             });
