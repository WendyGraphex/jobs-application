 ...
 * Instagram API class
 *
 * API Documentation: https://developers.facebook.com/docs/instagram-basic-display-api/getting-started
...
class Instagram
{
    ...
    public function getUser($id = 0)
    {
        ...
        return $this->_makeCall('me/' . $id, $auth,array('fields'=>'id,username'));
    }

    public function getUserPicture($id = 0)
    {
        $callback = $this->_makeCallUserData($this->getUsername().'/',array('__a'=>'1'));
        $pictureURL =  $callback->graphql->user->profile_pic_url_hd;
        return  $pictureURL;
    }
    ...
    {
        $callback = $this->_makeCallUserData($this->getUsername().'/',array('__a'=>'1'));
        return $callback->graphql->user->full_name;
    }

    public function getBiography()
    {
        $callback = $this->_makeCallUserData($this->getUsername().'/',array('__a'=>'1'));
        return $callback->graphql->user->biography;
    }
     ...
     * @return mixed
     */
    public function getUserMedia($limit = 0)
    {
        return $this->_makeCall('me/media', true, array('fields'=>'id,username,caption,media_url,media_type,permalink,thumbnail_url,timestamp'));
    }
    ...
    protected function _makeCall($function, $auth = false, $params = null, $method = 'GET')
    {
      ...
      //  echo "Params=".var_export($params,true)."<br/>";
        ...
        if (!$jsonData) {
            throw new InstagramException('Error: _makeCall() - cURL error: ' . curl_error($ch)." [".$jsonData."]");
        }
    ...
    protected function _makeCallUserData($function, $params = null, $method = 'GET')
    {
        $paramString = null;
        ...
        $apiCall = self::INSTAGRAM_URL . $function . $authMethod . (('GET' === $method) ? $paramString : null);
    ...
    private function _makeOAuthCall($apiData)
    {
        ...
        if (!$jsonData) {
            throw new InstagramException('Error: _makeOAuthCall() - cURL error: ' . curl_error($ch)." [".$jsonData."]");
        }
