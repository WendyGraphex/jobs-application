...
<div class="row admin-header-container">
    <div class="col-sm">
           <h3 class="admin-header-big-text">{__('Configurations|Warnings')}</h3>
           <h3 class="admin-header-small-text">{__('Manage Warnings')}</h3>
    </div>
...
<div class="breadcrumb-title">
    <p>
        ...
        <i class="fa fa-angle-right" style="margin: 0 5px 0 5px; font-size: 1em;"></i>{__('Warnings')}
    </p>
</div>
    ...
    var updated_dates = $( " .Range.UpdatedAtDate#updated_at_from, .Range.UpdatedAtDate#updated_at_to" ).datepicker({
        ...
        function updatePager(n)
        {
          ...
          {* =====================  P A G E R  A C T I O N S =============================== *}  
           ...
           $("#init").click(function() {                   
               return $.ajax2({ url:"{url_to('site_warning_ajax',['action'=>'ListPartialWarning'])}",                                    
                         target: "#actions"}); 
           }); 
          ...
          $("[name=nbitemsbypage]").change(function() { return updateFilter(); }); 
           ...
           $(".pager").click(function () {                      
                return $.ajax2({ data: getFilterParameters(), 
                                 url:"{url_to('site_warning_ajax',['action'=>'ListPartialWarning'])}?"+this.href.substring(this.href.indexOf("?")+1, this.href.length),
                                target: "#actions"
                });
        });
          {* =====================  A C T I O N S =============================== *}  
          ...
          $("#New").click( function () {             
            return $.ajax2({
                ...
                url: "{url_to('site_warning_ajax',['action'=>'NewWarningI18n'])}",              
                target: "#actions"
           });
         });
         
         $(".View").click( function () {                
                return $.ajax2({ data : { SiteWarningI18n : { 
                                                warning_id: $(this).attr('id'),
                                                lang: $("#SelectListLanguages option:selected").attr('id')                                            
                                    } },                                
                                url:"{url_to('site_warning_ajax',['action'=>'ViewWarningI18n'])}",                              
                                target: "#actions"});
         });
         
         
       $(".list").dblclick( function () {                    
                  return $.ajax2({ data : { SiteWarningI18n : { warning_id: $(this).attr('id'),lang: $("#SelectListLanguages option:selected").attr('id') } },                               
                                  url:"{url_to('site_warning_ajax',['action'=>'ViewWarningI18n'])}", 
                                  target: "#actions"});
          });
        ...
        $(".Delete").click( function () { 
                if (!confirm('{__("Freelancer Warning \"#0#\" will be deleted. Confirm ?")}'.format($(this).attr('name')))) return false; 
                return $.ajax2({ data :{ SiteWarning: $(this).attr('id') },
                                 url :"{url_to('site_warning_ajax',['action'=>'DeleteWarning'])}",                                
                                 success : function(resp) {
                                          if (resp.action != 'DeleteWarning')  return ;                                                                              
        ...
        $("#Positions").click( function () {                
                return $.ajax2({ url:"{url_to('site_warning_ajax',['action'=>'PositionsWarning'])}",
                                target: "#actions"});
         });   
        ...
        $("#Multiple").click( function () {                
                return $.ajax2({  data : { lang : { lang: $("#SelectListLanguages option:selected").attr('id'),token: "{mfForm::getToken('LanguageFrontendForm')}" } },     
                                url:"{url_to('site_warning_ajax',['action'=>'NewMultipleWarningI18n'])}",
                                target: "#actions"});
         });
</script>    
