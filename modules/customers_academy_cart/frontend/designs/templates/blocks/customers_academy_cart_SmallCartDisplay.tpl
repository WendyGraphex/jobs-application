<li class="nav-item dropdown mr-1 btnCartShop">
    <a class="nav-link count-indicator dropdown-toggle d-flex justify-content-center align-items-center" id="SmartCartBtn" href="#" data-toggle="dropdown">
        <i class="mdi mdi-cart-outline mx-0"></i>        
        <span class="countCart" id="SmartCart-Number-ctn" {if !$cart}style="display:none"{/if}>
            <span class="numberOfItems"  id="SmartCart-Number-Text">{if $cart}{*$cart->getFormatter()->getNumberOfQuotations()->getText()*}{/if}</span>
        </span>      
    </a>
    ...
    {JqueryScriptsReady}     
       ...
       $("#SmartCartBtn").click(function () { 
        ...
        //   if (!$("#SmallCartItems").hasClass('show')) return ;    
           return $.ajax2({ url :"{url_to('customers_academy_cart_ajax',['action'=>'SmallCartDisplay'])}", 
