<div class="row admin-header-container">
    <div class="col-sm">
        <h3 class="admin-header-big-text">{__("View Help")}</h3>
...
<div class="breadcrumb-title">
    <p>
        ...
        <i class="fa fa-angle-right" style="margin: 0 5px 0 5px; font-size: 1em;"></i>{__('View Help')}
    </p>
</div>
{alerts}
                        ...
                        <label>{__('Icon')}</label>                  
                            <div id="pictureForm">                            
                                <div class="custom-FileInput">
                                    {if $item_i18n->getHelp()->hasIcon()}
                                        <img id="blah" alt="" src="{$item_i18n->getHelp()->getIcon()->getURL('superadmin')}"{*/if*} /><br/>
                                        {else}
                                ...
                                <input id="item_file" type='file' hidden="" class="form-control Files Fields" name="SiteHelpI18n[help][icon]" /> {*name="SiteCityI18n[function][icon]"*}
                            </div>
                    </div>
                </div> 
              </div>
               ...
               <div class="col-md-10">
                   <div class="row">
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <input class="SiteHelp form-control Fields Input Color General input-color" type="text" value="{$item_i18n->getHelp()->get('color')}" name="color" id="color"/>
                        </div>
                   </div>
                    <div class="row">
                        <div class="form-group col-md-12">
                            ...
                            <div class="field_with_errors"><input class="SiteHelpI18n form-control Fields Input" type="text" value="{$item_i18n->get('title')}" name="title" id="title"></div>
                        </div>
                    </div>
                      <div class="row">
                        <div class="form-group col-md-12">
...
<script language="text/javascript">
   ...
   $(".Color").minicolors({
      // position: relative,
       ...
       $("#SiteHelp-ChangeIcon").click(function() {
           $("#SiteHelp-icon").show();
           $(this).hide();
           $("#Save").show();  
        });           
      
       $('#SiteHelp-DeleteIcon').click(function(){ 
            if (!confirm("{__("Icon will be deleted. Confirm ?")}")) return false; 
            return $.ajax2({ 
                              data : { SiteHelp: "{$item_i18n->getHelp()->get('id')}" }, 
                              url: "{url_to('site_help_ajax',['action'=>'DeleteIconContractHelp'])}",                              
                              success :function(response) {
                                          if (response.action=='DeleteIconContractHelp' && response.id=="{$item_i18n->getHelp()->get('id')}")
                                          {                                                     
                                                $("#SiteHelp-icon-container").hide(); 
                                                $("span#SiteHelp-text-icon").html("{__('Add icon')}");
                                          }
                              }
            });  
      }); 
      
    $(".SiteHelp,.SiteHelpI18n").click(function() {  $('#Save').show(); });  
         
     {* =================== L A N G U A G E ================================ *}
        
     {* =================== A C T I O N S ================================ *}
     $('#Cancel').click(function(){                
             return $.ajax2({ data: { filter: { lang:  '{$item_i18n->get('lang')}', token: "{mfForm::getToken('SiteHelpFormFilter')}" } },                              
      ...
      $('#Save').click(function(){                             
            var  params= {      iFrame:true,             
                                SiteHelpI18n: { 
                                   help_i18n : { lang: '{$item_i18n->get('lang')}', help_id : '{$item_i18n->get('help_id')}' },
                                   help : { },
        ...
        //      alert("Params="+params.toSource());   return ;      
          return $.ajax2({ data : params,  
                           files :'.Files',
                           url: "{url_to('site_help_ajax',['action'=>'SaveHelpI18n'])}",
                           target: "#actions"}); 
        });                          
</script>
