        ...
        <div class="card-body pr-2 pl-2" style="padding-left: 20px !important;">
            <div class="col-md-5" style="margin: 0 auto;">
                <div class="totalPrice" style="text-align: left;">
                    <div>
                        <span>{__('Events')} {$order->getFormatter()->getNumberOfEvents()->getChoicesText("[0]no event|[1]1 event|(1,Inf]%s events")}</span><label style="right: 18px;position: absolute;"> {$order->getFormatter()->getGlobalSalePriceWithTax()->getAmount()}</label>
                    </div>
                    <div>
                ...
                <div class="totalPrice" style="text-align: justify;margin-top: 25px;margin-bottom: 25px;font-size: 26px;">
                    {component name="/payments_event_user/list"} 
                </div>
                <div>
...
<script type="text/javascript">      
     ...
     $(".Payment").click(function () { 
            var url = "{url_to('payments_process_event_ajax')}".replace("{ldelim}payment{rdelim}",$(this).data('method'));
            $.ajax2({ data : { SiteEventOrder : { price : '{$order->getFormatter()->getGlobalSalePriceWithTax()->getText("#.00")}' , token : '{mfForm::getToken('SiteEventOrderPaymentPriceForm')}' } },
                         url : url,            
                         success : function (resp)
                         {
                             if (resp.action !='ProcessEventDebit') return ;
    ...
    $("#Proceed").click(function () { 
            ...
            var url = "{url_to('payments_proceed_event')}".replace("{ldelim}payment{rdelim}",$(".Payment:checked").data('method'));
            $.ajax2({ data : { SiteEventOrder : '{$order->get('id')}' },
                         url : url,            
                         success : function (resp)
                         {
