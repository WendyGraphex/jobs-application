<div class="row admin-header-container">
  <div class="col-sm">
              ...
              <h3 class="admin-header-small-text">{__('Sales Order Payment management')}</h3>
            </div>
...
<div class="breadcrumb-title">
  <p>              
      ...
      <i class="fa fa-angle-right" style="margin: 0 5px 0 5px; font-size: 1em;"></i>{__('Sale Payment Orders')}
  </p>
</div>
    ...
    <td class="count">{(($pager->getPage()-1)*$pager->getNbItemsByPage())+$item@iteration}</td>
                ...
                <td>{$item->get('reference')}</td>                 
                 <td>                
                  ...
                  <td>             
                      <a href="{$item->getEvent()->getI18n()->getUrl()}" target="_blank">{$item->getEvent()->getI18n()->getFormatter()->getMetaTitle()}</a>
                </td>
                 <td>                
                ...
                <td>       
                 ...
                 {*   <a href="#" class="list-action text-info Payments" id="{$item->get('id')}"  title="{__('Payments')}"><i class="fa fa-list"></i></a>                *}
                    <a target="_blank" class="list-action text-info" href="{url_to('site_event_order',['action'=>'ExportPdf'])}?order={$item->get('id')}" title="{__('Order')}">
...
<script type="text/javascript">
      ...
      function getFilterParameters()
        ...
        {          
           return $.ajax2({ data: getFilterParameters(), 
                            url:"{url_to('site_event_order_ajax',['action'=>'ListPartialPayment'])}" ,                          
                            target: "#actions"
                             });
        }
        ...
        $(".pager").click(function () {                     
                return $.ajax2({ data: getFilterParameters(), 
                                 url:"{url_to('site_event_order_ajax',['action'=>'ListPartialPayment'])}?page="+$(this).attr('data-page'),                                
                                 target: "#actions"
                });
        });
  ...
  {* =====================  P A G E R  A C T I O N S =============================== *}  
           ...
           $("#init").click(function() {                   
               $.ajax2({ url:"{url_to('site_event_order_ajax',['action'=>'ListPartialPayment'])}",                                    
                         target: "#actions"}); 
           }); 
         ...
         // $("[name=name]").change(function() { return updateFilter(); }); 
           ...
           $(".pager").click(function () {                      
                return $.ajax2({ data: getFilterParameters(), 
                                 url:"{url_to('site_event_order_ajax',['action'=>'ListPartialPayment'])}?"+this.href.substring(this.href.indexOf("?")+1, this.href.length),
                                target: "#actions"
                });
        });    
