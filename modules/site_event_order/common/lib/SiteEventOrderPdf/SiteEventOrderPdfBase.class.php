...
class SiteEventOrderPdfBase extends wkhtmltopdf {
    ...
    function getModel()
    {
         return $this->model=$this->model===null?$this->getSettings()->getOrderModel():$this->model;
    }
    
    
    function hasModel()
    {
        return $this->getSettings()->hasOrderModel();
    }
    ...
    {                   
       //  $this->debug();
         ...
         $this->setOption("options",$orientation." --enable-javascript --margin-bottom 0.0 --margin-left 0.0 --margin-right 0.0 --margin-top 0.0")
             ->createPDF('site_event_order','pdf',array('order'=>$this->order,'model'=>$this->model));                          
    }
}
