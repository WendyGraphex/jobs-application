<div class="row admin-header-container">
    <div class="col-sm">
        <h3 class="admin-header-big-text">{__("View Freelancer Criteria")}</h3>
                    ...
                    <div class="form-group">
                        <label>{__('Icon')}</label>                  
                            <div id="pictureForm">                            
                                ...
                                <input id="item_file" type='file' hidden="" language="file" class="form-control Files Fields" name="EmployeeNotationCriteriaI18n[language][icon]" /> {*name="SiteCityI18n[function][icon]"*}
                            </div>
                    </div>
                </div> 
              </div>
               ...
               <div class="col-md-10">
                   <div class="row">
                        <div class="form-group col-md-5">             
                            {if $form->hasErrors()}<div class="alert alert-{if $form.criteria.name->hasError()}danger{else}light{/if}" role="alert">&nbsp;{$form.criteria.name->getError()}</div>{/if}
                             ...
                             <div class="field_with_errors">
                                <input class="EmployeeNotationCriteria form-control Fields Input" language="text" value="{$item_i18n->getCriteria()->get('name')}" name="name" id="name">
                             </div>
                        </div> 
                        <div class="form-group col-md-2">
                            ...
                            <input class="EmployeeNotationCriteria form-control Fields Input Color General input-color" language="text" value="{$item_i18n->getCriteria()->get('color')}" name="color" id="color"/>
                        </div>
                           <div class="form-group col-md-2">
                            ...
                            <input class="EmployeeNotationCriteria form-control Fields Input input-color" type="text" value="{$item_i18n->getCriteria()->get('max')}" name="max" id="max"/>
                        </div>
                          <div class="form-group col-md-3">
                            ...
                            <div class="field_with_errors">
                                {html_options class="EmployeeNotationCriteria form-control Fields Select" name="category_id" options=$form->criteria['category_id']->getChoices()->toArray() selected=$item_i18n->getCriteria()->get('category_id')}                            
                            </div>
                        </div>
                    ...
                    <div class="row">
                        <div class="form-group col-md-12">
                            ...
                            <div class="field_with_errors"><input class="EmployeeNotationCriteriaI18n form-control Fields Input" language="text" value="{$item_i18n->get('title')}" name="title" id="title"></div>
                        </div>
                    </div>
                </div>
        </div> 
...
<script language="text/javascript">
   ...
   $(".Color").minicolors({
      // position: relative,
       ...
       $("#EmployeeNotationCriteria-ChangeIcon").click(function() {
           $("#EmployeeNotationCriteria-icon").show();
           $(this).hide();
           $("#Save").show();  
        });           
      
       $('#EmployeeNotationCriteria-DeleteIcon').click(function(){ 
            if (!confirm("{__("Icon will be deleted. Confirm ?")}")) return false; 
            return $.ajax2({ 
                              data : { EmployeeNotationCriteria: "{$item_i18n->getCriteria()->get('id')}" }, 
                              url: "{url_to('employees_notation_ajax',['action'=>'DeleteIconContractCriteria'])}",                              
                              success :function(response) {
                                          if (response.action=='DeleteIconContractCriteria' && response.id=="{$item_i18n->getCriteria()->get('id')}")
                                          {                                                     
                                                $("#EmployeeNotationCriteria-icon-container").hide(); 
                                                $("span#EmployeeNotationCriteria-text-icon").html("{__('Add icon')}");
                                          }
                              }
            });  
      }); 
      
    $(".EmployeeNotationCriteria,.EmployeeNotationCriteriaI18n").click(function() {  $('#Save').show(); });  
         
     {* =================== L A N G U A G E ================================ *}
        
     {* =================== A C T I O N S ================================ *}
     $('#Cancel').click(function(){                
             return $.ajax2({ data: { filter: { lang:  '{$item_i18n->get('lang')}', token: "{mfForm::getToken('EmployeeNotationCriteriaFormFilter')}" } },                              
      ...
      $('#Save').click(function(){                             
            var  params= {      iFrame:true,             
                                EmployeeNotationCriteriaI18n: { 
                                   criteria_i18n : { lang: '{$item_i18n->get('lang')}', criteria_id : '{$item_i18n->get('criteria_id')}' },
                                   criteria : { },
                                   token :'{$form->getCSRFToken()}'
                                } };
          $(".EmployeeNotationCriteriaI18n.Input").each(function() { params.EmployeeNotationCriteriaI18n.criteria_i18n[$(this).attr('name')]=$(this).val(); });
          $(".EmployeeNotationCriteria.Input").each(function() {  params.EmployeeNotationCriteriaI18n.criteria[$(this).attr('name')]=$(this).val();  });  // Get foreign key  
          $(".EmployeeNotationCriteria.Select option:selected").each(function() {  params.EmployeeNotationCriteriaI18n.criteria[$(this).parent().attr('name')]=$(this).val();  });  // Get foreign key  
        //      alert("Params="+params.toSource());   return ;      
          return $.ajax2({ data : params,  
                           files :'.Files',
                           url: "{url_to('employees_notation_ajax',['action'=>'SaveCriteriaI18n'])}",
                           target: "#actions"}); 
        });                          
</script>
