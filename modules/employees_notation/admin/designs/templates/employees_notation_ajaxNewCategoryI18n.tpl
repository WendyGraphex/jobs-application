...
<div class="row admin-header-container">
    <div class="col-sm">
        <h3 class="admin-header-big-text">{__("New Freelancer Notation Category")}</h3>
...
<div class="breadcrumb-title">
    <p>
        ...
        <i class="fa fa-angle-right" style="margin: 0 5px 0 5px; font-size: 1em;"></i>{__('New Freelancer Notation Category')}
    </p>
</div>
{alerts}
                    ...
                    <div class="form-group">
                        <label>{__('Icon')}</label>                  
                            <div id="pictureForm">                            
                                ...
                                <input id="item_file" type='file' hidden="" type="file" class="form-control Files Fields" name="EmployeeNotationCategoryI18n[category][icon]" /> {*name="SiteCityI18n[function][icon]"*}
                            </div>
                    </div>
                </div> 
              </div>
                        ...
                        <div class="form-group col-md-6">             
                            {if $form->hasErrors()}<div class="alert alert-{if $form.category.name->hasError()}danger{else}light{/if}" role="alert">&nbsp;{$form.category.name->getError()}</div>{/if}
                             ...
                             <div class="field_with_errors">
                                <input class="EmployeeNotationCategory form-control Fields Input" type="text" value="{$item_i18n->getCategory()->get('name')}" name="name" id="name">
                             </div>
                        </div> 
                        <div class="form-group col-md-6">
...
<script language="text/javascript"> 
   ...
   $(".Color").minicolors({
     ...
     $('#Cancel').click(function(){                
             return $.ajax2({ data: { filter: { lang:$("#SelectListLanguages option:selected").attr('id'), token: "{mfForm::getToken('EmployeeNotationCategoryFormFilter')}" } },                              
      ...
      $('#Save').click(function(){                             
        ...
        //      alert("Params="+params.toSource());   return ;      
          return $.ajax2({ data : params,  
                           files :'.Files',
                           url: "{url_to('employees_notation_ajax',['action'=>'SaveNewCategoryI18n'])}",
                           target: "#actions"}); 
        });                          
