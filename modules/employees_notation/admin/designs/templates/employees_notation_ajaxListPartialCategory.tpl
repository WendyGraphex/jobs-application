...
<div class="row admin-header-container">
    <div class="col-sm">
           <h3 class="admin-header-big-text">{__('Configurations|Freelancer Notation')}</h3>
           <h3 class="admin-header-small-text">{__('Manage Notation Categories of employees')}</h3>
    </div>
    <div class="col-sm" style="padding: 0;">
        <div style="float: right;"> 
            ...
            <a href="#" class="btn btn-success admin-header-button btn-color" id="New" title="{__('New')}">
             ...
             <a href="#" class="btn btn-success admin-header-button btn-color" id="Multiple" title="{__('New Multiple Categories')}">
                <i class="fa fa-list" style="margin-right: 5px;"></i>
                {__('New Multiple Category')}
            </a>
             <a href="#" class="btn btn-success admin-header-button btn-color" id="Multiple" title="{__('Download Categories')}">
...
<div class="breadcrumb-title">
    <p>
        ...
        <i class="fa fa-angle-right" style="margin: 0 5px 0 5px; font-size: 1em;"></i>{__('Freelancer Notation Categories')}
    </p>
</div>
    ...
    var updated_dates = $( " .Range.UpdatedAtDate#updated_at_from, .Range.UpdatedAtDate#updated_at_to" ).datepicker({
        ...
        function updatePager(n)
        {
          ...
          {* =====================  P A G E R  A C T I O N S =============================== *}  
           ...
           $("#init").click(function() {                   
               return $.ajax2({ url:"{url_to('employees_notation_ajax',['action'=>'ListPartialCategory'])}",                                    
                         target: "#actions"}); 
           }); 
          ...
          $("[name=nbitemsbypage]").change(function() { return updateFilter(); }); 
           ...
           $(".pager").click(function () {                      
                return $.ajax2({ data: getFilterParameters(), 
                                 url:"{url_to('employees_notation_ajax',['action'=>'ListPartialCategory'])}?"+this.href.substring(this.href.indexOf("?")+1, this.href.length),
                                target: "#actions"
                });
        });
          {* =====================  A C T I O N S =============================== *}  
          ...
          $("#New").click( function () {             
            return $.ajax2({
                ...
                url: "{url_to('employees_notation_ajax',['action'=>'NewCategoryI18n'])}",              
                target: "#actions"
           });
         });
         
         $(".View").click( function () {                
                return $.ajax2({ data : { EmployeeNotationCategoryI18n : { 
                                                category_id: $(this).attr('id'),
                                                lang: $("#SelectListLanguages option:selected").attr('id')                                            
                                    } },                                
                                url:"{url_to('employees_notation_ajax',['action'=>'ViewCategoryI18n'])}",                              
                                target: "#actions"});
         });
         
         
       $(".list").dblclick( function () {                    
                  return $.ajax2({ data : { EmployeeNotationCategoryI18n : { category_id: $(this).attr('id'),lang: $("#SelectListLanguages option:selected").attr('id') } },                               
                                  url:"{url_to('employees_notation_ajax',['action'=>'ViewCategoryI18n'])}", 
                                  target: "#actions"});
          });
        ...
        $(".Delete").click( function () { 
                if (!confirm('{__("Employee Category \"#0#\" will be deleted. Confirm ?")}'.format($(this).attr('name')))) return false; 
                return $.ajax2({ data :{ EmployeeNotationCategory: $(this).attr('id') },
                                 url :"{url_to('employees_notation_ajax',['action'=>'DeleteCategory'])}",                                
        ...
        $("#Positions").click( function () {                
                return $.ajax2({ url:"{url_to('employees_notation_ajax',['action'=>'PositionsCategory'])}",
                                target: "#actions"});
         });   
        ...
        $("#Multiple").click( function () {                
                return $.ajax2({  data : { lang : { lang: $("#SelectListLanguages option:selected").attr('id'),token: "{mfForm::getToken('LanguageFrontendForm')}" } },     
                                url:"{url_to('employees_notation_ajax',['action'=>'NewMultipleCategoryI18n'])}",
                                target: "#actions"});
         });
</script>    
