...
<div class="row admin-header-container">
    <div class="col-sm">
           ...
           <h3 class="admin-header-small-text">{__('Manage Notation Criterias of employees')}</h3>
    </div>
    ...
    var updated_dates = $( " .Range.UpdatedAtDate#updated_at_from, .Range.UpdatedAtDate#updated_at_to" ).datepicker({
        ...
        function updatePager(n)
        {
          ...
          {* =====================  P A G E R  A C T I O N S =============================== *}  
           ...
           $("#init").click(function() {                   
               return $.ajax2({ url:"{url_to('employees_notation_ajax',['action'=>'ListPartialCriteria'])}",                                    
                         target: "#actions"}); 
           }); 
          ...
          $("[name=nbitemsbypage]").change(function() { return updateFilter(); }); 
           ...
           $(".pager").click(function () {                      
                return $.ajax2({ data: getFilterParameters(), 
                                 url:"{url_to('employees_notation_ajax',['action'=>'ListPartialCriteria'])}?"+this.href.substring(this.href.indexOf("?")+1, this.href.length),
                                target: "#actions"
                });
        });
          {* =====================  A C T I O N S =============================== *}  
          ...
          $("#New").click( function () {             
            return $.ajax2({
                ...
                url: "{url_to('employees_notation_ajax',['action'=>'NewCriteriaI18n'])}",              
                target: "#actions"
           });
         });
         
         $(".View").click( function () {                
                return $.ajax2({ data : { EmployeeNotationCriteriaI18n : { 
                                                criteria_id: $(this).attr('id'),
                                                lang: $("#SelectListLanguages option:selected").attr('id')                                            
                                    } },                                
                                url:"{url_to('employees_notation_ajax',['action'=>'ViewCriteriaI18n'])}",                              
                                target: "#actions"});
         });
         
         
       $(".list").dblclick( function () {                    
                  return $.ajax2({ data : { EmployeeNotationCriteriaI18n : { criteria_id: $(this).attr('id'),lang: $("#SelectListLanguages option:selected").attr('id') } },                               
                                  url:"{url_to('employees_notation_ajax',['action'=>'ViewCriteriaI18n'])}", 
                                  target: "#actions"});
          });
        ...
        $(".Delete").click( function () { 
                if (!confirm('{__("Employee Criteria \"#0#\" will be deleted. Confirm ?")}'.format($(this).attr('name')))) return false; 
                return $.ajax2({ data :{ EmployeeNotationCriteria: $(this).attr('id') },
                                 url :"{url_to('employees_notation_ajax',['action'=>'DeleteCriteria'])}",                                
        ...
        $("#Positions").click( function () {                
                return $.ajax2({ url:"{url_to('employees_notation_ajax',['action'=>'PositionsCriteria'])}",
                                target: "#actions"});
         });   
        ...
        $("#Multiple").click( function () {                
                return $.ajax2({  data : { lang : { lang: $("#SelectListLanguages option:selected").attr('id'),token: "{mfForm::getToken('LanguageFrontendForm')}" } },     
                                url:"{url_to('employees_notation_ajax',['action'=>'NewMultipleCriteriaI18n'])}",
                                target: "#actions"});
         });
</script>    
