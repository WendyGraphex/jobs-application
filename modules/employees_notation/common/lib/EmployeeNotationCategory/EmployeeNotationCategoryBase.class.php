...
class EmployeeNotationCategoryBase extends mfOrderedObject3 {
     
    protected static $fields=array('name','icon','color','position','created_at','updated_at');
    const table="t_employee_notation_category";
    ...
    function hasI18n()
    {
     ...
     {       
         if ($this->i18n===null)
         {
             ...
             $this->i18n=new EmployeeNotationCategoryI18n(array('lang'=>$lang,"language_id"=>$this->get('id')));
         }   
         return $this->i18n;
     } 
     ...
     static function getAllI18nForSelect()
    {
        ...
        $db=mfSiteDatabase::getInstance()
                ...
                ->setQuery("SELECT ".EmployeeNotationCategoryI18n::getFieldsAndKeyWithTable()." FROM ".EmployeeNotationCategoryI18n::getTable().
                           " WHERE lang='{lang}' ORDER BY title ASC;")               
                ->makeSqlQuery(); 
        if (!$db->getNumRows())
            return $values;
        while ($item=$db->fetchObject('EmployeeNotationCategoryI18n'))
        { 
    ...
    static function getAllForSelect()
    {
        ...
        $db=mfSiteDatabase::getInstance()
                ->setParameters(array("lang"=>mfcontext::getInstance()->getUser()->getLanguage()))
                ->setObjects(array('EmployeeNotationCategoryI18n','EmployeeNotationCategory'))
                ->setQuery("SELECT {fields} FROM ".EmployeeNotationCategory::getTable().
        ...
        {             
            $values[$items->getEmployeeNotationCategory()->get('id')]=$items->getEmployeeNotationCategory()->setI18n($items->getEmployeeNotationCategoryI18n());
        }              
        return $values;
    }
    ...
    static function getCategoriesForSelect()
    {
        ...
        $db=mfSiteDatabase::getInstance()
                ->setParameters(array("lang"=>mfcontext::getInstance()->getUser()->getLanguage()))               
                ->setQuery("SELECT * FROM ".EmployeeNotationCategoryI18n::getTable().                           
                           " WHERE lang='{lang}' ORDER BY title ASC;")               
                ->makeSqlQuery(); 
        if (!$db->getNumRows())
            return $values;
      ...
      static function getCategoriesByPositionForSelect()
    {
        ...
        $db=mfSiteDatabase::getInstance()
                ->setParameters(array("lang"=>mfcontext::getInstance()->getUser()->getLanguage()))               
                ->setQuery("SELECT ".EmployeeNotationCategoryI18n::getFieldsAndKeyWithTable()." FROM ".EmployeeNotationCategoryI18n::getTable().                           
                           " INNER JOIN ".EmployeeNotationCategoryI18n::getOuterForJoin('category_id').
    ...
    {          
        ...
        $db=mfSiteDatabase::getInstance()               
                ...
                ->setObjects(array('EmployeeNotationCategory','EmployeeNotationCategoryI18n'))
                ->setQuery("SELECT {fields} FROM ".self::getTable(). 
                           " INNER JOIN ".EmployeeNotationCategoryI18n::getInnerForJoin('category_id')." AND lang='{lang}'".
   ...
   {
       return $this->formatter=$this->formatter===null?new EmployeeNotationCategoryFormatter($this):$this->formatter;
   }
    ...
    {
        return $this->criterias=$this->criterias===null?new EmployeeNotationCriteriaCollection():$this->criterias;
    }
