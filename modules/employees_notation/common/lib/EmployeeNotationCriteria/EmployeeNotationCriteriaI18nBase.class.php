...
class EmployeeNotationCriteriaI18nBase extends mfObjectI18n {
     
    protected static $fields=array('title','criteria_id','lang','created_at','updated_at');
    const table="t_employee_notation_criteria_i18n"; 
    protected static $foreignKeys=array('criteria_id'=>'EmployeeNotationCriteria'); // By default
     ...
     static function create(mfArray $values,$category,$max,$lang)
    {
        ...
        if ($values->isEmpty())
            return ;       
        $collection_i18n = new EmployeeNotationCriteriaI18nCollection();
        foreach ($values as $value)        
        {
            $item_i18n = new EmployeeNotationCriteriaI18n();
            $item_i18n->add(array('lang'=>(string)$lang,'title'=>$value,'category_id'=>($category?$category:null)));
