...
<div>
    ...
    <ul class="ulCategory">
        ...
        <li>
            ...
            <div>
                <ul class="allStars Notation" data-rate="0" data-criteria="{$criteria->get('id')}" data-category="{$category->get('id')}" style="color:#222;">
                    {section name=note start=0 loop=$criteria->get('max')}
                        <li> <a href="#" id="{$smarty.section.note.index}" class="Criteria-{$criteria->get('id')} Criteria" data-criteria="{$criteria->get('id')}" data-category="{$category->get('id')}">
...
<script type="text/javascript">
    ...
    $("#SaveReview-btn").click(function () { 
           var params = { EmployerReviewNotations : { notations :[ ], employer_user_id : '{$employer->get('username')}', employee_advert_i18n_id : '{$advert_i18n->get('id')}' , token : '{$form->getCSRFToken()}' } };
           ...
           return $.ajax2({ 
                            data : params,
                            url :"{url_to('employees_notation_ajax',['action'=>'SaveEmployerNotation'])}",                           
