...
<div>
    <a id="AddPermissionsGroup" href="#"><img src="{url('/icons/add.gif','picture')}"
                                              alt="{__('new')}"/>{__('Add permission group')}</a>
...
<div>
    <img class="{$site->getSiteID()}-PermissionsGroup" id="{$formFilter.lang}" name="lang"
         src="{url("/flags/`$formFilter.lang|default:"__"`.png","picture")}"
         title="{if $formFilter.lang->hasError()}{$formFilter.lang}{else}{format_country($formFilter.lang)}{/if}"/>
    <style> .ui-dialog {
            font-size: 62.5%;
        }</style>
    <a id="{$site->getSiteID()}-PermissionsGroup-ChangeLang" href="#" title="{__('change')}"><img
                src="{url('/icons/edit.gif','picture')}" alt='{__("language")}'/></a>
...
<table cellpadding="0" cellspacing="0">
    <tr>
        ...
        <th>{__('id')}
            <div>
                <a href="#" class="PermissionsGroup-order{$formFilter.order.id->getValueExist('asc','_active')}"
                   id="asc" name="id"><img
                            src='{url("/icons/sort_asc`$formFilter.order.id->getValueExist("asc","_h")`.gif","picture")}'
                            alt="{__('order_asc')}"/></a>
                <a href="#" class="PermissionsGroup-order{$formFilter.order.id->getValueExist('desc','_active')}"
                   id="desc" name="id"><img
        ...
        <th>{__('name')}
            <div>
                <a href="#" class="PermissionsGroup-order{$formFilter.order.name->getValueExist('asc','_active')}"
                   id="asc" name="name"><img
                            src='{url("/icons/sort_asc`$formFilter.order.name->getValueExist("asc","_h")`.gif","picture")}'
                            alt="{__('order_asc')}"/></a>
                <a href="#" class="PermissionsGroup-order{$formFilter.order.name->getValueExist('desc','_active')}"
        ...
        <th>{__('value')}
            <div>
                <a href="#" class="PermissionsGroup-order{$formFilter.order.value->getValueExist('asc','_active')}"
                   id="asc" name="value"><img
                            src='{url("/icons/sort_asc`$formFilter.order.value->getValueExist("asc","_h")`.gif","picture")}'
                            alt="{__('order_asc')}"/></a>
                <a href="#" class="PermissionsGroup-order{$formFilter.order.value->getValueExist('desc','_active')}"
                   id="desc" name="value"><img
                            src='{url("/icons/sort_desc`$formFilter.order.value->getValueExist("desc","_h")`.gif","picture")}'
                            alt="{__('order_desc')}"/></a>
            </div>
        </th>
        <th>{__('actions')}</th>
    ...
    <tr>
        ...
        <td><input class="PermissionsGroup-search" type="text" size="5" name="name" value="{$formFilter.search.name}">
        </td>
        <td></td>
        <td></td>
    </tr>
    {foreach $pager as $permission}
        <tr class="PermissionsGroup" id="PermissionsGroup-{$permission->getPermissionGroup()->get('id')}">
            ...
            {if $pager->getNbItems()>5}
                <td><input class="PermissionsGroup-selection" type="checkbox"
                           name="{$permission->getPermissionGroup()->get('id')}"
                           id="{$permission->getPermissionGroupI18n()->get('value')}"/></td>{/if}
            <td>
            ...
            <td>
                <a href="#" title="{__('edit')}" class="{$site->getSiteID()}-PermissionsGroup-View"
                   id="{$permission->getPermissionGroup()->get('id')}">
                    <img src="{url('/icons/edit.gif','picture')}" alt='{__("edit")}'/></a>
                {if $permission->hasPermissionGroupI18n()}
                    <a href="#" title="{__('delete')}" class="PermissionsGroups-Delete"
                       id="{$permission->getPermissionGroupI18n()->get('id')}"
                       name="{$permission->getPermissionGroupI18n()->get('value')}"><img
...
{elseif $pager->getNbItems()>5}
    ...
    <a href="#" title="{__('delete')}" id="PermissionsGroup-Delete"><img src="{url('/icons/delete.gif','picture')}"
                                                                         alt='{__("delete")}'/></a>
{/if}
...
<script type="text/javascript">
    ...
    function getPermissionsFilterParameters() {
        ...
        $(".PermissionsGroup-search").each(function () {
    ...
    function updatePermissionsFilter() {
        ...
        return $.ajax2({
            data: getPermissionsFilterParameters(),
    ...
    function updatePager(n) {
        ...
        $(".PermissionsGroup-count").each(function (id) {
            $(this).html(start + id)
        }); // Update index column
            ...
            success: function (resp) {
                if (resp.action == 'deletePermissiongroup') {
                    ...
                    $.each(resp.parameters, function (id) {
                        $("tr#PermissionsGroup-" + this).remove();
                    });
    ...
    $(".PermissionsGroups-Delete").click(function () {
        ...
        return $.ajax2({
            data: {PermissionGroupI18n: this.id},
        ...
        return $.ajax2({
            data: {
                lang: {
                    ...
                    token: "{mfForm::getToken('LanguageFrontendForm')}"
                }
            },
