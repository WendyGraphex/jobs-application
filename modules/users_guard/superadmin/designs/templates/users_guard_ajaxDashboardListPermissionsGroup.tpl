...
<script type="text/javascript">
        ...
        function updatePager(n)
        {
         ...
         $('.order').click(function() {
          ...
          $("#init").click(function() { $.ajax2({ data: { group : "{$group->get('id')}" },
            url: "{url_to('users_guard_ajax',['action'=>'DashboardListPermissionsGroup'])}",
             loading: "#tab-dashboard-superadmin-loading",
            target: "#actions"}); }); 
          ...
          $(".pager").click(function () {
                    return $.ajax2({ data: getPermissionsFilterParameters(), 
                    url:"{url_to('users_guard_ajax',['action'=>'DashboardListPermissionsGroup'])}?"+this.href.substring(this.href.indexOf("?")+1, this.href.length),
                     loading: "#tab-dashboard-superadmin-loading",
                    target: "#actions"});
          });
          ...
          $("#DeletePermissions").click( function () { 
             ...
             return $.ajax2({ data:params,
                              url: "{url_to('users_guard_ajax',['action'=>'DashboardDeletePermissionsGroup'])}",
                               loading: "#tab-dashboard-superadmin-loading",
          ...
          $(".DeletePermission").click( function () { 
                ...
                return $.ajax2({ data: { groupPermission: this.id },
                                 url: "{url_to('users_guard_ajax',['action'=>'DashboardDeletePermissionGroup'])}",
                                 success : function(resp) {
          ...
          $("#AddPermission").click( function () { 
                return $.ajax2({ data: { group : "{$group->get('id')}" },
            url: "{url_to('users_guard_ajax',['action'=>'DashboardAddPermissionsGroup'])}",
             loading: "#tab-dashboard-superadmin-loading",
            target: "#actions"});
          });
