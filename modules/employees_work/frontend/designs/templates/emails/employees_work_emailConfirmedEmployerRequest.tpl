            ...
            <div class="container email email-div">
                <div style="padding: 20px;background-color: #dfdfdf;">
                    {if in_array($_request->getLanguage(),['en'])}
                        {include file="./emails/i18n/`$_request->getLanguage()`/_emailConfirmedEmployerRequest.inc.tpl"}
                    {else}
                        {include file="./emails/i18n/en/_emailConfirmedEmployerRequest.inc.tpl"}
                    {/if}
                </div>
            </div>      
        {/if}
