          ...
          <div class="row admin-header-container">
            <div class="col-sm">
              ...
              <h3 class="admin-header-small-text">{__('Customer Administrator messages management')}</h3>
            </div>
            <div class="col-sm" style="padding: 0;">
                <div style="float: right;"> 
                    {if $formFilter.equal.sender->getValue()!='ADMIN'}                       
                  <a class="btn btn-info admin-header-button btn-color"  href="#" id="Sent"><i class="fa fa-list"></i> {__('Sent messages')}</a>                                  
                  {else}
                   <a class="btn btn-info admin-header-button btn-color"  href="#" id="Received"><i class="fa fa-list"></i> {__('Received messages')}</a>                         
                   {/if}
                </div>  
            </div>
          </div>
      ...
      <tr class="list CustomerMessage" id="{$item->get('id')}" role="row">
          ...
          <td>
                ...
                {else}
                   ...
                   {else}
                        {$item->getUser()->getFormatter()->getFirstname()->ucfirst()}  {$item->getUser()->getFormatter()->getLastname()->ucfirst()}
                   {/if}    
                {/if} 
          </td>
          <td>
...
{* =================================== PAGER/FILTER ======================================================== *}
        ...
        {          
           return $.ajax2({ data: getFilterParameters(), 
                            url:"{url_to('customers_messager_ajax',['action'=>'ListPartialAdminMessage'])}" ,                          
                            target: "#actions"
                             });
        }
        ...
        $(".pager").click(function () {                     
                return $.ajax2({ data: getFilterParameters(), 
                                 url:"{url_to('customers_messager_ajax',['action'=>'ListPartialAdminMessage'])}?page="+$(this).attr('data-page'),                                
                                 target: "#actions"
                });
        });
  ...
  $("#init,#Received").click( function () {             
   ...
   $("#Sent").click(function () { 
        ...
        params.filter.equal.sender ='ADMIN';
        params.filter.notequal.sender = '';
        return $.ajax2({    
                    data : params,
                    url: "{url_to('customers_messager_ajax',['action'=>'ListPartialAdminMessage'])}",               
                    target: "#actions"
...
{* =================================== ACTIONS ======================================================== *}
      ...
      $(".View").click( function () {                    
                  return $.ajax2({ data : { CustomerMessage : $(this).attr('id') },                               
                                  url:"{url_to('customers_messager_ajax',['action'=>'ViewAdminMessage'])}",                                                       
     ...
     $(".Reply").click( function () {                    
                  return $.ajax2({ data : { CustomerMessage : $(this).attr('id') },                               
                                  url:"{url_to('customers_messager_ajax',['action'=>'ReplyAdminMessage'])}",                                                       
                                  target :"#Modal-Message-ctn",
                                  success: function () { $("#Modal-Message").show(); }
                      });        
    });
