<h4 style="margin-bottom: 20px;">{__('Leave your review for %s',$employee->get('username'))}</h4>
...
<script type="text/javascript">
    ...
    $("#SaveReview-btn").click(function () { 
           var params = { EmployeeReviewNotations : { notations :[ ], employee_user_id : '{$employee->get('username')}', employer_advert_i18n_id : '{$advert_i18n->get('id')}' , token : '{$form->getCSRFToken()}' } };
           $(".Notation").each(function () { params.EmployeeReviewNotations.notations.push({ criteria_id: $(this).attr('data-criteria'), notation: $(this).attr('data-rate') }); });
           return $.ajax2({ 
                            data : params,
                            url :"{url_to('employers_notation_ajax',['action'=>'SaveEmployeeNotation'])}",                           
