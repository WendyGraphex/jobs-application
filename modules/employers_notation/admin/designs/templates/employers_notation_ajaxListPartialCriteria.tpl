...
<div class="row admin-header-container">
    <div class="col-sm">
           <h3 class="admin-header-big-text">{__('Configurations|Employer Notation')}</h3>
           <h3 class="admin-header-small-text">{__('Manage Notation Criterias of employers')}</h3>
    </div>
    <div class="col-sm" style="padding: 0;">
        <div style="float: right;"> 
            ...
            <a href="#" class="btn btn-success admin-header-button btn-color" id="New" title="{__('New')}">
             ...
             <a href="#" class="btn btn-success admin-header-button btn-color" id="Multiple" title="{__('New Multiple Criterias')}">
                <i class="fa fa-list" style="margin-right: 5px;"></i>
                {__('New Multiple Criteria')}
            </a>
             <a href="#" class="btn btn-success admin-header-button btn-color" id="Multiple" title="{__('Download Criterias')}">
...
<div class="breadcrumb-title">
    <p>
        ...
        <i class="fa fa-angle-right" style="margin: 0 5px 0 5px; font-size: 1em;"></i>{__('Employer Notation Criterias')}
    </p>
</div>
          ...
          <tr class="list" id="{$item->get('id')}"> 
             ...
             {if $item->hasCategory()}
                 {if $item->getCategory()->hasI18n()}
                    {$item->getCategory()->getI18n()->getTitle()}
                 {/if}
             {/if}
        </td>
        <td>
    ...
    var updated_dates = $( " .Range.UpdatedAtDate#updated_at_from, .Range.UpdatedAtDate#updated_at_to" ).datepicker({
        ...
        function updatePager(n)
        {
          ...
          {* =====================  P A G E R  A C T I O N S =============================== *}  
           ...
           $("#init").click(function() {                   
               return $.ajax2({ url:"{url_to('employers_notation_ajax',['action'=>'ListPartialCriteria'])}",                                    
                         target: "#actions"}); 
           }); 
          ...
          $("[name=nbitemsbypage]").change(function() { return updateFilter(); }); 
           ...
           $(".pager").click(function () {                      
                return $.ajax2({ data: getFilterParameters(), 
                                 url:"{url_to('employers_notation_ajax',['action'=>'ListPartialCriteria'])}?"+this.href.substring(this.href.indexOf("?")+1, this.href.length),
                                target: "#actions"
                });
        });
          {* =====================  A C T I O N S =============================== *}  
          ...
          $("#New").click( function () {             
            return $.ajax2({
                ...
                url: "{url_to('employers_notation_ajax',['action'=>'NewCriteriaI18n'])}",              
                target: "#actions"
           });
         });
         
         $(".View").click( function () {                
                return $.ajax2({ data : { EmployerNotationCriteriaI18n : { 
                                                criteria_id: $(this).attr('id'),
                                                lang: $("#SelectListLanguages option:selected").attr('id')                                            
                                    } },                                
                                url:"{url_to('employers_notation_ajax',['action'=>'ViewCriteriaI18n'])}",                              
                                target: "#actions"});
         });
         
         
       $(".list").dblclick( function () {                    
                  return $.ajax2({ data : { EmployerNotationCriteriaI18n : { criteria_id: $(this).attr('id'),lang: $("#SelectListLanguages option:selected").attr('id') } },                               
                                  url:"{url_to('employers_notation_ajax',['action'=>'ViewCriteriaI18n'])}", 
                                  target: "#actions"});
          });
        ...
        $(".Delete").click( function () { 
                if (!confirm('{__("Employer Criteria \"#0#\" will be deleted. Confirm ?")}'.format($(this).attr('name')))) return false; 
                return $.ajax2({ data :{ EmployerNotationCriteria: $(this).attr('id') },
                                 url :"{url_to('employers_notation_ajax',['action'=>'DeleteCriteria'])}",                                
                                 success : function(resp) {
                                          if (resp.action != 'DeleteCriteria')  return ;                                                                              
        ...
        $("#Positions").click( function () {                
                return $.ajax2({ url:"{url_to('employers_notation_ajax',['action'=>'PositionsCriteria'])}",
                                target: "#actions"});
         });   
        ...
        $("#Multiple").click( function () {                
                return $.ajax2({  data : { lang : { lang: $("#SelectListLanguages option:selected").attr('id'),token: "{mfForm::getToken('LanguageFrontendForm')}" } },     
                                url:"{url_to('employers_notation_ajax',['action'=>'NewMultipleCriteriaI18n'])}",
                                target: "#actions"});
         });
</script>    
