        ...
        <div class="row">
            ...
            <div class="row">   
                 <div class="form-group col-md-2">
                            ...
                            <input class="EmployerNotationCriteriaI18n form-control Fields Input input-color" type="text" value="{$form.max}" name="max" id="max"/>
                        </div>
                <div class="form-group col-md-4">
                  ...
                  <div class="field_with_errors">
                      {html_options class="EmployerNotationCriteriaI18n form-control Fields Select" name="category_id" options=$form->category_id->getChoices()->toArray() selected=$form.category_id}                            
                  </div>
              </div> 
            </div>
                  <div>
            <div class="row">
                 <div class="form-group col-md-12">             
                     {if $form->hasErrors()}<div class="alert alert-{if $form.values->hasError()}danger{else}light{/if}" role="alert">&nbsp;{$form.values->getError()}</div>{/if}
                      ...
                      <div class="field_with_errors">
                          <textarea class="form-control EmployerNotationCriteriaI18n Input" name="values" cols="80" rows="5">{$form.values|escape}</textarea>
                      </div>
                 </div>                 
            </div>                                              
    </div>          
...
<script language="text/javascript"> 
   ...
   $('#Save').click(function(){                             
            var  params= {     EmployerNotationCriteriaI18nMultiple: { 
                                   lang: $("#SelectListLanguages option:selected").attr('id'),                              
                                   token :'{$form->getCSRFToken()}'
                                } };
          $(".EmployerNotationCriteriaI18n.Input").each(function() { params.EmployerNotationCriteriaI18nMultiple[$(this).attr('name')]=$(this).val(); });                
          $(".EmployerNotationCriteriaI18n.Select option:selected").each(function() {  params.EmployerNotationCriteriaI18nMultiple[$(this).parent().attr('name')]=$(this).val();  });  // Get foreign key  
          return $.ajax2({ data : params,                            
                           url: "{url_to('employers_notation_ajax',['action'=>'SaveNewMultipleCriteriaI18n'])}",
                           target: "#actions"}); 
        });                      
