     ...
     var start_at_dates = $( ".Range.StartAtDate#start_at_from, .Range.StartAtDate#start_at_to" ).datepicker({
        ...
        {           
           return $.ajax2({ data: getFilterParameters(), 
                            url:"{url_to('sales_ajax',['action'=>'ListPartialEmployerCommission'])}" ,                                                      
                            target: "#actions"
                             });
        }
        ...
        function updatePager(n)
        {
          ...
          $("[name=nbitemsbypage]").change(function() { return updateFilter(); }); 
           ...
           $(".pager").click(function () {                      
                return $.ajax2({ data: getFilterParameters(), 
                                 url:"{url_to('sales_ajax',['action'=>'ListPartialEmployerCommission'])}?"+this.href.substring(this.href.indexOf("?")+1, this.href.length),
                                target: "#actions"
                });
        });
         ...
         $(".View").click( function () {                
                return $.ajax2({ data : { SaleEmployerCommission : $(this).attr('id') },                                
                                url:"{url_to('sales_ajax',['action'=>'ViewEmployerCommission'])}",                              
                                target: "#actions"});
         });
          ...
          $(".Rates").click( function () {                
                return $.ajax2({ data : { SaleEmployerCommission : $(this).attr('id') },                                
                                url:"{url_to('sales_ajax',['action'=>'ListPartialRateForEmployerCommission'])}",                              
                                target: "#actions"});
         });
         
       $(".list").dblclick( function () {                    
                  return $.ajax2({  url:"{url_to('sales_ajax',['action'=>'ViewEmployerCommission'])}", 
                                  target: "#actions"});
          });
        ...
        $(".Delete").click( function () { 
                ...
                return $.ajax2({ data :{ SaleEmployerCommission: $(this).attr('id') },
                                 url :"{url_to('sales_ajax',['action'=>'DeleteEmployerCommission'])}",                                
                                 success : function(resp) {
                                          if (resp.action != 'DeleteEmployerCommission')  return ;                                                                              
