<div class="row admin-header-container">
  <div class="col-sm">
              ...
              <h3 class="admin-header-small-text">{__('Sales management')}</h3>
            </div>
...
<script type="text/javascript">
      ...
      function getFilterParameters()
        ...
        {          
           return $.ajax2({ data: getFilterParameters(), 
                            url:"{url_to('sales_ajax',['action'=>'ListPartialSale'])}" ,                          
                            target: "#actions"
                             });
        }
        ...
        $(".pager").click(function () {                     
                return $.ajax2({ data: getFilterParameters(), 
                                 url:"{url_to('sales_ajax',['action'=>'ListPartialSale'])}?page="+$(this).attr('data-page'),                                
                                 target: "#actions"
                });
        });
       ...
       $(".Delete").click( function () { 
                if (!confirm('{__(" \"#0#\" will be deleted. Confirm ?")}'.format($(this).attr('name')))) return false; 
                return $.ajax2({ data :{ Sale: $(this).attr('id') },
                                 url :"{url_to('sales_ajax',['action'=>'DeleteSale'])}",                               
                                 success : function(resp) {
                                       if (resp.action!='Delete') return ;                                          
  ...
  {* =====================  P A G E R  A C T I O N S =============================== *}  
           ...
           $("#init").click(function() {                   
               $.ajax2({ url:"{url_to('sales_ajax',['action'=>'ListPartialSale'])}",                                    
                         target: "#actions"}); 
           }); 
         ...
         // $("[name=name]").change(function() { return updateFilter(); }); 
           ...
           $(".pager").click(function () {                      
                return $.ajax2({ data: getFilterParameters(), 
                                 url:"{url_to('sales_ajax',['action'=>'ListPartialSale'])}?"+this.href.substring(this.href.indexOf("?")+1, this.href.length),
                                target: "#actions"
                });
        });    
