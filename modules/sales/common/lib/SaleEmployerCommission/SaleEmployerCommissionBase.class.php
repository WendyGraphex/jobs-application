...
class SaleEmployerCommissionBase extends mfObject3 {
   ...
   {
       return $this->formatter=$this->formatter===null?new SaleEmployerCommissionFormatter($this):$this->formatter;
   }
   ...
   function getRates()
   {
       if ($this->rates===null)
       {
           $this->rates=new SaleEmployerCommissionRateCollection();
           $db=mfSiteDatabase::getInstance()
                ->setParameters(array('commission_id'=>$this->get('id')))
                ->setQuery("SELECT * FROM ".SaleEmployerCommissionRate::getTable().                           
                            " WHERE commission_id='{commission_id}'".
                            ";")               
                ->makeSqlQuery(); 
        if (!$db->getNumRows())
            return $this->rates;       
        while ($item=$db->fetchObject('SaleEmployerCommissionRate'))
        {
   ...
   function updateRates(mfArray $rates)
   {
       ...
       $db=mfSiteDatabase::getInstance()
                ->setParameters(array())
                ->setQuery("DELETE FROM ".SaleEmployerCommissionRate::getTable().";")               
                ->makeSqlQuery(); 
       $this->rates=new SaleEmployerCommissionRateCollection();
       foreach ($rates as $values)
       {
           $item=new SaleEmployerCommissionRate();
