...
<html lang="{$_request->getLanguage()}">
    <head>
        ...
        <title>{component name="/site_company/Display" field='name' COMMENT=false} - {$event_i18n->getFormatter()->getMetaTitle()->ucfirst()}</title>  
    ...
    <body>
            ...
            <div class="divAllInside">
                <div class="container">
                    ...
                    <section class="ftco-section bg-light sectionCategoriesTop sectionMarginTop">
                        <div>
                            <div class="divImageEvent">
                                <h1>{$event_i18n->getFormatter()->getMetaTitle()->ucfirst()}</h1>
                                {if $event_i18n->getEvent()->hasPicture()}
                                <img title="{$event_i18n->getFormatter()->getMetaTitle()->ucfirst()}" alt="{$event_i18n->getFormatter()->getMetaTitle()->ucfirst()}" src="{$event_i18n->getEvent()->getPicture()->getUrl()}" style="max-width: 100%;"/>
                                {else}
                                {/if}    
                                <div>
                                    <h2 class="date-event">{$event_i18n->getEvent()->getFormatter()->getRange()}</h2>
                                    <p>{$event_i18n->getContent()}</p>
                                    {if $event_i18n->getEvent()->hasPlace()}
                                    <h5 class="place-event">
                                        <div>{$event_i18n->getEvent()->getPlace()->getFormatter()->getName()->ucfirst()} {$event_i18n->getEvent()->getPlace()->getFormatter()->getCity()->ucfirst()},</div>
                                        <div>{$event_i18n->getEvent()->getPlace()->getFormatter()->getCountry()->ucfirst()}</div>
                                    </h5>
                                    {/if}
                                    {component name="/site_event_cart/AddToCartBtn" event=$event_i18n->getEvent() tpl="single"}                                       
                                </div>
                            </div>
                        </div>
                    </section> 
        ...
        {component name="/site_event/dialog"}
