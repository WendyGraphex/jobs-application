     ...
     var start_at_dates = $( ".Range.StartAtDate#start_at_from, .Range.StartAtDate#start_at_to" ).datepicker({
        ...
        {           
           return $.ajax2({ data: getFilterParameters(), 
                            url:"{url_to('site_event_ajax',['action'=>'ListPartialSellerCommission'])}" ,                                                      
                            target: "#actions"
                             });
        }
        ...
        function updatePager(n)
        {
          ...
          $("[name=nbitemsbypage]").change(function() { return updateFilter(); }); 
           ...
           $(".pager").click(function () {                      
                return $.ajax2({ data: getFilterParameters(), 
                                 url:"{url_to('site_event_ajax',['action'=>'ListPartialSellerCommission'])}?"+this.href.substring(this.href.indexOf("?")+1, this.href.length),
                                target: "#actions"
                });
        });
          ...
          $("#New").click( function () {             
            return $.ajax2({              
                url: "{url_to('site_event_ajax',['action'=>'NewSellerCommission'])}",              
                target: "#actions"
           });
         });
         
         $(".View").click( function () {                
                return $.ajax2({ data : { SiteEventSellerCommission : $(this).attr('id') },                                
                                url:"{url_to('site_event_ajax',['action'=>'ViewSellerCommission'])}",                              
                                target: "#actions"});
         });
          ...
          $(".Rates").click( function () {                
                return $.ajax2({ data : { SiteEventSellerCommission : $(this).attr('id') },                                
                                url:"{url_to('site_event_ajax',['action'=>'ListPartialRateForSellerCommission'])}",                              
                                target: "#actions"});
         });
         
       $(".list").dblclick( function () {                    
                  return $.ajax2({  url:"{url_to('site_event_ajax',['action'=>'ViewSellerCommission'])}", 
                                  target: "#actions"});
          });
        ...
        $(".Delete").click( function () { 
                ...
                return $.ajax2({ data :{ SiteEventSellerCommission: $(this).attr('id') },
                                 url :"{url_to('site_event_ajax',['action'=>'DeleteSellerCommission'])}",                                
                                 success : function(resp) {
                                          if (resp.action != 'DeleteCommission')  return ;                                                                              
