          <div class="row admin-header-container">
            <div class="col-sm">
              ...
              <h3 class="admin-header-small-text">{__('Places')}</h3>
            </div>
            <div class="col-sm" style="padding: 0;">
                <div style="float: right;"> 
                    <a class="btn btn-success admin-header-button"  href="#" id="New"><i class="fa fa-plus"></i> {__('New Place')}</a> 
          ...
          <div class="breadcrumb-title">
            <p>              
                ...
                <i class="fa fa-angle-right" style="margin: 0 5px 0 5px; font-size: 1em;"></i>{__('Places')}
            </p>
          </div>
...
{alerts}              
         ...
         {*</div>  *}  
      <table id="tableSiteEventPlace" class="table table-bordered table-hover table-striped" cellpadding="0" cellspacing="0"> 
        <thead class="bgSpec">
        <tr class="list-header">    
           ...
           <tr class=" input-list">                
                <td></td>
                <td></td>                              
                <td><input  placeholder="{__('Placename, Firstname, Lastname, Email, Phone, Mobile, Address')}" class="search form-control" type="text" size="5" name="query" value="{$formFilter.search.query}"></td> 
                <td>
...
{* =================================== METHODS ======================================================== *}
  ...
  {* =====================  P A G E R  A C T I O N S =============================== *}  
           ...
           $("#init").click(function() {                   
               $.ajax2({ url:"{url_to('site_event_ajax',['action'=>'ListPartialPlace'])}",                                    
                         target: "#actions"}); 
           }); 
         ...
         // $("[name=name]").change(function() { return updateFilter(); }); 
           ...
           $(".pager").click(function () {                      
                return $.ajax2({ data: getFilterParameters(), 
                                 url:"{url_to('site_event_ajax',['action'=>'ListPartialPlace'])}?"+this.href.substring(this.href.indexOf("?")+1, this.href.length),
                                target: "#actions"
                });
        }); 
...
{* =================================== ACTIONS ======================================================== *}
      ...
      $("#New").click( function () {             
                return $.ajax2({                    
                    url: "{url_to('site_event_ajax',['action'=>'NewPlace'])}",               
                    target: "#actions"
              });
            });
         
     $(".Delete").click( function () { 
                if (!confirm('{__("SiteEventPlace \"#0#\" will be deleted. Confirm ?")}'.format($(this).attr('name')))) return false; 
                return $.ajax2({ data :{ SiteEventPlace: $(this).attr('id') },
        ...
        $(".ChangeIsActive").click(function () {
            return $.ajax2({
                 data : { SiteEventPlace : { id: $(this).attr('id') , value:$(this).attr('name'),token :'{mfForm::getToken('ChangeForm')}' } },                  
                url: "{url_to('site_event_ajax',['action'=>'ChangeIsActivePlace'])}",              
                success: function (resp) {
                    if (resp.action != 'ChangeIsActivePlace') return ;
        ...
        $(".ChangeIsValidated").click(function () {
            return $.ajax2({
                 data : { SiteEventPlace : { id: $(this).attr('id') , value:$(this).attr('name'),token :'{mfForm::getToken('ChangeForm')}' } },                  
                url: "{url_to('site_event_ajax',['action'=>'ChangeIsValidatedPlace'])}",              
                success: function (resp) {
                    if (resp.action != 'ChangeIsValidatedPlace') return ;
        ...
        $(".ChangeIsCompleted").click(function () {
            return $.ajax2({
                 data : { SiteEventPlace : { id: $(this).attr('id') , value:$(this).attr('name'),token :'{mfForm::getToken('ChangeForm')}' } },                  
                url: "{url_to('site_event_ajax',['action'=>'ChangeIsCompletedPlace'])}",              
                success: function (resp) {
                    if (resp.action != 'ChangeIsCompletedPlace') return ;
