          <div class="row admin-header-container">
            ...
            <div class="col-sm" style="padding: 0;">
                <div style="float: right;"> 
                   {* <a class="btn btn-success admin-header-button"  href="#" id="New"><i class="fa fa-plus"></i> {__('Add SiteEventUser')}</a> *}
...
{alerts}              
         ...
         {*</div>  *}  
      <table id="tableSiteEventUser" class="table table-bordered table-hover table-striped" cellpadding="0" cellspacing="0"> 
        <thead class="bgSpec">
        <tr class="list-header">    
            ...
            <th class="footable-first-column" data-toggle="true">
             ...
             {*<th data-hide="phone">{__('fax')}</th>*}

             {*<th class="footable-first-column" data-toggle="true">
                <span>{__('city')}</span>   
                <div style="float: right;">
                    <a href="#" class="order{$formFilter.order.city->getValueExist('asc','_active')}" id="asc" name="city"><i class="fas fa-sort-up"></i></a>
                    <a href="#" class="order{$formFilter.order.city->getValueExist('desc','_active')}" id="desc" name="city"><i class="fas fa-sort-down"></i></a>            
                </div>
            </th>*}
           ...
           <tr class=" input-list">                
                <td></td>
                <td></td>                              
                <td colspan="3"><input  placeholder="{__('Username, Firstname, Lastname, Email, Phone, Mobile, Address')}" class="search form-control" type="text" size="5" name="firstname" value="{$formFilter.search.firstname}"></td> 
                <td>
                ...
                <td>
                    {html_options style="height: 31px;" class="equal Select show-menu-arrow form-control" name="is_validated" options=$formFilter->equal.is_validated->getOption('choices') selected=(string)$formFilter.equal.is_active}
                </td>
                <td>
                    {html_options style="height: 31px;" class="equal Select show-menu-arrow form-control" name="is_completed" options=$formFilter->equal.is_completed->getOption('choices') selected=(string)$formFilter.equal.is_active}
                </td>
                
                {*<td><input  style="height: 36px;" class="search form-control" type="text" size="5" name="fax" value="{$formFilter.search.fax}"></td>*}
                {*<td></td>*}
                <td>
...
{* =================================== METHODS ======================================================== *}
      ...
      function getFilterParameters()
        ...
        {          
           return $.ajax2({ data: getFilterParameters(), 
                            url:"{url_to('site_event_ajax',['action'=>'ListPartialUser'])}" ,                          
                            target: "#page-wrapper"
                             });
        }
  ...
  {* =====================  P A G E R  A C T I O N S =============================== *}  
           ...
           $("#init").click(function() {                   
               $.ajax2({ url:"{url_to('site_event_ajax',['action'=>'ListPartialUser'])}",                                    
                         target: "#page-wrapper"}); 
           }); 
         ...
         // $("[name=name]").change(function() { return updateFilter(); }); 
           ...
           $(".pager").click(function () {                      
                return $.ajax2({ data: getFilterParameters(), 
                                 url:"{url_to('site_event_ajax',['action'=>'ListPartialUser'])}?"+this.href.substring(this.href.indexOf("?")+1, this.href.length),
                                target: "#page-wrapper"
                });
        }); 
...
{* =================================== ACTIONS ======================================================== *}
        ...
        $(".pager").click(function () {                     
                return $.ajax2({ data: getFilterParameters(), 
                                 url:"{url_to('site_event_ajax',['action'=>'ListPartialUser'])}?page="+$(this).attr('data-page'),                                
                                 target: "#page-wrapper"
                });
        });
      ...
      $(".View").click( function () {                    
                  return $.ajax2({ data : { SiteEventUserUser : $(this).attr('id') },                               
                                  url:"{url_to('site_event_ajax',['action'=>'ViewUser'])}", 
                                  target: "#page-wrapper"});
          });
    
      $("#New").click( function () {             
                return $.ajax2({                    
                    url: "{url_to('site_event_ajax',['action'=>'NewSiteEventUser'])}",               
                    target: "#page-wrapper"
              });
            });
         
     $(".Delete").click( function () { 
                if (!confirm('{__("SiteEventUser \"#0#\" will be deleted. Confirm ?")}'.format($(this).attr('name')))) return false; 
                return $.ajax2({ data :{ SiteEventUser: $(this).attr('id') },
                                 url :"{url_to('site_event_ajax',['action'=>'DeleteUser'])}",                               
        ...
        $(".ChangeIsActive").click(function () {
            return $.ajax2({
                 data : { SiteEventUser : { id: $(this).attr('id') , value:$(this).attr('name'),token :'{mfForm::getToken('ChangeForm')}' } },                  
                url: "{url_to('site_event_ajax',['action'=>'ChangeIsActiveUser'])}",              
                success: function (resp) {
                    if (resp.action != 'ChangeIsActiveUser') return ;
        ...
        $(".ChangeIsValidated").click(function () {
            return $.ajax2({
                 data : { SiteEventUser : { id: $(this).attr('id') , value:$(this).attr('name'),token :'{mfForm::getToken('ChangeForm')}' } },                  
                url: "{url_to('site_event_ajax',['action'=>'ChangeIsValidatedUser'])}",              
                success: function (resp) {
                    if (resp.action != 'ChangeIsValidatedUser') return ;
        ...
        $(".ChangeIsCompleted").click(function () {
            return $.ajax2({
                 data : { SiteEventUser : { id: $(this).attr('id') , value:$(this).attr('name'),token :'{mfForm::getToken('ChangeForm')}' } },                  
                url: "{url_to('site_event_ajax',['action'=>'ChangeIsCompletedUser'])}",              
                success: function (resp) {
                    if (resp.action != 'ChangeIsCompletedUser') return ;
