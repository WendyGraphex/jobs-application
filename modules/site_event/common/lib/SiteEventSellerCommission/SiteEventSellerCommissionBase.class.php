...
class SiteEventSellerCommissionBase extends mfObject3 {
   ...
   {
       return $this->formatter=$this->formatter===null?new SiteEventSellerCommissionFormatter($this):$this->formatter;
   }
   ...
   function getRates()
   {
       if ($this->rates===null)
       {
           $this->rates=new SiteEventSellerCommissionRateCollection();
           $db=mfSiteDatabase::getInstance()
                ->setParameters(array('commission_id'=>$this->get('id')))
                ->setQuery("SELECT * FROM ".SiteEventSellerCommissionRate::getTable().                           
                            " WHERE commission_id='{commission_id}'".
                            ";")               
                ->makeSqlQuery(); 
        if (!$db->getNumRows())
            return $this->rates;       
        while ($item=$db->fetchObject('SiteEventSellerCommissionRate'))
        {
   ...
   function updateRates(mfArray $rates)
   {
       ...
       $db=mfSiteDatabase::getInstance()
                ->setParameters(array())
                ->setQuery("DELETE FROM ".SiteEventSellerCommissionRate::getTable().";")               
                ->makeSqlQuery(); 
       $this->rates=new SiteEventSellerCommissionRateCollection();
       foreach ($rates as $values)
       {
           $item=new SiteEventSellerCommissionRate();
