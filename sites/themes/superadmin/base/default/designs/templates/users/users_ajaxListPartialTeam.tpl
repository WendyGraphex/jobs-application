...
<h3>{__('Teams')}</h3>    
<div>
    <a href="#" class="btn" id="{$site->getSiteID()}-UserTeam-New" title="{__('new team')}" ><i class="fa fa-plus" style="margin-right:10px;"></i>{*<img  src="{url('/icons/new.gif','picture')}" alt="{__('new')}"/>*}{__('New team')}</a>   
    <a class="btn" id="{$site->getSiteID()}-UserTeam-Import" href="#">
    ...
    <a class="btn" id="{$site->getSiteID()}-UserTeam-RemoveDuplicate" href="#">
        <i class="fa fa-caret-square-o-down" style=" margin-right: 10px"></i>
        {__('Remove Duplicate')}
    </a>
</div>
{include file="./_pagers/default/_default_pager_top.tpl" pager=$pager formfilter=$formFilter class="`$site->getSiteID()`-UserTeam"}
<button id="{$site->getSiteID()}-UserTeam-filter"  class="btn-table">{__("Filter")}</button>   
<button id="{$site->getSiteID()}-UserTeam-init"  class="btn-table">{__("Init")}</button>
<table class="tabl-list  footable table" cellpadding="0" cellspacing="0"> 
    ...
    <tr class="{$site->getSiteID()}-UserTeam list" id="{$site->getSiteID()}-UserTeam-{$item->get('id')}"> 
...
<script type="text/javascript">
	...
	$("#{$site->getSiteID()}-UserTeam-Import").click( function () { 
                return $.ajax2({
                    url: "{url_to('users_site_ajax',['action'=>'ImportTeam'])}",
                    errorTarget: ".{$site->getSiteID()}-site-errors",
                    target: "#{$site->getSiteID()}-actions"
               });
          }); 
          
             $("#{$site->getSiteID()}-UserTeam-RemoveDuplicate").click( function () {                 
                return $.ajax2({ url :"{url_to('users_site_ajax',['action'=>'RemoveDuplicateTeamUser'])}",
                                 errorTarget: ".{$site->getSiteID()}-site-errors",     
                                 loading: "#tab-site-{$site->getSiteID()}-site-x-settings-loading",
                                 success : function(resp) {
