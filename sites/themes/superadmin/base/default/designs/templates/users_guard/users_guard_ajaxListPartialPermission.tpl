...
<div>
    <a id="{$site->getSiteID()}-NewPermission" class="btn" href="#"><i class="fa fa-plus" style="margin-right:10px;"></i>{*<img  src="{url('/icons/add.gif','picture')}" alt="{__('new')}"/>*}{__('New permission')}</a>
    <a id="{$site->getSiteID()}-PermissionsGroupList" class="btn" href="#"><i class="fa fa-plus" style="margin-right:10px;"></i>{*<img  src="{url('/icons/add.gif','picture')}" alt="{__('new')}"/>*}{__('Permissions groups')}</a>
      <a class="btn" id="{$site->getSiteID()}-Permissions-Import" href="#">
        <i class="fa fa-caret-square-o-down" style="margin-right:10px;"></i>
        {*<img src="{url('/icons/import.gif','picture')}" alt="{__('import')}"/>*}{__('Import')}
    </a> 
</div>
{include file="./_pagers/default/_default_pager_top.tpl" pager=$pager formfilter=$formFilter class="`$site->getSiteID()`-Permissions"}
<button id="{$site->getSiteID()}-Permissions-filter" class="btn-table">{__("Filter")}</button>   
<button id="{$site->getSiteID()}-Permissions-init" class="btn-table">{__("Init")}</button>
<table class="tabl-list  footable table" cellpadding="0" cellspacing="0">
    <thead>
     <tr class="list-header">
        ...
        <th data-hide="phone" style="display: table-cell;">{__('id')}
         ...
         <th class="footable-first-column" data-toggle="true" class="label">{__('Name')}
          ...
          <th data-hide="phone" style="display: table-cell;">{__('Date creation')}
                <div>
          ...
          <th data-hide="phone" style="display: table-cell;">{__('Date update')}
                <div>
     ...
     <tr class="input-list">
        ...
        <tr class="{$site->getSiteID()}-Permissions list" id="{$site->getSiteID()}-Permission-{$permission->get('id')}">
            ...
            <td>
                {$permission->get('name')|escape} ({__($permission->get('name'),'','permissions')})
            </td>           
            <td>
...
<script type="text/javascript">
        ...
        function updateSite{$site->getSiteKey()}Pager(n)
        {
         ...
         $("#{$site->getSiteID()}-Permissions-Import").click( function () { 
                return $.ajax2({
                    url: "{url_to('users_guard_site_ajax',['action'=>'ImportPermission'])}",
                    loading: "#tab-site-{$site->getSiteID()}-dashboard-site-x-settings-loading",
                    errorTarget: ".{$site->getSiteID()}-site-errors",
                    target: "#{$site->getSiteID()}-actions"
               });
           ...
           $("#{$site->getSiteID()}-PermissionsGroupList").click( function () { 
                return $.ajax2({    loading: "#tab-site-{$site->getSiteID()}-dashboard-site-x-settings-loading",
