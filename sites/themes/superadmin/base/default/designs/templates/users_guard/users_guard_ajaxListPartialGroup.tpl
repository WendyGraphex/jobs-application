...
<div>
    <a id="{$site->getSiteID()}-New" class="btn" href="#"><i class="fa fa-plus" style="margin-right:10px;"></i>{*<img  src="{url('/icons/add.gif','picture')}" alt="{__('new')}"/>*}{__('New group')}</a>
       <a class="btn" id="{$site->getSiteID()}-Groups-Import" href="#">
...
<button id="{$site->getSiteID()}-filter" class="btn-table">{__("Filter")}</button>   
<button id="{$site->getSiteID()}-init" class="btn-table">{__("Init")}</button>
...
<script type="text/javascript">
        ...
        function changeSite{$site->getSiteKey()}GroupState(resp) 
        {
            $.each(resp.selection?resp.selection:[resp.id], function (id) { 
                sel="a.{$site->getSiteID()}-ChangeGroup[id="+this+"]";
        ...
        function updateSite{$site->getSiteKey()}Pager(n)
        {
         ...
         $("#{$site->getSiteID()}-Groups-Import").click( function () { 
                return $.ajax2({
                    url: "{url_to('users_guard_site_ajax',['action'=>'ImportGroup'])}",
                    errorTarget: ".{$site->getSiteID()}-site-errors",
                    target: "#{$site->getSiteID()}-actions"
               });
          }); 
