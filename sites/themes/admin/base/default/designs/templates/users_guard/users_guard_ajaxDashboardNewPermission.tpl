...
<table cellpadding="0" cellspacing="0">
    ...
    <tr id="permission_{$smarty.section.index.index}" class="permission_form">
        <td>#{$smarty.section.index.index+1}</td>
        <td>{__("name")}</td>
        <td> 
            <div>&nbsp;{$form['permissions'][$smarty.section.index.index]['name']->getError()}</div> 
...
<div>
   <a id="Add" href="#"  style="{if (string)$form['permissions_count']>=$form->permissions_count->getOption('max')}display:none{/if}"><img  src="{url('/icons/add.gif','picture')}" alt="{__('new')}"/>{__('add permission')|capitalize}</a>
</div>
<script type="text/javascript">
        ...
        $('#Save').click(function(){ 
              ...
              $(".Delete").off('click');
              $.ajax2({ data: params, url: "{url_to('users_guard_ajax',['action'=>'DashboardNewPermission'])}",target: "#actions"}); 
         });  
