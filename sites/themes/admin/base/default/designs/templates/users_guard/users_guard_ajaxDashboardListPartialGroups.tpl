...
<button id="DashboardGroups-filter">{__("filter")|capitalize}</button>   <button id="DashboardGroups-init">{__("init")|capitalize}</button>
<table cellpadding="0" cellspacing="0">
     ...
     <tr>
        ...
        <tr class="groups" id="group_{$group->get('id')}">
...
<script type="text/javascript">
        ...
        {
           return $.ajax2({ data: getGroupsFilterParameters(), url:"{url_to('users_guard_ajax',['action'=>'DashboardListPartialGroups'])}" , target: "#actions" });
        }
    ...
    {JqueryScriptsReady}        
         ...
         $('.DashboardGroups-order').click(function() {
          ...
          $("#DashboardGroups-init").click(function() { $.ajax2({ url:"{url_to('users_guard_ajax',['action'=>'DashboardListPartialGroups'])}",target: "#actions"}); }); 
                             
          $(".DashboardGroups-pager").click(function () {
                    return $.ajax2({ data: getGroupsFilterParameters(), url:"{url_to('users_guard_ajax',['action'=>'DashboardListPartialGroups'])}?"+this.href.substring(this.href.indexOf("?")+1, this.href.length),target: "#actions"});
          });
                              ...
                              success: function(resp) {
                                            ...
                                            {                                    
                                                if ($(".groups").length-resp.parameters.length==0)
                                                   return $.ajax2({ url: "{url_to('users_guard_ajax',['action'=>'DashboardListPartial'])}",target: "#actions"});
                          ...
                          success: function(resp) {
                                            ...
                                            {    
                                                if ($(".groups").length-1==0)
                                                   return $.ajax2({ url:"{url_to('users_guard_ajax',['action'=>'DashboardListPartialGroups'])}",target: "#actions"});
          ...
          $(".View").click( function () { 
                return $.ajax2({ data : { id: this.id },url: "{url_to('users_guard_ajax',['action'=>'DashboardViewGroup'])}",target: "#actions"});
          });
          
          $("#New").click( function () { 
                return $.ajax2({ url:"{url_to('users_guard_ajax',['action'=>'DashboardNewGroup'])}",target: "#actions"});
          });
          ...
          $(".ViewPermissions").click( function () { 
                 return $.ajax2({ data: { group: this.id },url: "{url_to('users_guard_ajax',['action'=>'DashboardPermissionsList'])}",target: "#actions"});
          });
