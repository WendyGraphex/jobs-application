{messages}
    <div class="row admin-header-container">
      <div class="col-sm">
        <h3 class="admin-header-big-text">{__('Text Models')}</h3>
        <h3 class="admin-header-small-text">{__('Text Models management')}</h3>
      </div>
      <div class="col-sm" style="padding: 0;">
          <div style="float: right;">                                     
            <a class="btn btn-success admin-header-button"  href="#" id="New"><i class="fa fa-plus"></i> {__('New model')}</a>
          </div> 
      </div>
    </div>
    ...
    <div class="breadcrumb-title">
      <p>
          ...
          <i class="fa fa-angle-right" style="margin: 0 5px 0 5px; font-size: 1em;"></i>{__('Text Models')}
      </p>
    </div>
    ...
    <table class="table table-bordered table-hover table-striped" cellpadding="0" cellspacing="0">
        ...
        <tr class="TextModel-list list" id="{$item->get('id')}"> 
            <td class="TextModel-count">{(($pager->getPage()-1)*$pager->getNbItemsByPage())+$item@iteration}</td>
            <td>{$item->get('id')}</td> 
                {if $pager->getNbItems()>5}
                    <td>        
                        {if $item->hasSystemModelTextI18n()}
                        <input class="selection" type="checkbox" id="{$item->getSystemModelTextI18n()->get('id')}" name="{$item->getSystemModelTextI18n()->get('name')}"/>   
                        {/if}
                    </td>
                {/if}          
                <td>                
                        {$item->getSystemModelText()->get('name')}
                </td>            
                <td>
                    {if $item->hasSystemModelTextI18n()}
                         {$item->getSystemModelTextI18n()->get('value')}
                    {else}
                        {__('----')}
                    {/if}    
                </td>
                <td>
                    {if $item->hasSystemModelTextI18n()}
                         {$item->getSystemModelTextI18n()->get('created_at')}
                    {else}
                        {__('----')}
                    {/if}    
                </td>
                <td>
                    {if $item->hasSystemModelTextI18n()}
                         {$item->getSystemModelTextI18n()->get('updated_at')}
                    {else}
                        {__('----')}
                    {/if}    
                </td>
                <td>               
                    <a href="#" title="{__('Edit')}" class="View" id="{$item->getSystemModelText()->get('id')}">
                        <i class="fa fa-pencil-alt"></i> {*<img  src="{url('/icons/edit.gif','picture')}" alt='{__("Edit")}'/>*}</a>              
                    <a href="#" title="{__('Delete')}" class="Delete" id="{$item->get('id')}"  name="{if $item->hasSystemModelTextI18n()}{$item->getSystemModelTextI18n()->get('value')}{/if}">
                       <i class="fa fa-trash"></i>{*<img  src="{url('/icons/delete.gif','picture')}" alt='{__("Delete")}'/>*}
                    </a>                  
                    {*<a href="#" class="list-action text-info View" id="{$item->getSystemModelText()->get('id')}"  data-toggle="modal"  data-target="#exampleModal" title="{__('Edit')}"><i class="fa fa-pencil-alt"></i></a>
                    <a href="#" class="list-action text-danger Delete" id="{$item->get('id')}" name="{$item->getSystemModelTextI18n()->get('value')}" data-toggle="tooltip" title="{__('Delete')}"><i class="fa fa-trash-alt"></i></a>
               *}
                </td>
        </tr>    
        {/foreach}    
    ...
    {else}
        {if $pager->getNbItems()>5}
            <input type="checkbox" id="TextModel-all" /> 
              <a style="opacity:0.5" class="TextModel-actions_items" href="#" title="{__('delete')}" id="TextModel-Delete">
    ...
    var updated_dates = $( ".Range.UpdatedAtDate#updated_at_from, .Range.UpdatedAtDate#updated_at_to" ).datepicker({
        ...
        function updatePager(n)
        {
          ...
          {* =====================  P A G E R  A C T I O N S =============================== *}  
           ...
           $("#init").click(function() {               
               return $.ajax2({ url:"{url_to('system_texts_ajax',['action'=>'ListPartialModel'])}",                                              
                         target: "#actions"}); 
           }); 
          ...
          $("[name=nbitemsbypage],#SelectListLanguages").change(function() { return updateFilter(); }); 
           ...
           $(".pager").click(function () {      
                ...
                return $.ajax2({ data: getFilterParameters(), 
                                 url:"{url_to('system_texts_ajax',['action'=>'ListPartialModel'])}?"+this.href.substring(this.href.indexOf("?")+1, this.href.length),                                
                                 target: "#actions"
                });
        });
          {* =====================  A C T I O N S =============================== *}  
          ...
          $("#New").click( function () {            
            return $.ajax2({
                ...
                url: "{url_to('system_texts_ajax',['action'=>'NewModelI18n'])}",              
                target: "#actions"
           });
         });
         
         $(".View").click( function () {               
                return $.ajax2({ data : { SystemModelTextI18n : { 
                                                model_id: $(this).attr('id'),
                                                  lang: $("#SelectListLanguages option:selected").attr('id')                                   
                                    } },                              
                                url:"{url_to('system_texts_ajax',['action'=>'ViewModelI18n'])}",                              
                                target: "#actions"});
         });                        
        ...
        $(".list").dblclick( function () {                             
          ...
          $(".Delete").click( function () { 
                ...
                return $.ajax2({ data :{ SystemModelText: $(this).attr('id') },
                                 url :"{url_to('system_texts_ajax',['action'=>'DeleteModel'])}",                              
